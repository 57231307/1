...
- **高性能：基于Rust语言开发，性能优异**
- **高安全性：JWT认证、权限管理、请求验证、CORS配置、安装向导安全锁**
- **高可扩展性：模块化设计，易于添加新功能**
...
秉羲管理系统提供了一套安全、智能的系统初始化向导，旨在简化初次部署过程中的数据库配置和基础数据写入。

### 初始化机制
- **智能探测**：系统启动时，会自动尝试读取 `.env` 配置并连接数据库。同时系统会智能查询数据库中是否已有核心数据（如 `users` 表是否存在数据）。
- **向导模式**：若未连接成功或检测到数据库为空，后端将启动为**安装向导模式**，此时仅开放 `/init/*` 配置接口。用户可直接在网页前端填写数据库配置并创建超级管理员。
- **配置持久化与热重启**：用户提交配置后，后端不仅执行数据库迁移，还会将连接信息自动安全写入 `.env` 文件。初始化进度达到100%后，后端将自动退出进程并触发热重启。
- **安全锁死（防重装机制）**：一旦系统重启并检测到数据库已被成功配置且存在数据，后端将启动为**完整业务模式**，并且**永久销毁/屏蔽所有配置引导接口（`/init/initialize-with-db` 等）**，前端也会立刻将试图访问向导页面的用户强制重定向至登录页。**除非人为清空数据库**，否则引导页面绝对无法再次运行，从根本上杜绝了生产环境被恶意重置的风险。
