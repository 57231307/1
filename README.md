...
系统支持通过邮件发送业务事件通知（如订单状态变更、审批提醒、库存预警等）。如需启用邮件通知，需配置以下环境变量：

```bash
# 邮件服务提供商：sendgrid, aliyun, tencent
...
# 发件人名称（可选，默认为"系统通知"）
export EMAIL_FROM_NAME="秉羲面料管理系统"

# 自定义 API 端点（可选，用于私有部署）
# export EMAIL_API_URL="https://custom-api.example.com/v1/mail/send"
```

**配置说明**：
- `EMAIL_PROVIDER`：目前支持 `sendgrid`（完整实现）、`aliyun`（框架预留）、`tencent`（框架预留）
- `EMAIL_API_KEY`：对应邮件服务商的 API 密钥
- `EMAIL_FROM`：系统发件邮箱地址
- `EMAIL_FROM_NAME`：邮件中显示的发件人名称

**验证邮件服务**：
邮件服务配置成功后，系统启动时会自动检测。如果环境变量未配置，邮件通知功能将自动禁用，仅保留站内通知，不影响其他功能。
...
当前邮件通知使用硬编码邮箱 `user@example.com`，建议从用户表中读取真实邮箱地址，实现精准邮件推送。

### 2. 用户通知偏好设置

建议添加用户级通知偏好配置，支持以下设置：
- 是否接收邮件通知（全局开关）
- 各类事件的通知方式（站内通知 / 邮件 / 全部）
- 通知频率控制（实时 / 摘要 / 关闭）
...
各模块可配置独立的数据范围（全部 / 本部门 / 部门及以下 / 仅本人 / 自定义）和字段级权限。
