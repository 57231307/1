...
- **契约一致性脚本**（尚未接入 CI，是否升级为阻断项待评审）：check-api-paths（前端调用 ↔ 后端路由，A 类失配 0、22 条功能缺口逐条显式登记原因）、check-api-envelope（响应信封形状，106 条可比对：**现存失配 0 条**、28 条后端手搓 `serde_json::Value` 载荷无法静态判定，判负口径为“解析不出即失败”，不静默放行）
