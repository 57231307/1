...
- **契约一致性脚本**（尚未接入 CI，是否升级为阻断项待评审）：check-api-paths（前端调用 ↔ 后端路由，A 类失配 0、22 条功能缺口逐条显式登记原因）、check-api-envelope（响应信封形状，101 条可比对：**失配 0 / 未分类 0 / handler 符号未定义 0**；后端标注 `serde_json::Value` 的 28 条已按「函数体真实构造形态」全部还原——`to_value(具体类型)`、`json!({...})` 顶层键、变量与元组回溯、`define_*_handlers!` 宏展开、`pub use` 转出、必要时递归进 service；判负口径为「解析不出即失败」，不静默放行。盲区 386 条主要是前端把 data 声明为具名 TS 接口（269 条）而本门禁不展开其定义，故「0 失配」不等于全部消费点已核对）
