...
冰溪 ERP 系统是**面向纺织行业的全栈式企业资源计划系统**，通过 5 大业务域（采购/销售/库存/生产/财务）+ 5 大行业特性子模块（报价/色卡/多色号/定制订单/主备隔离）+ 4 大智能能力（AI 分析 / BI / BPM / 多租户）的组合，为面料纺织企业提供端到端数字化解决方案。
...
- **CSP**：严格策略 + 违规上报
- **密码策略**：复杂度 + 历史 + 过期 + 锁定
- **多租户**：行级安全 + 严格隔离（`extract_tenant_id`）
- **认证授权**：JWT + RBAC + 字段级权限
- **审计日志**：登录 / 操作 / 数据变更全记录
...
打开浏览器访问 [http://localhost:5173](http://localhost:5173)，使用以下默认账号登录：
