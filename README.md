...
| 前端 Views 子模块 | 110 个                                                                                                                        |
| 前端 API 模块     | 121 个                                                                                                                        |
| 前端 i18n 翻译键  | zh-CN 10,168 / en-US 10,191（引用键 9,667，缺失校验 0；常量表内的键名由 `tests/unit/translated-value-select.test.ts` 校验）                                                                       |
...
| **前端 E2E 合计**     | **1,204**             | —                     | 240 个 spec 文件（chromium project，CI 实际执行口径）                                |
...
> `webkit` 与 chromium 同集，故三 project 全量为 1,204 × 2 + 126 = 2,534。
...
- **完整性校验**：check-i18n（zh/en 键对齐 0 缺失）+ check-contract（前后端契约映射 0 错误）纳入质量门禁
