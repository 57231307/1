...
use bingxi_backend::utils::cache::{Cache, MemoryCache, CacheStats, CacheKey};
use std::thread;
...
fn test_cache_ttl_not_expired() {
    ...
    cache.set("key1".to_string(), "value1".to_string(), Some(Duration::from_secs(10)));
    assert_eq!(cache.get(&"key1".to_string()), Some("value1".to_string()));
}
...
fn test_cache_cleanup_expired() {
    ...
    cache.set("key1".to_string(), "value1".to_string(), Some(Duration::from_millis(1)));
    cache.set("key2".to_string(), "value2".to_string(), None);
...
fn test_cache_key_to_string() {
    assert_eq!(CacheKey::DashboardOverview("7d".to_string()).to_string(), "dashboard:overview:7d");
    assert_eq!(CacheKey::LowStockAlerts.to_string(), "inventory:low_stock");
    assert_eq!(CacheKey::ProductDetails(42).to_string(), "products:details:42");
