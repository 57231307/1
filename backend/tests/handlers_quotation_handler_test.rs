...
mod tests {
    ...
    fn make_quotation_model(id: i32, status: &str) -> QuotationModel {
        QuotationModel {
            ...
            quotation_date: Utc::now().naive_utc(),
    ...
    fn test_conversion_to_order() {
        ...
        quotation.conversion_date = Some(Utc::now().naive_utc());
    ...
    fn test_valid_until_expired() {
        ...
        quotation.valid_until = Some(Utc::now().naive_utc() - chrono::Duration::days(30));

        // 验证已过期
        assert!(quotation.valid_until.unwrap() < Utc::now().naive_utc());
    }
