...
fn fetch_with_mirrors(api_path: &str, timeout: u32) -> Option<String> {
    ...
    // 1. 尝试直连
    if let Ok(output) = run_cmd("curl", &[
        "-s", "-m", &timeout.to_string(),
...
fn cmd_status() {
    ...
    if let Ok(ss) = run_cmd("ss", &["-tlnp"]) {
        println!("{} 端口 8082 (后端)", if ss.contains(":8082") { "[OK]" } else { "[STOPPED]" });
        println!("{} 端口 80 (HTTP)", if ss.contains(":80 ") { "[OK]" } else { "[STOPPED]" });
    }
...
fn cmd_backup(backup_type: &str) {
    ...
    if backup_type == "database" || backup_type == "all" {
        ...
        match run_cmd("pg_dump", &["-h", &db_host, "-U", &db_user, "-d", &db_name, "-f", &db_file]) {
...
fn cmd_health() {
    ...
    match run_cmd("curl", &["-s", "-o", "/dev/null", "-w", "%{http_code}", "http://127.0.0.1:8082/api/v1/erp/health"]) {
        Ok(code) => println!("{} HTTP 状态码: {}", if code == "200" { "[OK]" } else { "[WARN]" }, code),
    ...
    match run_cmd("redis-cli", &["ping"]) {
        Ok(out) => println!("{} Redis: {}", if out.contains("PONG") { "[OK]" } else { "[WARN]" }, out),
...
fn cmd_upgrade(version: Option<String>, no_backup: bool) {
    ...
    if !download_with_mirrors(&release_url, &download_path, 120) {
        println!("\n[ERROR] 下载失败");
        println!("\n请手动下载:");
        println!("  curl -fsSL -o /tmp/release-{}.tar.gz {}", target, release_url);
        println!("\n然后执行:");
