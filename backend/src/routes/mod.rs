...
use crate::handlers::{
    auth_handler, init_handler, user_handler, role_handler, product_handler,
    product_category_handler, warehouse_handler, department_handler,
    dashboard_handler, finance_payment_handler, finance_invoice_handler,
    sales_order_handler, sales_fabric_order_handler,
    inventory_stock_handler, inventory_transfer_handler,
    inventory_count_handler, inventory_adjustment_handler, batch_handler,
    batch_new_handler, customer_handler, account_subject_handler, voucher_handler,
    dual_unit_converter_handler, five_dimension_handler,
    assist_accounting_handler, business_trace_handler, health_handler,
    supplier_handler, purchase_order_handler, purchase_receipt_handler,
    purchase_return_handler, purchase_inspection_handler,
    ap_invoice_handler, ap_payment_request_handler, ap_payment_handler,
    ap_verification_handler, ap_reconciliation_handler, ap_report_handler,
    ar_invoice_handler, cost_collection_handler,
    // P1 模块
    fixed_asset_handler, purchase_contract_handler, sales_contract_handler, customer_credit_handler,
    fund_management_handler, budget_management_handler, quality_standard_handler,
    // P2 模块
    financial_analysis_handler, supplier_evaluation_handler,
    purchase_price_handler, sales_price_handler,
    sales_analysis_handler, quality_inspection_handler,
};
...
pub fn create_router(db: Arc<DatabaseConnection>) -> Router {
    ...
    // 产品管理路由
        let product_routes = Router::new()
            ...
            // 色号管理路由
            .route("/:product_id/colors", get(product_handler::list_product_colors))
            .route("/:product_id/colors", post(product_handler::create_product_color))
            .route("/:product_id/colors/:color_id", put(product_handler::update_product_color))
            .route("/:product_id/colors/:color_id", delete(product_handler::delete_product_color))
            .route("/:product_id/colors/batch", post(product_handler::batch_create_colors));
    ...
    let product_category_routes = Router::new()
        ...
        .route("/:id", put(product_category_handler::update_product_category))
        .route("/:id", delete(product_category_handler::delete_product_category))
    ...
    let dashboard_routes = Router::new()
        ...
        .route("/inventory-stats", get(dashboard_handler::get_inventory_statistics))
        .route("/low-stock-alerts", get(dashboard_handler::get_low_stock_alerts));
    ...
    let finance_routes = Router::new()
        ...
        .route("/invoices/:id", put(finance_invoice_handler::update_invoice))
        .route("/invoices/:id", delete(finance_invoice_handler::delete_invoice))
        .route("/invoices/:id/approve", post(finance_invoice_handler::approve_invoice))
        .route("/invoices/:id/verify", post(finance_invoice_handler::verify_invoice));
    ...
    let sales_routes = Router::new()
        ...
        // 面料行业销售订单路由
        .route("/fabric-orders", get(sales_fabric_order_handler::list_fabric_orders))
        .route("/fabric-orders", post(sales_fabric_order_handler::create_fabric_order))
        .route("/fabric-orders/:id", get(sales_fabric_order_handler::get_fabric_order))
        .route("/fabric-orders/:id", put(sales_fabric_order_handler::update_fabric_order))
        .route("/fabric-orders/:id", delete(sales_fabric_order_handler::delete_fabric_order))
        .route("/fabric-orders/:id/approve", post(sales_fabric_order_handler::approve_fabric_order));
    ...
    // 库存管理路由
        let inventory_routes = Router::new()
            ...
            // 面料行业库存接口
            .route("/stock/fabric", get(inventory_stock_handler::list_stock_fabric))
            .route("/stock/fabric", post(inventory_stock_handler::create_stock_fabric))
            .route("/stock/transactions", get(inventory_stock_handler::list_transactions))
            .route("/stock/summary", get(inventory_stock_handler::get_inventory_summary))
            .route("/transfers", get(inventory_transfer_handler::list_transfers))
            .route("/transfers", post(inventory_transfer_handler::create_transfer))
            .route("/transfers/:id", get(inventory_transfer_handler::get_transfer))
            .route("/transfers/:id", put(inventory_transfer_handler::update_transfer))
            .route("/transfers/:id/approve", post(inventory_transfer_handler::approve_transfer))
            .route("/transfers/:id/ship", post(inventory_transfer_handler::ship_transfer))
            .route("/transfers/:id/receive", post(inventory_transfer_handler::receive_transfer))
            ...
            .route("/adjustments", get(inventory_adjustment_handler::list_adjustments))
            .route("/adjustments", post(inventory_adjustment_handler::create_adjustment))
            .route("/adjustments/:id", get(inventory_adjustment_handler::get_adjustment))
            .route("/adjustments/:id/approve", post(inventory_adjustment_handler::approve_adjustment))
            .route("/adjustments/:id/reject", post(inventory_adjustment_handler::reject_adjustment));
    ...
    let dual_unit_routes = Router::new()
        .route("/convert", post(dual_unit_converter_handler::convert_dual_unit))
        .route("/validate", post(dual_unit_converter_handler::validate_dual_unit));

    // 五维管理路由
    let five_dimension_routes = Router::new()
        .route("/stats", get(five_dimension_handler::get_five_dimension_stats))
        .route("/list", get(five_dimension_handler::list_five_dimension_stats))
        .route("/search", get(five_dimension_handler::search_five_dimension))
        .route("/:five_dimension_id", get(five_dimension_handler::get_stats_by_five_dimension_id))
        .route("/parse", post(five_dimension_handler::parse_five_dimension_id));

    // 辅助核算路由
    let assist_accounting_routes = Router::new()
        .route("/dimensions", get(assist_accounting_handler::list_assist_dimensions))
        .route("/records", get(assist_accounting_handler::query_assist_records))
        .route("/records/business", get(assist_accounting_handler::get_assist_records_by_business))
        .route("/records/five-dimension/:five_dimension_id", get(assist_accounting_handler::get_assist_records_by_five_dimension))
        .route("/summary", get(assist_accounting_handler::get_assist_summary));

    // 业务追溯路由
    let business_trace_routes = Router::new()
        .route("/five-dimension/:five_dimension_id", get(business_trace_handler::get_trace_by_five_dimension))
        .route("/forward", get(business_trace_handler::forward_trace))
        .route("/backward", get(business_trace_handler::backward_trace))
        .route("/snapshot/:trace_chain_id", post(business_trace_handler::create_trace_snapshot));
    ...
    let supplier_routes = Router::new()
        ...
        // 联系人管理
        .route("/:id/contacts", get(supplier_handler::list_supplier_contacts))
        .route("/:id/contacts", post(supplier_handler::create_supplier_contact))
        .route("/:id/contacts/:contact_id", put(supplier_handler::update_supplier_contact))
        .route("/:id/contacts/:contact_id", delete(supplier_handler::delete_supplier_contact))
        // 资质管理
        .route("/:id/qualifications", get(supplier_handler::list_supplier_qualifications))
        .route("/:id/qualifications", post(supplier_handler::create_supplier_qualification));
    ...
    let purchase_order_routes = Router::new()
        ...
        .route("/:id/items", post(purchase_order_handler::create_order_item))
        .route("/:id/items/:item_id", put(purchase_order_handler::update_order_item))
        .route("/:id/items/:item_id", delete(purchase_order_handler::delete_order_item));
    ...
    let purchase_receipt_routes = Router::new()
        ...
        .route("/:id/items", get(purchase_receipt_handler::list_receipt_items))
        .route("/:id/items", post(purchase_receipt_handler::create_receipt_item))
        .route("/:id/items/:item_id", put(purchase_receipt_handler::update_receipt_item))
        .route("/:id/items/:item_id", delete(purchase_receipt_handler::delete_receipt_item));
    ...
    let purchase_return_routes = Router::new()
        ...
        .route("/:id/approve", post(purchase_return_handler::approve_return))
    ...
    let purchase_inspection_routes = Router::new()
        ...
        .route("/:id/complete", post(purchase_inspection_handler::complete_inspection));
    ...
    let purchase_contract_routes = Router::new()
        ...
        .route("/:id/execute", post(purchase_contract_handler::execute_contract))
        .route("/:id/cancel", post(purchase_contract_handler::cancel_contract));
    ...
    let sales_contract_routes = Router::new()
        ...
        .route("/:id/execute", post(sales_contract_handler::execute_contract))
    ...
    let customer_credit_routes = Router::new()
        ...
        .route("/:customer_id/occupy", post(customer_credit_handler::occupy_credit))
        .route("/:customer_id/release", post(customer_credit_handler::release_credit))
        .route("/:customer_id/adjust", post(customer_credit_handler::adjust_credit_limit))
        .route("/:customer_id/deactivate", post(customer_credit_handler::deactivate_credit));
    ...
    let fund_management_routes = Router::new()
        ...
        .route("/:id/unfreeze", post(fund_management_handler::unfreeze_funds))
    ...
    let budget_management_routes = Router::new()
        ...
        .route("/controls/:id/data", get(budget_management_handler::get_budget_control_data))
        .route("/controls/:id/executions", get(budget_management_handler::get_plan_executions));
    ...
    let financial_analysis_routes = Router::new()
        .route("/indicators", get(financial_analysis_handler::list_indicators))
        .route("/indicators", post(financial_analysis_handler::create_indicator))
        .route("/results", post(financial_analysis_handler::create_analysis_result))
    ...
    let supplier_evaluation_routes = Router::new()
        ...
        .route("/evaluations", post(supplier_evaluation_handler::create_evaluation_record))
        .route("/evaluations", get(supplier_evaluation_handler::list_evaluation_records))
        .route("/evaluations/:id", get(supplier_evaluation_handler::get_evaluation_record))
    ...
    let purchase_price_routes = Router::new()
        ...
        .route("/history/:material_id", get(purchase_price_handler::get_price_history));
    ...
    let quality_inspection_routes = Router::new()
        ...
        .route("/defects/:id", post(quality_inspection_handler::process_defect));
    ...
    let ap_invoice_routes = Router::new()
        ...
        .route("/balance-summary", get(ap_invoice_handler::get_balance_summary))
    ...
    let ap_payment_request_routes = Router::new()
        ...
        .route("/:id/submit", post(ap_payment_request_handler::submit_request))
        .route("/:id/approve", post(ap_payment_request_handler::approve_request))
        .route("/:id/reject", post(ap_payment_request_handler::reject_request));
    ...
    let ap_verification_routes = Router::new()
        ...
        .route("/:id/cancel", post(ap_verification_handler::cancel_verification))
        .route("/unverified-invoices", get(ap_verification_handler::get_unverified_invoices))
        .route("/unverified-payments", get(ap_verification_handler::get_unverified_payments));

    // 供应商对账路由
    let ap_reconciliation_routes = Router::new()
        .route("/", get(ap_reconciliation_handler::list_reconciliations))
        .route("/", post(ap_reconciliation_handler::generate_reconciliation))
        .route("/:id", get(ap_reconciliation_handler::get_reconciliation))
        .route("/:id/confirm", post(ap_reconciliation_handler::confirm_reconciliation))
        .route("/:id/dispute", post(ap_reconciliation_handler::dispute_reconciliation))
        .route("/summary", get(ap_reconciliation_handler::get_supplier_summary));
