...
pub fn create_router(state: AppState) -> Router {
    ...
    let purchase_routes = Router::new()
        ...
        .route("/returns/:id/items", get(purchase_return_handler::list_purchase_return_items))
        .route("/returns/:id/items", post(purchase_return_handler::create_purchase_return_item))
        .route("/returns/:id/items/:item_id", put(purchase_return_handler::update_purchase_return_item))
        .route("/returns/:id/items/:item_id", delete(purchase_return_handler::delete_purchase_return_item));
    ...
    let budget_management_routes = Router::new()
        ...
        .route("/items", post(budget_management_handler::create_budget_item))
        .route("/items/:id", get(budget_management_handler::get_budget_item))
        .route("/items/:id", put(budget_management_handler::update_budget_item))
        .route("/items/:id", delete(budget_management_handler::delete_budget_item))
    ...
    Router::new()
        ...
        .nest("/api/v1/erp/supplier-evaluation/evaluations", supplier_evaluation_routes)
        ...
        .nest("/api/v1/erp/production", Router::new()
            ...
            .route("/orders/:id", get(production_order_handler::get_production_order).put(production_order_handler::update_production_order))
            .route("/orders/:id/status", put(production_order_handler::update_production_order_status))
            .route("/orders/:id/submit-approval", post(production_order_handler::submit_for_approval))
            .route("/orders/:id/approve", post(production_order_handler::approve_production_order))
        )
        // BOM物料清单路由
        .nest("/api/v1/erp/boms", Router::new()
            ...
            .route("/:id/requirements", post(bom_handler::calculate_bom_requirements))
        ...
        .nest("/api/v1/erp/scheduling", Router::new()
            ...
            .route("/results/:id/confirm", post(scheduling_handler::confirm_schedule_result))
        )
        // 产能分析路由
        ...
        .nest("/api/v1/erp/material-shortage", Router::new()
            .route("/alerts", get(material_shortage_handler::list_shortage_alerts))
            .route("/check", post(material_shortage_handler::check_material_shortage))
            .route("/summary", get(material_shortage_handler::get_shortage_summary))
            .route("/threshold", get(material_shortage_handler::get_threshold_config).post(material_shortage_handler::save_threshold_config))
            .route("/replenishment", get(material_shortage_handler::get_replenishment_suggestions))
        )
        // BPM流程定义路由
        .nest("/api/v1/erp/bpm/definitions", Router::new()
            .route("/", get(bpm_definition_handler::list_process_definitions).post(bpm_definition_handler::create_process_definition))
            .route("/:id", get(bpm_definition_handler::get_process_definition).put(bpm_definition_handler::update_process_definition).delete(bpm_definition_handler::delete_process_definition))
            .route("/:id/copy", post(bpm_definition_handler::create_version))
            .route("/:id/versions", get(bpm_definition_handler::list_versions))
            .route("/:id/versions/:version/activate", post(bpm_definition_handler::activate_version))
            .route("/:id/template", post(bpm_definition_handler::save_as_template))
            .route("/templates", get(bpm_definition_handler::list_templates))
            .route("/templates/:template_id", post(bpm_definition_handler::create_from_template))
        )
        // CRM客户管理路由
        ...
        // 应收对账增强路由
        .nest("/api/v1/erp/ar-reconciliations/enhanced", Router::new()
            .route("/auto-match", post(ar_reconciliation_enhanced_handler::auto_match))
            .route("/aging-report", get(ar_reconciliation_enhanced_handler::aging_report))
            .route("/:id/details", get(ar_reconciliation_enhanced_handler::get_reconciliation_details))
            .route("/:id/confirm", post(ar_reconciliation_enhanced_handler::confirm_reconciliation))
            .route("/:id/dispute", post(ar_reconciliation_enhanced_handler::dispute_reconciliation))
            .route("/:id/pdf", get(ar_reconciliation_enhanced_handler::export_reconciliation_pdf))
            .route("/generate", post(ar_reconciliation_enhanced_handler::generate_reconciliation))
        )
        // 报表增强路由
        .nest("/api/v1/erp/reports/enhanced", Router::new()
            .route("/templates", get(report_enhanced_handler::list_report_templates).post(report_enhanced_handler::create_report_template))
            .route("/templates/:id", get(report_enhanced_handler::get_report_template).put(report_enhanced_handler::update_report_template).delete(report_enhanced_handler::delete_report_template))
            .route("/templates/:id/execute", post(report_enhanced_handler::execute_custom_report))
            .route("/export/pdf", post(report_enhanced_handler::export_pdf))
            .route("/export/excel", post(report_enhanced_handler::export_excel))
            .route("/subscriptions", get(report_enhanced_handler::list_subscriptions).post(report_enhanced_handler::create_subscription))
            .route("/subscriptions/:id", get(report_enhanced_handler::get_subscription).put(report_enhanced_handler::update_subscription).delete(report_enhanced_handler::delete_subscription))
            .route("/subscriptions/:id/toggle", post(report_enhanced_handler::toggle_subscription))
            .route("/subscriptions/:id/trigger", post(report_enhanced_handler::trigger_subscription))
        )
        // 导入导出路由
        .nest("/api/v1/erp/import", Router::new()
            .route("/csv", post(import_export_handler::import_csv))
            .route("/excel", post(import_export_handler::import_excel))
            .route("/templates/:import_type", get(import_export_handler::download_template))
        )
        .nest("/api/v1/erp/export", Router::new()
            .route("/csv/:export_type", get(import_export_handler::export_csv))
            .route("/excel/:export_type", get(import_export_handler::export_excel_type))
        )
        // 审计日志路由
        .nest("/api/v1/erp/audit", Router::new()
            .route("/logs", get(audit_enhanced_handler::list_audit_logs))
            .route("/logs/export", get(audit_enhanced_handler::export_audit_logs))
        )
        // 登录安全路由
        .nest("/api/v1/erp/security", Router::new()
        ...
        .nest("/api/v1/erp/webhooks/integrations", Router::new()
            .route("/", get(webhook_integration_handler::list_integrations).post(webhook_integration_handler::create_integration))
        ...
        .nest("/api/v1/erp/tenant/config", Router::new()
            .route("/settings", get(tenant_config_handler::list_configs).post(tenant_config_handler::set_config))
            .route("/settings/:key", delete(tenant_config_handler::delete_config))
            .route("/plans", get(tenant_config_handler::list_plans).post(tenant_config_handler::create_plan))
        ...
        .nest("/api/v1/erp/crm", Router::new()
            ...
            .route("/leads/:id", get(crate::handlers::crm_handler::get_lead).put(crate::handlers::crm_handler::update_lead).delete(crate::handlers::crm_handler::delete_lead))
            .route("/leads/:id/status", put(crate::handlers::crm_handler::update_lead_status))
            .route("/leads/:id/convert", post(crate::handlers::crm_handler::convert_lead))
