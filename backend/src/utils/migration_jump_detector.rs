...
mod tests {
    ...
    fn test_detect_migration_jumps_no_gap() {
        ...
        // 创建连续的迁移文件
        std::fs::write(dir.path().join("20260809000001_test.sql"), "").unwrap();
        std::fs::write(dir.path().join("20260809000002_test.sql"), "").unwrap();
        std::fs::write(dir.path().join("20260809000003_test.sql"), "").unwrap();
