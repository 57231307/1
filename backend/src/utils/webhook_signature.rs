...
pub fn verify_webhook_signature(
    ...
    let mut mac = HmacSha256::new_from_slice(secret.as_bytes())
        .map_err(|e| crate::utils::error::AppError::internal(
            format!("HMAC 初始化失败：{}", e)
        ))?;
    ...
    let sig_bytes = hex::decode(signature).map_err(|_| {
        crate::utils::error::AppError::unauthorized("无效的签名格式")
    })?;
    let computed_bytes = hex::decode(&computed).map_err(|_| {
        crate::utils::error::AppError::internal("签名计算异常")
    })?;
