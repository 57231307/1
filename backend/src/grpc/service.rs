...
use crate::services::{user_service::UserService as UserServiceImpl, auth_service::AuthService as AuthServiceImpl};
...
impl UserServiceTrait for GrpcUserService {
    async fn get_user(
        ...
        match self.user_service.find_by_id(req.user_id).await {
            ...
            Err(e) => {
                Err(Status::not_found(format!("用户不存在：{}", e)))
            }
        }
    }
    ...
    async fn create_user(
        ...
        // 创建用户
        match self.user_service.create_user(
            req.username,
            password_hash,
            Some(req.email),
            Some(req.phone),
            ...
            Err(e) => {
                Err(Status::internal(format!("创建用户失败：{}", e)))
            }
        }
    }
    ...
    async fn update_user(
        ...
        match self.user_service.find_by_id(user_id).await {
            Ok(_existing_user) => {
                ...
                // 更新用户信息
                match self.user_service.update_user(
                    user_id,
                    None,
                    Some(req.phone),
                    role_id,
                    ...
                    Err(e) => {
                        Err(Status::internal(format!("更新用户失败：{}", e)))
                    }
                }
            }
            Err(_) => {
                Err(Status::not_found("用户不存在"))
            }
        }
    }
    ...
    async fn delete_user(
        ...
        match self.user_service.delete_user(req.user_id).await {
            ...
            Err(e) => {
                Err(Status::internal(format!("删除用户失败：{}", e)))
            }
        }
    }
    ...
    async fn list_users(
        ...
        match self.user_service.list_users(page, page_size).await {
            ...
            Err(e) => {
                Err(Status::internal(format!("获取用户列表失败：{}", e)))
            }
        }
    }
}
...
impl AuthServiceTrait for GrpcUserService {
    async fn login(
        ...
        match self.auth_service.authenticate(&req.username, &req.password).await {
            Ok((token, user)) => {
            ...
            Err(e) => {
                Err(Status::unauthenticated(format!("登录失败：{}", e)))
            }
        }
    }
