...
impl AssistAccountingServiceTrait for GrpcNewServices {
    ...
    async fn create_assist_record(
        ...
        // 解析金额
        let debit_amount = req.debit_amount.parse::<rust_decimal::Decimal>()
            .map_err(|e| Status::invalid_argument(format!("借方金额格式错误：{}", e)))?;
        let credit_amount = req.credit_amount.parse::<rust_decimal::Decimal>()
            .map_err(|e| Status::invalid_argument(format!("贷方金额格式错误：{}", e)))?;
        let quantity_meters = req.quantity_meters.parse::<rust_decimal::Decimal>()
            .map_err(|e| Status::invalid_argument(format!("米数格式错误：{}", e)))?;
        let quantity_kg = req.quantity_kg.parse::<rust_decimal::Decimal>()
        ...
        match self.assist_accounting_service.create_assist_record(
            ...
            req.batch_no,
            req.color_no,
            if req.dye_lot_no.is_empty() { None } else { Some(req.dye_lot_no) },
            req.grade,
            req.warehouse_id,
            quantity_meters,
            quantity_kg,
            if req.workshop_id == 0 { None } else { Some(req.workshop_id) },
            if req.customer_id == 0 { None } else { Some(req.customer_id) },
            if req.supplier_id == 0 { None } else { Some(req.supplier_id) },
            if req.remarks.is_empty() { None } else { Some(req.remarks) },
            if req.created_by == 0 { None } else { Some(req.created_by) },
        ).await {
    ...
    async fn query_assist_records(
        ...
        let business_type = if req.business_type.is_empty() { None } else { Some(req.business_type.as_str()) };
        let warehouse_id = if req.warehouse_id == 0 { None } else { Some(req.warehouse_id) };
...
impl SupplierEvaluationServiceTrait for GrpcNewServices {
    ...
    async fn create_indicator(
        ...
        let weight = req.weight.parse::<rust_decimal::Decimal>()
        ...
        let create_req = crate::services::supplier_evaluation_service::CreateEvaluationIndicatorRequest {
            ...
            evaluation_method: if req.evaluation_method.is_empty() { None } else { Some(req.evaluation_method) },
        };
    ...
    async fn create_evaluation_record(
        ...
        let score = req.score.parse::<rust_decimal::Decimal>()
    ...
    async fn get_evaluation_records(
        ...
        let supplier_id = if req.supplier_id == 0 { None } else { Some(req.supplier_id) };
        let period = if req.period.is_empty() { None } else { Some(req.period) };
...
impl FiveDimensionQueryServiceTrait for GrpcNewServices {
    async fn generate_five_dimension_id(
        ...
        let dye_lot_no = if req.dye_lot_no.is_empty() { None } else { Some(req.dye_lot_no.as_str()) };
...
impl InventoryReservationServiceTrait for GrpcNewServices {
    async fn create_reservation(
        ...
        let created_by = if req.created_by == 0 { None } else { Some(req.created_by) };
    ...
    async fn batch_create_reservations(
        ...
        for item in req.items {
            let quantity = item.quantity.parse::<rust_decimal::Decimal>()
                .map_err(|e| Status::invalid_argument(format!("数量格式错误：{}", e)))?;
...
impl FinancialAnalysisServiceTrait for GrpcNewServices {
    async fn get_indicators_list(
        ...
        let params = crate::services::financial_analysis_service::IndicatorQueryParams {
            indicator_type: if req.indicator_type.is_empty() { None } else { Some(req.indicator_type) },
            status: if req.status.is_empty() { None } else { Some(req.status) },
            page: req.page as i64,
            page_size: req.page_size as i64,
        };
    ...
    async fn create_indicator(
        ...
        let create_req = crate::services::financial_analysis_service::CreateIndicatorRequest {
            ...
            formula: if req.formula.is_empty() { None } else { Some(req.formula) },
    ...
    async fn create_analysis_result(
        ...
        let indicator_value = req.indicator_value.parse::<rust_decimal::Decimal>()
            .map_err(|e| Status::invalid_argument(format!("指标值格式错误：{}", e)))?;
        let target_value = if req.target_value.is_empty() { 
            None 
        } else { 
            Some(req.target_value.parse::<rust_decimal::Decimal>()
