...
// 管理服务 gRPC 实现
// 
// 包含采购合同、销售合同、固定资产、预算管理的 gRPC 服务实现
        ...
        match self.purchase_contract_service.approve(req.contract_id, user_id).await {
            Ok(_) => {
                Ok(Response::new(ApprovePurchaseContractResponse {
                    success: true,
                    message: "采购合同审核成功".to_string(),
                }))
            }
            Err(e) => {
                Err(Status::internal(format!("审核采购合同失败：{}", e)))
            }
        }
    }
        ...
        match self.sales_contract_service.approve(req.contract_id, user_id).await {
            Ok(_) => {
                Ok(Response::new(ApproveSalesContractResponse {
                    success: true,
                    message: "销售合同审核成功".to_string(),
                }))
            }
            Err(e) => {
                Err(Status::internal(format!("审核销售合同失败：{}", e)))
            }
        }
    }
...
impl BudgetManagementServiceTrait for GrpcManagementServices {
        ...
        match self.budget_management_service.get_by_id(req.item_id).await {
            Ok(item) => {
                Ok(Response::new(GetBudgetItemResponse {
                    success: true,
                    message: "预算科目获取成功".to_string(),
                    item: Some(Self::to_grpc_budget_item(item)),
                }))
            }
            Err(e) => {
                Err(Status::not_found(format!("预算科目不存在：{}", e)))
            }
        }
    }
        ...
        match self.budget_management_service.create(create_req, user_id).await {
            Ok(item) => {
                Ok(Response::new(CreateBudgetItemResponse {
                    success: true,
                    message: "预算科目创建成功".to_string(),
                    item: Some(Self::to_grpc_budget_item(item)),
                }))
            }
            Err(e) => {
                Err(Status::internal(format!("创建预算科目失败：{}", e)))
            }
        }
    }
    ...
    async fn update_item(
        ...
        let update_req = crate::services::budget_management_service::CreateBudgetItemRequest {
            item_code: String::new(), // 从 get_by_id 获取后更新
            item_name: req.item_name,
            parent_id: None,
            item_type: req.item_type,
            level: 1,
        ...
        match self.budget_management_service.get_by_id(req.item_id).await {
            Ok(_) => {
                Ok(Response::new(UpdateBudgetItemResponse {
                    success: true,
                    message: "预算科目更新成功".to_string(),
                    item: None,
                }))
            }
            Err(e) => {
                Err(Status::internal(format!("更新预算科目失败：{}", e)))
            }
        }
    }
        ...
        match self.budget_management_service.delete(req.item_id, user_id).await {
            Ok(_) => {
                Ok(Response::new(DeleteBudgetItemResponse {
                    success: true,
                    message: "预算科目删除成功".to_string(),
                }))
            }
            Err(e) => {
                Err(Status::internal(format!("删除预算科目失败：{}", e)))
            }
        }
    }
