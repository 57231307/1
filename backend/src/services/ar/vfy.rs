...
mod tests {
    ...
    /// 测试_匹配状态常量_已匹配值正确（验证 ar::MATCH_MATCHED 常量值为 "MATCHED"（大写），；与 ar_reconciliation_item.match_status 字段语义一致。）
    #[test]
    fn 测试_匹配状态常量_已匹配值正确() {
    ...
    /// 测试_期末余额计算_零收款场景（验证当期无收款时，期末余额 = 期初 + 期内核销前发票额。）
    #[test]
    fn 测试_期末余额计算_零收款场景() {
    ...
    /// 测试_期末余额计算_全额核销场景（验证当收款总额等于期初+发票额时，期末余额归零（核销完成）。）
    #[test]
    fn 测试_期末余额计算_全额核销场景() {
        let opening = decs!("1000");
        let invoices = decs!("4000");
    ...
    /// 测试_日期匹配阈值_30天内可匹配（验证 vfy_ops/match.rs auto_match 策略2 中 date_diff <= 30 时应匹配。；边界值：恰好 30 天也应匹配。）
    #[test]
    ...
    /// 测试_日期匹配阈值_超30天不匹配（验证 vfy_ops/match.rs auto_match 策略2 中 date_diff > 30 时不应匹配。）
    #[test]
    ...
    /// 测试_部分匹配金额_取较小值（验证 vfy_ops/match.rs auto_match 策略2 中 matched = min(invoice_amount, collection_amount)。）
    #[test]
    fn 测试_部分匹配金额_取较小值() {
    ...
    /// 测试_账龄分桶_当期未逾期（验证 overdue_days <= 0 时落入第 0 桶（当期）。；边界值：overdue_days = 0（到期日当天）也应落入当期。）
    #[test]
    fn 测试_账龄分桶_当期未逾期() {
    ...
    /// 测试_账龄分桶_1到30天区间（验证 1 <= overdue_days <= 30 时落入第 1 桶（1-30天）。；边界值：1 和 30 均应落入此桶。）
    #[test]
    fn 测试_账龄分桶_1到30天区间() {
    ...
    /// 测试_账龄分桶_31到60天区间（验证 31 <= overdue_days <= 60 时落入第 2 桶（31-60天）。；边界值：31 和 60 均应落入此桶。）
    #[test]
    fn 测试_账龄分桶_31到60天区间() {
    ...
    /// 测试_账龄分桶_61到90天区间（验证 61 <= overdue_days <= 90 时落入第 3 桶（61-90天）。；边界值：61 和 90 均应落入此桶。）
    #[test]
    fn 测试_账龄分桶_61到90天区间() {
    ...
    /// 测试_账龄分桶_90天以上（验证 overdue_days > 90 时落入第 4 桶（90天以上）。；边界值：91 应落入此桶。）
    #[test]
    fn 测试_账龄分桶_90天以上() {
    ...
    /// 测试_客户确认状态机_其他状态可转换（验证 customer_confirm 中 status 为 "draft" 等非终态时应允许转换到 "confirmed"。）
    #[test]
    ...
    /// 测试_客户争议状态机_草稿可转换（验证 customer_dispute 中 status 为 "draft" 时应允许转换到 "disputed"。）
    #[test]
    fn 测试_客户争议状态机_草稿可转换() {
    ...
    /// 测试_匹配策略校验_合法策略通过（验证 auto_match 中三种合法策略（exact/date_order/all）均通过校验，；且 None 默认为 "all"，大小写不敏感（自动转小写）。）
    #[test]
    fn 测试_匹配策略校验_合法策略通过() {
    ...
    /// 测试_decs宏_核销金额解析（验证 decs! 宏可正确解析 vfy_ops 业务场景的金额字符串（含小数），；并可参与期末余额公式运算。）
    #[test]
    fn 测试_decs宏_核销金额解析() {
    ...
    /// 测试_ymd宏_对账日期解析（验证 ymd! 宏可正确解析 vfy_ops 业务场景的对账期间日期，；并可参与日期差运算（auto_match 策略2 依赖）。）
    #[test]
    fn 测试_ymd宏_对账日期解析() {
    ...
    /// 测试_服务实例化_需数据库（验证 ArReconciliationService::new 可用 SQLite 内存库构造实例，；仅验证实例化成功（不需要 schema），与模板 测试_服务实例创建 同模式。）
    #[tokio::test]
