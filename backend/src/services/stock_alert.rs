...
/// 预警类型
/// 批次 126 v8 复审 P2 修复：接入 inventory_stock_query.get_stock_alerts 业务，；compute_alert_type 函数根据库存数量/补货点/过期日期派生 AlertType。；v11 批次 144 P1-5 修复：OverStock / SlowMoving 接入 compute_alert_type 业务，；移除 dead_code 标注。
...
pub enum AlertType {
    ...
    /// 低于下限（可用量 < 补货点）
    LowStock,
    /// 高于上限（可用量 > max_stock_point，且设置了上限）（v11 批次 144 P1-5：接入 compute_alert_type 业务，移除 dead_code 标注。）
    OverStock,
...
/// 正常库存的 alert_type 字符串（批次 126 v8 复审 P2 修复：当库存状态正常（无告警）时返回此值。）
pub const ALERT_TYPE_NORMAL: &str = "normal";

/// 即将过期的阈值天数（默认 30 天）（批次 126 v8 复审 P2 修复：expiry_date 距当前时间 ≤ 此天数视为"即将过期"。；TODO(tech-debt): 后续可改为从配置读取，支持按产品类别差异化配置。）
