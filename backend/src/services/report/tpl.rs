...
impl ReportEngineService {
    ...
    pub async fn create_custom_template(
        ...
        let now = Utc::now();
        let template_id = format!("custom_{}", chrono::Utc::now().timestamp_millis());
        let active_model = report_template::ActiveModel {
            id: Default::default(),
            code: Set(template_id.clone()),
            name: Set(req.name.clone()),
            report_type: Set(req.category.clone()),
            ...
            data_source_sql: Set(Some(req.data_source.clone())),
        ...
        Ok(ReportTemplate {
            ...
            report_type: req.report_type.unwrap_or_default(),
            columns: req.columns,
            filters: req.filters,
    ...
    pub async fn get_all_templates(
        ...
        if let Some(ref cat) = category {
            query = query.filter(report_template::Column::Category.eq(cat.clone()));
        }

        let custom_templates = query.all(&*self.db).await?;

        for ct in custom_templates {
            // 反序列化 JSON（model 的 columns 字段是 Json = serde_json::Value）
