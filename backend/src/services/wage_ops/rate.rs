...
impl WageRateService {
    ...
    pub async fn create(&self, req: CreateWageRateRequest) -> Result<RateModel, AppError> {
        ...
        Self::validate_wage_type(&wage_type_value)?;

        let piece_price = req.piece_price.unwrap_or(Decimal::ZERO);
        let time_price = req.time_price.unwrap_or(Decimal::ZERO);
        Self::validate_prices_non_negative(piece_price, time_price)?;
        Self::validate_wage_type_prices(&wage_type_value, piece_price, time_price)?;

        let grade_a_ratio = req.grade_a_ratio.unwrap_or_else(|| Decimal::new(10, 1));
        let grade_b_ratio = req.grade_b_ratio.unwrap_or_else(|| Decimal::new(8, 1));
        let grade_c_ratio = req.grade_c_ratio.unwrap_or(Decimal::ZERO);
        Self::validate_grade_ratios(grade_a_ratio, grade_b_ratio, grade_c_ratio)?;
        Self::validate_expiry_date(req.effective_date, req.expiry_date)?;
    ...
    /// 校验工价类型合法
    fn validate_wage_type(wage_type_value: &str) -> Result<(), AppError> {
    ...
    /// 校验单价非负
    fn validate_prices_non_negative(
        piece_price: Decimal,
        time_price: Decimal,
    ) -> Result<(), AppError> {
    ...
    /// 校验等级系数范围 [0, 1]
    fn validate_grade_ratios(
        grade_a: Decimal,
        grade_b: Decimal,
        grade_c: Decimal,
    ) -> Result<(), AppError> {
        for (name, value) in [("A 级", grade_a), ("B 级", grade_b), ("C 级", grade_c)] {
    ...
    fn build_rate_active_model(
        ...
        RateActiveModel {
            ...
            route_code: Set(route_code.to_string()),
            route_name: Set(route_name.to_string()),
            wage_type: Set(wage_type_value.to_string()),
