...
impl InventoryTransferService {
    /// 检查调出仓库库存是否充足（在调拨单创建事务中调用，确保所有调拨明细在源仓库有足够库存。；采用批量查询优化 N+1：先一次性查出所有相关 product 的库存记录，；再用 HashMap 内存匹配。）
...
mod tests {
    ...
    /// 测试夹具：复现无库存记录错误消息（对应业务代码第 60-63 行的 format! 模板。）
    fn format_no_stock_error(product_id: i32) -> String {
        format!("产品 {} 在调出仓库没有库存记录", product_id)
    }

    /// 测试夹具：复现 product_id 过滤逻辑（对应业务代码第 29 行 `items.iter().filter_map(|item| item.product_id).collect()`。）
