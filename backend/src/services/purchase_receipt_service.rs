//! 采购入库 Service（负责采购入库的核心业务逻辑：入库单创建、确认、更新等全流程管理）。
//!
// 批次 101 v6 复审 P2 修复：calculate_receipt_total_txn 审计操作人 Some(0) 占位改为真实 user_id，三处内部调用方同步透传（P2-6）。
...
impl PurchaseReceiptService {
    ...
    /// 计算入库单总金额（事务版本，主表查询加 lock_exclusive 串行化并发重算，user_id 由调用方注入用于审计）。
