...
/// JWT JTI 黑名单（进程内降级回退表：jti -> 过期时间戳）（仅在 Redis 不可用时使用，避免阻塞业务。生产环境应配置 Redis。）
static JTI_BLACKLIST: LazyLock<RwLock<HashMap<String, i64>>> =
    LazyLock::new(|| RwLock::new(HashMap::new()));

/// 分布式 JTI 黑名单 Redis 客户端（懒初始化）（通过环境变量 `JTI_REDIS_URL` 或回退 `REDIS_URL` 启用。）
...
/// 吊销指定 JTI
/// 将给定 JTI 加入黑名单（优先写 Redis；Redis 不可用时回退到进程内 HashMap）。；后续请求将拒绝持有该 JTI 的 Token。；# 参数；`jti`: 待吊销的 Token 唯一标识（当前实现取自 `AppClaims::session_id`）；`expires_at`: Token 的过期时间戳（Unix 秒）
...
/// 检查 JTI 是否在黑名单（# 参数；`jti`: 待检查的 Token 唯一标识；# 返回；`true`: 该 JTI 已被吊销；`false`: 该 JTI 仍然有效）
...
/// 当使用 Redis 后端时，TTL 自动清理过期条目，此函数为 noop。；当回退到进程内存储时，主动清理已超过过期时间的记录，避免内存泄漏。；# 参数；`_max_age_secs`: 允许的最大存活时间（秒），当前实现忽略该参数
...
/// 吊销指定用户的所有活跃 JWT
/// 将 user_id 加入内存吊销表，记录当前时间戳为吊销点。；后续 middleware 收到该用户 Token 时，若 `iat < revoked_at` 则拒绝。；# 参数；`user_id`: 被吊销用户的 ID；`reason`: 吊销原因（如 `"USER_DELETED"`、`"USER_DEACTIVATED"`），仅用于日志；# 返回；`Ok(())`: 成功加入吊销表；`Err(AuthError::InternalError)`: 当前实现下不会失败，保留 Result 供后续扩展
...
/// 检查某用户 Token 是否已被吊销
/// 判定规则：若 user_id 不在吊销表中，返回 `false`（未吊销）；若 token_iat >= revoked_at，返回 `false`（Token 在吊销后签发，仍有效）；若 token_iat < revoked_at，返回 `true`（Token 在吊销前签发，必须拒绝）；# 参数；`user_id`: Token 所属用户 ID；`token_iat`: Token 签发时间戳（Unix 秒）
...
/// 清理过期的用户吊销记录（建议定期调用）
/// v11 批次 145 P1-7 修复：原实现为占位（仅打印日志），现实现真实 TTL 清理。；清理策略：吊销记录超过 `REVOKED_USER_TTL_SECS`（默认 7 天）后自动清理；理由：JWT 最长有效期 2 小时，7 天后所有旧 Token 已过期，吊销记录无意义；清理后该用户的旧 Token 已自然过期，无需再保留吊销标记；返回被清理的记录数。
...
/// v11 批次 145 P1-7 修复：接入 user_service.update_user 业务，；当用户状态从"禁用"恢复为"active"时调用此函数清除吊销标记，；允许用户重新登录获取新 Token。；# 参数；`user_id`: 需注销的用户 ID
