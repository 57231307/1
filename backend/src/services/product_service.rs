...
impl ProductService {
    ...
    pub async fn list_products(
        ...
        if let Some(keyword) = search {
            query = query.filter(
                product::Column::Name.like(format!("%{}%", keyword))
    ...
    pub async fn create_product(
        ...
        let active_model = product::ActiveModel {
            ...
            unit: Set(unit),
            standard_price: Set(standard_price.map(|p| Decimal::from_f64_retain(p).unwrap_or(Decimal::ZERO))),
            cost_price: Set(cost_price.map(|p| Decimal::from_f64_retain(p).unwrap_or(Decimal::ZERO))),
            ...
            gram_weight: Set(gram_weight.map(|g| Decimal::from_f64_retain(g).unwrap_or(Decimal::ZERO))),
            structure: Set(structure),
            finish: Set(finish),
            min_order_quantity: Set(min_order_quantity.map(|q| Decimal::from_f64_retain(q).unwrap_or(Decimal::ZERO))),
    ...
    pub async fn delete_product(&self, id: i32) -> Result<(), sea_orm::DbErr> {
        ...
        if result.rows_affected == 0 {
            return Err(sea_orm::DbErr::RecordNotFound(format!("产品 ID {} 不存在", id)));
        }
        Ok(())
    }
    ...
    pub async fn update_product(
        ...
        if let Some(moq) = min_order_quantity {
            product.min_order_quantity = Set(Some(Decimal::from_f64_retain(moq).unwrap_or(Decimal::ZERO)));
        }
    ...
    pub async fn batch_create_product_colors(
        ...
        for input in colors {
            let result = self.create_product_color(
                product_id,
                input.color_no,
                input.color_name,
                input.pantone_code,
    ...
    pub async fn delete_product_color(&self, id: i32) -> Result<(), sea_orm::DbErr> {
        ...
        if result.rows_affected == 0 {
            return Err(sea_orm::DbErr::RecordNotFound(format!("产品色号 ID {} 不存在", id)));
        }
        Ok(())
    }
    ...
    pub async fn find_color_by_product_and_color_no(
        ...
        ProductColorEntity::find()
            ...
            .one(&*self.db)
            .await?
            .ok_or_else(|| sea_orm::DbErr::RecordNotFound(format!("产品 {} 的色号 {} 不存在", product_id, color_no)))
    }
}
