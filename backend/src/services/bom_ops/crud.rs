...
impl BomService {
    ...
    /// 更新BOM
    /// 批次 203 P1-4 修复：原实现存在两个缺陷——事务仅包裹"删除旧明细 + 创建新明细"；（主表 update 在事务外，若明细插入失败，主表 update 不会回滚）；明细采用循环内；逐条 `insert(&txn)`，N 条明细 = N 次 INSERT（N+1 写）。现将事务范围扩大到包含；主表 update，明细改用 `insert_many` 单次 INSERT。
