...
impl RolePermissionService {
    ...
    pub async fn assign_permission(
        ...
        // 查询现有权限记录（决定走更新还是新建分支）
        let existing = self.find_existing_role_permission(&request).await?;
    ...
    /// 按 role/resource_type/action/resource_id 查询现有权限记录
    async fn find_existing_role_permission(
        &self,
        request: &AssignPermissionRequest,
    ...
    async fn update_existing_permission(
        ...
        // 写入权限变更审计日志（best-effort，失败不阻塞主流程）
    ...
    async fn create_new_permission(
        ...
        // 写入权限变更审计日志（新建权限，old_value=None）
        // 使用 perm_entity 的字段值（实际插入数据库的值）
        self.write_permission_audit(
            "role_permission_assign",
            user_id,
            perm_entity.role_id,
    ...
    /// 由实体构建权限详情响应（String 字段 clone 避免 move）
    fn build_permission_detail(entity: &role_permission::Model) -> RolePermissionDetail {
