...
impl SocialInsuranceService {
    ...
    /// 创建社保缴纳记录（自动计算五险一金）
    /// 业务规则（《社会保险法》第58条 + 《住房公积金管理条例》第14条）：用工之日起 30 日内办理社保登记；缴费基数应为上年度月平均工资，不得低于当地最低基数或高于最高基数；五险一金单位/个人部分按费率自动计算
    pub async fn create(
        &self,
    ...
    /// 计算五险一金扣缴金额（纯函数）
    /// 业务规则：养老保险：单位 16% + 个人 8%；医疗保险：单位 8% + 个人 2%；失业保险：单位 0.5% + 个人 0.5%；工伤保险：单位 0.4%（个人不缴）；生育保险：单位 1%（个人不缴）；公积金：单位 12% + 个人 12%
