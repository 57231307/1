...
impl ApInvoiceService {
    ...
    pub async fn auto_generate_from_receipt(
        ...
        let _supplier = crate::models::supplier::Entity::find_by_id(receipt.supplier_id)
            ...
            .await?
            .ok_or(AppError::ResourceNotFound(
                format!("供应商 {}", receipt.supplier_id),
            ))?;
    ...
    pub async fn update(
        ...
        if invoice.invoice_status != "DRAFT" {
            return Err(AppError::BusinessError(
                format!("应付单状态为{}，不可修改", invoice.invoice_status)
            ));
        }
    ...
    pub async fn delete(&self, id: i32) -> Result<(), AppError> {
        ...
        if invoice.invoice_status != "DRAFT" {
            return Err(AppError::BusinessError(
                format!("应付单状态为{}，不可删除", invoice.invoice_status)
            ));
        }
    ...
    pub async fn approve(&self, id: i32, user_id: i32) -> Result<ap_invoice::Model, AppError> {
        ...
        if invoice.invoice_status != "DRAFT" {
            return Err(AppError::BusinessError(
                format!("应付单状态为{}，不可审核", invoice.invoice_status)
            ));
        }
    ...
    pub async fn cancel(
        ...
        if !["AUDITED", "PARTIAL_PAID"].contains(&invoice.invoice_status.as_str()) {
            return Err(AppError::BusinessError(
                format!("应付单状态为{}，不可取消", invoice.invoice_status)
            ));
        }
    ...
    pub async fn get_aging_analysis(
        ...
        let mut aging_map: std::collections::BTreeMap<String, AgingAnalysisItem> = std::collections::BTreeMap::new();
        ...
        for invoice in invoices {
            ...
            let entry = aging_map.entry(aging_bucket.clone()).or_insert_with(|| AgingAnalysisItem {
                aging_bucket,
                invoice_count: 0,
