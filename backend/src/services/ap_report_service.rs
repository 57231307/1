...
impl ApReportService {
    ...
    pub async fn get_statistics_report(
        ...
        let mut status_map: std::collections::HashMap<String, StatusStatistics> = std::collections::HashMap::new();
        let mut type_map: std::collections::HashMap<String, TypeStatistics> = std::collections::HashMap::new();
        ...
        for invoice in invoices {
            ...
            // 按状态统计
            let status_entry = status_map.entry(invoice.invoice_status.clone()).or_insert_with(|| StatusStatistics {
            ...
            // 按类型统计
            let type_entry = type_map.entry(invoice.invoice_type.clone()).or_insert_with(|| TypeStatistics {
    ...
    pub async fn get_daily_report(
        ...
        // 查询当日新增应付单
        let new_invoices = query.clone()
            .filter(ap_invoice::Column::InvoiceDate.eq(report_date))
            .all(&*self.db)
            .await?;
        
        let new_invoice_count = new_invoices.len() as i64;
        let new_invoice_amount: Decimal = new_invoices.iter()
            .map(|inv| inv.amount)
            .sum();
        
        // 查询当日到期应付单
        let due_invoices = query.clone()
        ...
        let due_invoice_amount: Decimal = due_invoices.iter()
        ...
        let payment_amount: Decimal = payments.iter()
    ...
    pub async fn get_monthly_report(
        ...
        // 获取统计报表
        let statistics = self.get_statistics_report(supplier_id, start_date, end_date).await?;
