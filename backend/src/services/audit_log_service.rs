...
//!
//! 提供两类能力：`update_with_audit`（旧业务调用接口）与 `record(event: AuditEvent)`（P13 增强通用接口，异步落库不阻塞业务事务）。
//! 设计要点：列名兼容（旧 `old_value`/`new_value` 与新 `before_snapshot`/`after_snapshot` 双写），JSON 快照用 `audit_log::AuditValue` 包装存 JSONB 列。
...
impl AuditLogService {
    /// 作为 SeaORM 中间件自动拦截并生成 Update 审计日志（update 前后序列化快照，仅 i32 主键支持 before_snapshot 查询）。
