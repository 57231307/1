//! 凭证服务-工作流子模块（voucher_ops/workflow）
//!
//! 批次 488 D10-2a 拆分：从原 `voucher_service.rs` L523-717 迁移。
//! 包含 5 个工作流方法：
//! - submit / review / post（公开 API，状态机 draft→submitted→reviewed→posted）
//! - validate_voucher / validate_voucher_in_transaction（post 的私有校验拆分）
//!
//! 业务规则：
//! - 状态机：draft → submitted → reviewed → posted（不可逆）
...
//! - 所有状态更新通过 update_with_audit 记录审计日志
