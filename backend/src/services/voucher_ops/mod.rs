...
//!
//! 批次 488 D10-4 拆分：从原 `voucher_service.rs` L115-1291 迁移 impl VoucherService 方法。
//!
//! 模块层级关系：
//! - `voucher_ops` 与 `voucher_service` 同为 `crate::services` 下的兄弟模块
//! - `VoucherService` struct + `new` 定义在 `crate::services::voucher_service`（facade）
...
pub use types::{AssistRecordContext, BalanceUpdateContext};
