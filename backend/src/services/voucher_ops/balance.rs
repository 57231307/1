//! 凭证服务-过账与余额更新子模块（voucher_ops/balance）
//!
//! 批次 488 D10-4 拆分：从原 `voucher_service.rs` L679-1050 迁移。
//! 包含 14 个方法：
//! - validate_voucher / validate_voucher_in_transaction（pub(crate)，state.rs 调用）
//! - update_account_balances（pub(crate)，state.rs 调用）
...
use crate::services::voucher_ops::types::BalanceUpdateContext;

impl VoucherService {
    /// 验证凭证（借贷平衡）
    pub(crate) async fn validate_voucher(&self, id: i32) -> Result<(), AppError> {
    ...
    /// 验证凭证（事务内）
    pub(crate) async fn validate_voucher_in_transaction(
        &self,
        id: i32,
