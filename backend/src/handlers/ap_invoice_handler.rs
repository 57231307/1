...
pub async fn list_invoices(
    ...
    info!("用户 {} 查询应付单列表，供应商 ID: {:?}", auth.username, params.supplier_id);
...
pub async fn update_invoice(
    ...
    info!("用户 {} 更新应付单成功：{}", auth.username, invoice.invoice_no);
...
pub async fn delete_invoice(
    ...
    Ok(Json(ApiResponse::success_with_message((), "应付单删除成功")))
}
...
pub async fn approve_invoice(
    ...
    info!("用户 {} 审核应付单成功：{}", auth.username, invoice.invoice_no);
...
pub async fn cancel_invoice(
    ...
    info!("用户 {} 取消应付单 ID: {}, 原因：{}", auth.username, id, req.reason);
    
    let service = ApInvoiceService::new(db);
    let invoice = service.cancel(id, req.reason.clone(), auth.user_id).await?;
    
    info!("用户 {} 取消应付单成功：{}", auth.username, invoice.invoice_no);
...
pub async fn auto_generate(
    ...
    info!("用户 {} 自动生成应付单，入库单 ID: {}", auth.username, req.receipt_id);
    
    let service = ApInvoiceService::new(db);
    let invoice = service.auto_generate_from_receipt(req.receipt_id, auth.user_id).await?;
    
    info!("用户 {} 自动生成应付单成功：{}", auth.username, invoice.invoice_no);
...
pub async fn get_aging_analysis(
    ...
    info!("用户 {} 查询账龄分析，供应商 ID: {:?}", auth.username, params.supplier_id);
    ...
    Ok(Json(ApiResponse::success(serde_json::to_value(aging_data)?)))
}
...
pub async fn get_balance_summary(
    ...
    info!("用户 {} 查询应付余额表，供应商 ID: {:?}", auth.username, params.supplier_id);
