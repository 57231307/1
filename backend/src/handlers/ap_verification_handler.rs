...
use crate::services::ap_verification_service::{
    ApVerificationService, ManualVerifyRequest,
};
...
pub async fn list_verifications(
    ...
    info!("用户 {} 查询核销列表，供应商 ID: {:?}", auth.username, params.supplier_id);
    
    let service = ApVerificationService::new(db);
    let (verifications, total) = service.get_list(
        params.supplier_id,
...
pub async fn get_verification(
    ...
    info!("用户 {} 查询核销详情成功：{}", auth.username, verification.verification_no);
    
    Ok(Json(ApiResponse::success(serde_json::to_value(verification)?)))
}
...
pub async fn auto_verify(
    ...
    info!("用户 {} 执行自动核销，供应商 ID: {}", auth.username, req.supplier_id);
    
    let service = ApVerificationService::new(db);
    let verification = service.auto_verify(req.supplier_id, auth.user_id).await?;
    
    info!("用户 {} 自动核销成功：{}", auth.username, verification.verification_no);
...
pub async fn manual_verify(
    ...
    info!("用户 {} 执行手工核销，供应商 ID: {}", auth.username, req.supplier_id);
    ...
    info!("用户 {} 手工核销成功：{}", auth.username, verification.verification_no);
...
pub async fn cancel_verification(
    ...
    info!("用户 {} 取消核销 ID: {}, 原因：{}", auth.username, id, req.reason);
    
    let service = ApVerificationService::new(db);
    let verification = service.cancel(id, req.reason, auth.user_id).await?;
    
    info!("用户 {} 取消核销成功：{}", auth.username, verification.verification_no);
...
pub async fn get_unverified_invoices(
    ...
    info!("用户 {} 查询未核销应付单列表，供应商 ID: {}", auth.username, supplier_id);
    
    let service = ApVerificationService::new(db);
    let invoices = service.get_unverified_invoices(supplier_id).await?;
    
    info!("用户 {} 查询未核销应付单成功，共 {} 条", auth.username, invoices.len());
...
pub async fn get_unverified_payments(
    ...
    info!("用户 {} 查询未核销付款单列表，供应商 ID: {}", auth.username, supplier_id);
    
    let service = ApVerificationService::new(db);
    let payments = service.get_unverified_payments(supplier_id).await?;
    
    info!("用户 {} 查询未核销付款单成功，共 {} 条", auth.username, payments.len());
