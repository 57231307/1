...
pub async fn list_leads(
    ...
    let mut value = serde_json::to_value(res).map_err(|e| AppError::InternalError(format!("序列化失败: {}", e)))?;
...
pub async fn convert_lead(
    ...
    let service = CrmService::new(state.db.clone());
    let customer = service.convert_lead_to_customer(id, req, auth.user_id).await?;
    let value = serde_json::to_value(customer).map_err(|e| AppError::InternalError(format!("序列化失败: {}", e)))?;
    Ok(Json(ApiResponse::success(value)))
}
