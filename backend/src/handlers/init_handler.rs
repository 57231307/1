...
use crate::services::init_service::{InitService, InitRequest, InitStatus};
...
) -> Result<Json<crate::services::init_service::InitializationResult>, (StatusCode, Json<ErrorResponse>)> {
    let init_service = InitService::new(db);
    
    match init_service.initialize(&payload.admin_username, &payload.admin_password).await {
        Ok(result) => Ok(Json(result)),
        Err(e) => {
            let error_type = match e {
                crate::services::init_service::InitError::AlreadyInitialized =>
                    "already_initialized",
                crate::services::init_service::InitError::HashError =>
                    "hash_error",
                crate::services::init_service::InitError::DatabaseError(_) =>
                    "database_error",
            ...
            let message = match e {
                ...
                crate::services::init_service::InitError::HashError =>
                    "密码加密失败".to_string(),
                crate::services::init_service::InitError::DatabaseError(msg) =>
                    msg,
                crate::services::init_service::InitError::UserNotFound =>
                    "用户不存在".to_string(),
            };
    ...
    match init_service.reset_password(&payload.username, &payload.new_password).await {
        ...
        Err(e) => {
            let message = match e {
                ...
                crate::services::init_service::InitError::AlreadyInitialized => "系统已初始化".to_string(),
            };
