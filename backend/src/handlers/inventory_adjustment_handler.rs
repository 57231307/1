...
use crate::services::inventory_adjustment_service::{
    InventoryAdjustmentService, CreateAdjustmentRequest, AdjustmentItemRequest,
};
...
pub async fn create_adjustment(
    ...
    // 解析日期
    let adjustment_date: DateTime<Utc> = payload.adjustment_date
        .parse::<DateTime<Utc>>()
    ...
    let request = CreateAdjustmentRequest {
        ...
        items: payload.items.into_iter().map(|item| {
            AdjustmentItemRequest {
                stock_id: item.stock_id,
                quantity: item.quantity.parse::<Decimal>()
                    .map_err(|e| format!("数量格式错误：{}", e)).unwrap(),
    ...
    match service.create_adjustment(request).await {
        Ok(detail) => Ok(Json(AdjustmentResponse {
            ...
            items: detail.items.into_iter().map(|item| AdjustmentItemResponse {
                id: item.id,
                stock_id: item.stock_id,
...
pub async fn approve_adjustment(
    ...
    match service.approve_adjustment(id, 1).await { // TODO: 从 Token 中获取用户 ID
        Ok(_adjustment) => {
...
pub async fn list_adjustments(
    ...
    match service.list_adjustments(params.page.unwrap_or(0), params.page_size.unwrap_or(20)).await {
        Ok((adjustments, total)) => {
            Ok(Json(AdjustmentListResponse {
                adjustments: adjustments.into_iter().map(|a| AdjustmentSummary {
                    id: a.id,
