...
) -> Result<Json<ApiResponse<Vec<PurchaseContract>>>, AppError> {
    let contracts = PurchaseContractEntity::find()
...
pub async fn list_purchase_prices(
    State(state): State<AppState>,
) -> Result<Json<ApiResponse<Vec<PurchasePrice>>>, AppError> {
    let prices = PurchasePriceEntity::find()
...
pub async fn list_sales_returns(
    State(state): State<AppState>,
) -> Result<Json<ApiResponse<Vec<SalesReturn>>>, AppError> {
    let returns = SalesReturnEntity::find()
