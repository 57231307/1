...
) -> Result<Json<ApiResponse<Vec<SalesForecastResponse>>>, StatusCode> {
...
) -> Result<Json<ApiResponse<Vec<InventorySuggestionResponse>>>, StatusCode> {
    let service = AiAnalysisService::new(state.db);
...
) -> Result<Json<ApiResponse<Vec<AnomalyDetectionResponse>>>, StatusCode> {
    let service = AiAnalysisService::new(state.db);
