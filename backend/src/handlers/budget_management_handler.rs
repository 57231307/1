...
/// 预算科目导出表头（11 列）
fn budget_items_export_headers() -> Vec<String> {
    vec![
        "ID".to_string(),
        "预算编码".to_string(),
        "预算名称".to_string(),
...
/// 从 serde_json::Value 提取预算科目行数据
fn build_budget_item_row(i: &serde_json::Value) -> Vec<String> {
...
/// 构造预算科目列表 xlsx 表格
fn build_budget_items_table(items_json: &[serde_json::Value]) -> XlsxTable {
    XlsxTable {
        sheet_name: "预算科目".to_string(),
        headers: budget_items_export_headers(),
        rows: items_json.iter().map(build_budget_item_row).collect(),
    }
}

/// 异步记录预算科目导出操作（审计自身，best-effort 不阻塞响应）
fn record_budget_items_export_audit(
    state: &AppState,
    auth: &AuthContext,
    row_count: usize,
    filename: &str,
...
pub async fn export_budget_items(
    ...
    let table = build_budget_items_table(&items_json);
    let filename = format!(
        "budget_items_export_{}",
        chrono::Utc::now().format("%Y%m%d_%H%M%S")
    );
    record_budget_items_export_audit(&state, &auth, row_count, &filename);
...
pub async fn variance_analysis(
    ...
    let budget_year = req
        .get("budget_year")
        .and_then(|v| v.as_i64())
        .unwrap_or_else(|| chrono::Datelike::year(&chrono::Utc::now().date_naive()) as i64) as i32;
...
pub async fn create_budget_with_mode(
    ...
    let target_year = req
        .get("target_year")
        .and_then(|v| v.as_i64())
        .unwrap_or_else(|| chrono::Datelike::year(&chrono::Utc::now().date_naive()) as i64 + 1) as i32;
    let department_id = req
        .get("department_id")
        .and_then(|v| v.as_i64())
        .unwrap_or(1) as i32;
