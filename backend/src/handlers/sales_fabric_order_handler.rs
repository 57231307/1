...
use sea_orm::{DatabaseConnection, TransactionTrait, QueryOrder, Order, EntityTrait, ColumnTrait, QueryFilter, PaginatorTrait, Set, ActiveModelTrait};
use std::sync::Arc;
...
pub async fn list_fabric_orders(
    ...
    if let Some(no) = query.order_no {
        query_builder = query_builder.filter(sales_order::Column::OrderNo.like(format!("%{}%", no)));
    }
    ...
    let orders = paginator.fetch_page(page - 1).await.map_err(|e| AppError::DatabaseError(e.to_string()))?;
    ...
    Ok(Json(PaginatedResponse::new(orders_json, total, page, page_size).into()))
}
...
pub async fn create_fabric_order(
    ...
    // 开启事务
    let txn = db.begin().await.map_err(|e| AppError::InternalError(format!("开启事务失败：{}", e)))?;
    ...
    let created_order = order.insert(&txn).await.map_err(|e| AppError::BadRequest(format!("创建订单失败：{}", e)))?;

    // 创建订单明细
    for item in &req.items {
        let quantity_meters = Decimal::from_f64_retain(item.quantity_meters).unwrap_or_default();
        let quantity_kg = Decimal::from_f64_retain(item.quantity_kg).unwrap_or_default();
        let base_price_val = Decimal::from_f64_retain(item.base_price.unwrap_or(item.unit_price_meters)).unwrap_or_default();
        let color_extra = Decimal::from_f64_retain(item.color_extra_cost.unwrap_or(0.0)).unwrap_or_default();
        let grade_diff = Decimal::from_f64_retain(item.grade_price_diff.unwrap_or(0.0)).unwrap_or_default();
        let final_p = Decimal::from_f64_retain(item.final_price.unwrap_or(item.unit_price_meters)).unwrap_or_default();
        ...
        let order_item = sales_order_item::ActiveModel {
            ...
            gram_weight: Set(Some(Decimal::from_f64_retain(item.gram_weight.unwrap_or(0.0)).unwrap_or_default())),
            width: Set(Some(Decimal::from_f64_retain(item.width.unwrap_or(0.0)).unwrap_or_default())),
        ...
        order_item.insert(&txn).await.map_err(|e| AppError::BadRequest(format!("创建订单明细失败：{}", e)))?;
    }

    // 提交事务
    txn.commit().await.map_err(|e| AppError::InternalError(format!("提交事务失败：{}", e)))?;

    let order_json = serde_json::to_value(created_order).unwrap_or_default();
    Ok(Json(ApiResponse::success_with_msg(order_json, "订单创建成功")))
}
...
pub async fn update_fabric_order(
    ...
    let updated = order.update(&*db).await.map_err(|e| AppError::BadRequest(format!("更新订单失败：{}", e)))?;
    let order_json = serde_json::to_value(updated).unwrap_or_default();
    Ok(Json(ApiResponse::success_with_msg(order_json, "订单更新成功")))
}
...
pub async fn approve_fabric_order(
    ...
    let updated = order.update(&*db).await.map_err(|e| AppError::BadRequest(format!("审核订单失败：{}", e)))?;
    let order_json = serde_json::to_value(updated).unwrap_or_default();
    Ok(Json(ApiResponse::success_with_msg(order_json, "订单审核成功")))
}