...
test.describe('02 运单状态流转', () => {
  ...
  test('02-01 待发货运单可发货（pending → shipped）', async ({ page }) => {
    ...
    if (await shipBtn.isVisible({ timeout: 3000 }).catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; })) {
      await shipBtn.click();
      await page.getByRole('button', { name: /确定/ }).click();
      await expect(page.getByText(/发货成功/)).toBeVisible({ timeout: 30000 }).catch((e) => {
  ...
  test('02-02 已发货运单可更新状态至运输中（shipped → in_transit）', async ({ page }) => {
    ...
    if (await updateBtn.isVisible({ timeout: 3000 }).catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; })) {
  ...
  test('02-03 已签收运单无操作按钮', async ({ page }) => {
    ...
    if (await deliveredTag.isVisible({ timeout: 3000 }).catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; })) {
