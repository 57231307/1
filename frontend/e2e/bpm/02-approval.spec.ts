...
test.describe('02 审批中心', () => {
  ...
  test('02-03 待办任务可审批拒绝', async ({ page }) => {
    ...
    if (await rejectBtn.isVisible({ timeout: 3000 }).catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; })) {
  ...
  test('02-04 已办任务可追溯审批链', async ({ page }) => {
    ...
    if (await chainBtn.isVisible({ timeout: 3000 }).catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; })) {
