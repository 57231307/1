...
test.describe('生产计划 - 02 生产执行', () => {
  ...
  test('已计划工单可开始生产（planned → in_production）', async ({ page }) => {
    ...
    if (
      await startBtn.isVisible({ timeout: 3000 }).catch(e => {
      ...
      await expect(page.getByText(/开始生产成功|状态更新成功/))
  ...
  test('生产中工单可完成（in_production → completed）', async ({ page }) => {
    ...
    if (
      await completeBtn.isVisible({ timeout: 3000 }).catch(e => {
        console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`);
        return false;
      })
    ) {
      ...
      await page.getByRole('button', { name: /确定/ }).click();
      await expect(page.getByText(/完成成功|状态更新成功/))
        .toBeVisible({ timeout: 30000 })
        .catch(e => {
          console.warn(`[E2E] 断言容错: ${(e as Error).message}`);
