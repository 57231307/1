...
test.describe.serial('引导页初始化真实链路（真实后端 + 真实 PostgreSQL，零 mock）', () => {
  ...
  test('数据库连接真实测试：填真实凭据 → test-database 200 → 下一步解禁', async ({ page }) => {
    ...
    await page.locator('input[placeholder="bingxi"]').first().fill(ctx.db);
  ...
  test('UI 真实登录进入主应用：向导创建的管理员可登录并看到 Dashboard', async ({ page }) => {
    ...
    if (!isChecked) {
      ...
      let nowChecked = await checkboxInput.isChecked();
      if (!nowChecked) {
        // fallback: 点击 label 区域
        await page.locator('.el-checkbox').first().click();
        await page.waitForTimeout(300);
        nowChecked = await checkboxInput.isChecked();
      }
      if (!nowChecked) {
