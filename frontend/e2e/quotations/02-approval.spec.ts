...
test.describe('02 报价单审批与转订单', () => {
  ...
  test('02-03 已批准报价单可转为销售订单', async ({ page }) => {
    ...
    if (
      await convertBtn.isVisible({ timeout: 3000 }).catch(e => {
        console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`);
        return false;
      })
    ) {
      ...
      await page.getByRole('button', { name: /确定/ }).click();
      await expect(page.getByText(/转订单成功/))
        .toBeVisible({ timeout: 30000 })
        .catch(e => {
          console.warn(`[E2E] 断言容错: ${(e as Error).message}`);
  ...
  test('02-04 草稿报价单可取消', async ({ page }) => {
    ...
    if (
      await cancelBtn.isVisible({ timeout: 3000 }).catch(e => {
        console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`);
        return false;
      })
    ) {
      ...
      await page.getByRole('button', { name: /确定/ }).click();
      await expect(page.getByText(/取消成功/))
        .toBeVisible({ timeout: 30000 })
        .catch(e => {
          console.warn(`[E2E] 断言容错: ${(e as Error).message}`);
