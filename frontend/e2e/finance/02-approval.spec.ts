...
test.describe('02 凭证审批工作流', () => {
  ...
  test('02-02 已提交凭证可审核（submitted → reviewed）', async ({ page }) => {
    ...
    if (await reviewBtn.isVisible({ timeout: 3000 }).catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; })) {
      await reviewBtn.click();
      await page.getByRole('button', { name: /确定/ }).click();
      await expect(page.getByText(/审核成功/)).toBeVisible({ timeout: 30000 }).catch((e) => {
      console.warn(`[E2E] 断言容错: ${(e as Error).message}`);
  ...
  test('02-03 已审核凭证可过账（reviewed → posted）', async ({ page }) => {
    ...
    if (await postBtn.isVisible({ timeout: 3000 }).catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; })) {
      await postBtn.click();
      await page.getByRole('button', { name: /确定/ }).click();
      await expect(page.getByText(/过账成功/)).toBeVisible({ timeout: 30000 }).catch((e) => {
      console.warn(`[E2E] 断言容错: ${(e as Error).message}`);
  ...
  test('02-04 已过账凭证无提交/审核/过账按钮', async ({ page }) => {
    ...
    if (await postedTag.isVisible({ timeout: 3000 }).catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; })) {
