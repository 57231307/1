...
test.describe('P5.12 遍历：purchase/production', () => {
  const mods = TRAVERSAL_MODULES.filter(m => ['purchase', 'production'].includes(m.domain));
...
test.describe('P5.12 遍历：quality/bpm/advanced', () => {
  const mods = TRAVERSAL_MODULES.filter(m => ['quality', 'bpm', 'advanced'].includes(m.domain));
  for (const mod of mods) {
    test(`${mod.id} (${mod.tier})`, async ({ page }) => {
      await visitModule(page, mod);
    });
