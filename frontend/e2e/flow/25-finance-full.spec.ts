...
test.describe('财务模块全量：API 端点 + 真实 UI 交互', () => {
  ...
  test('资金管理：账户+存取+冻结+转账+审批+报表+预测', async ({ page }) => {
    ...
    const list = await apiCallRaw<{ items: Array<{ id: number }> }>(page, 'GET', '/fund-management/accounts?page=1&page_size=1').catch(() => ({ items: [] as Array<{ id: number }> }));
  ...
  test('会计期间 UI：新建期间+初始化年度', async ({ page }) => {
    ...
    const initVisible = await initBtn.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test('财务报表 UI：生成+导出', async ({ page }) => {
    ...
    await page.waitForTimeout(3000);
    await page.locator('.el-card, .el-table, .el-form').first().waitFor({ state: 'visible', timeout: 30_000 });
    // 验证生成报表按钮
