...
test.describe('色卡+色卡价格：API 端点 + 真实 UI 交互', () => {
  ...
  test('色卡借出→归还→丢失→损坏状态机', async ({ page }) => {
    const issues = await apiCallRaw<{ items: Array<{ id: number }> }>(page, 'GET', '/color-cards/issues?page=1&page_size=1').catch(() => ({ items: [] as Array<{ id: number }> }));
  ...
  test('色卡价格：CRUD+批量调价+审批+历史+阶梯+季节规则', async ({ page }) => {
    ...
    const list = await apiCallRaw<{ items: Array<{ id: number }> }>(page, 'GET', '/color-prices/?page=1&page_size=1').catch(() => ({ items: [] as Array<{ id: number }> }));
  ...
  test('色卡列表 UI：搜索+表格+新建跳转', async ({ page }) => {
    ...
    // 验证搜索框
    const searchInput = page.locator('input[placeholder*="卡号"], input[placeholder*="卡名"]').first();
  ...
  test('色卡价格批量调价 UI 页面', async ({ page }) => {
    ...
    await page.waitForTimeout(3000);
    const visible = await page.locator('.el-card, .el-form, .el-table, .el-empty, body').first().isVisible().catch(() => false);
