...
test.describe('批量操作与弹窗确认', () => {
  ...
  test('采购订单批量选择+审批', async ({ page }) => {
    ...
    if (checkboxCount > 1) {
      ...
      await batchApproveBtn
        ...
        .catch(e => console.error('[E2E] 操作失败:', (e as Error).message));
      const batchBtnVisible = await batchApproveBtn.isVisible().catch(e => {
      ...
      if (batchBtnVisible) {
        ...
        await confirmDialog
          ...
          .catch(e => console.error('[E2E] 操作失败:', (e as Error).message));
        const dialogVisible = await confirmDialog.isVisible().catch(e => {
          console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`);
          return false;
        });
        if (dialogVisible) {
  ...
  test('删除确认弹窗取消交互', async ({ page }) => {
    ...
    await deleteBtn
      .waitFor({ state: 'visible', timeout: 5000 })
      .catch(e => console.error('[E2E] 操作失败:', (e as Error).message));
    const deleteVisible = await deleteBtn.isVisible().catch(e => {
      console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`);
      return false;
    });
    if (deleteVisible) {
      ...
      await popconfirm
        .waitFor({ state: 'visible', timeout: 5000 })
        .catch(e => console.error('[E2E] 操作失败:', (e as Error).message));
      const popVisible = await popconfirm.isVisible().catch(e => {
        console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`);
        return false;
      });
      if (popVisible) {
        ...
        await cancelBtn
          .waitFor({ state: 'visible', timeout: 3000 })
          .catch(e => console.error('[E2E] 操作失败:', (e as Error).message));
        const cancelVisible = await cancelBtn.isVisible().catch(e => {
          console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`);
          return false;
        });
        if (cancelVisible) {
          ...
          const popStillVisible = await popconfirm.isVisible().catch(e => {
            console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`);
            return false;
          });
  ...
  test('导出功能交互', async ({ page }) => {
    ...
    if (exportVisible) {
      const downloadPromise = page.waitForEvent('download', { timeout: 5000 }).catch(e => {
  ...
  test('下拉级联选择交互', async ({ page }) => {
    ...
    await supplierSelect
      .waitFor({ state: 'visible', timeout: 5000 })
      .catch(e => console.error('[E2E] 操作失败:', (e as Error).message));
    const selectVisible = await supplierSelect.isVisible().catch(e => {
      console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`);
      return false;
    });
    if (selectVisible) {
