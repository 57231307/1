...
test.describe('生产模块全量：API 端点 + 真实 UI 交互', () => {
  ...
  test('MRP 计算 UI：表单+计算按钮', async ({ page }) => {
    ...
    await calcBtn
      .waitFor({ state: 'visible', timeout: 5000 })
      .catch(e => console.error('[E2E] 操作失败:', (e as Error).message));
    const btnVisible = await calcBtn.isVisible().catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; });
    if (btnVisible) {
  ...
  test('产能分析 UI：筛选+图表', async ({ page }) => {
    ...
    await datePicker
      .waitFor({ state: 'visible', timeout: 5000 })
      .catch(e => console.error('[E2E] 操作失败:', (e as Error).message));
    const dateVisible = await datePicker.isVisible().catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; });
    // 验证工作中心选择
    const select = page.locator('.el-select').first();
    await select
      .waitFor({ state: 'visible', timeout: 5000 })
      .catch(e => console.error('[E2E] 操作失败:', (e as Error).message));
    const selectVisible = await select.isVisible().catch((e) => { console.warn(`[E2E] 元素状态查询失败: ${(e as Error).message}`); return false; });
