...
test.describe('生产模块全量：API 端点 + 真实 UI 交互', () => {
  ...
  test('流转卡：CRUD+状态机+步骤+反馈+工艺路线', async ({ page }) => {
    ...
    const list = await apiCallRaw<{ items: Array<{ id: number }> }>(page, 'GET', '/production/flow-cards?page=1&page_size=1').catch(() => ({ items: [] as Array<{ id: number }> }));
  ...
  test('验布打卷：CRUD+状态机+疵点+物理测试', async ({ page }) => {
    ...
    const list = await apiCallRaw<{ items: Array<{ id: number }> }>(page, 'GET', '/production/fabric-inspections?page=1&page_size=1').catch(() => ({ items: [] as Array<{ id: number }> }));
    ...
    await safePostAction(page, '/production/fabric-inspections/physical-tests', { inspection_id: 1, test_type: 'tensile', result: 'pass' });
  });
  ...
  test('产量工资：工价+工票+计算+确认+支付', async ({ page }) => {
    ...
    const list = await apiCallRaw<{ items: Array<{ id: number }> }>(page, 'GET', '/production/wage-records?page=1&page_size=1').catch(() => ({ items: [] as Array<{ id: number }> }));
  ...
  test('能耗管理：仪表+消耗+规则+分配', async ({ page }) => {
    ...
    const list = await apiCallRaw<{ items: Array<{ id: number }> }>(page, 'GET', '/production/energy-consumptions?page=1&page_size=1').catch(() => ({ items: [] as Array<{ id: number }> }));
  ...
  test('产能分析+排程+质量标准+质量检验+BOM+打样+缺料+缸号状态机+委外', async ({ page }) => {
    ...
    const ctx = getCtx();
    if (ctx.dyeBatchId) {
      await verifyEndpointHealthy(page, `/production/dye-batch-lifecycle-logs/by-batch/${ctx.dyeBatchId}`);
      await verifyEndpointHealthy(page, `/production/dye-batch-operations/by-batch/${ctx.dyeBatchId}`);
    }
  ...
  test('BOM 列表 UI：搜索+新建弹窗+表单', async ({ page }) => {
    ...
    // 搜索
    const searchInput = page.locator('input[placeholder*="产品名称"], input[placeholder*="产品"]').first();
  ...
  test('MRP 计算 UI：表单+计算按钮', async ({ page }) => {
    ...
    await page.waitForTimeout(3000);
    await page.locator('.el-card, .el-form, .el-table').first().waitFor({ state: 'visible', timeout: 30_000 });
    // 验证计算按钮存在
    const calcBtn = page.locator('button:has-text("开始计算")').first();
    const btnVisible = await calcBtn.isVisible({ timeout: 5000 }).catch(() => false);
    if (btnVisible) {
  ...
  test('产能分析 UI：筛选+图表', async ({ page }) => {
    ...
    await page.waitForTimeout(3000);
    await page.locator('.el-card, .el-table, .el-empty').first().waitFor({ state: 'visible', timeout: 30_000 });
    // 验证日期选择器存在
  ...
  test('染色配方 UI：搜索+新建弹窗+色号字段', async ({ page }) => {
    ...
    for (let i = 0; i < headerCount; i++) headerTexts.push((await headers.nth(i).textContent()) || '');
