...
test.describe('P5.11b 专用审批流', () => {
  ...
  test('坏账核销双级审批链', async ({ page }) => {
    ...
    const body = (await list.json().catch(e => {
      console.warn(`[E2E] 操作失败（降级跳过）: ${(e as Error).message}`);
      return null;
    })) as { data?: { items?: Array<{ id: number; status?: string }> } } | null;
    ...
    // 一级：finance-approve
    const l1 = await page.request
      .post(`${API_BASE}${API_PREFIX}/bad-debts/writeoffs/${pending.id}/finance-approve`, {
    ...
    const l2 = await page.request
      .post(`${API_BASE}${API_PREFIX}/bad-debts/writeoffs/${pending.id}/general-manager-approve`, {
  ...
  test('资金转账审批链', async ({ page }) => {
    ...
    const resp = await page.request
      .post(`${API_BASE}${API_PREFIX}/fund-management/transfers/${pending.id}/approve`, {
  ...
  test('角色变更审批链', async ({ page }) => {
    ...
    const resp = await page.request
      .post(`${API_BASE}${API_PREFIX}/role-change-approvals/${pending.id}/approve-l1`, {
...
test.describe('P5.11c BPM 引擎全流程', () => {
  ...
  test('流程实例审批链查询', async ({ page }) => {
    ...
    const body = (await list.json().catch(e => {
      console.warn(`[E2E] 操作失败（降级跳过）: ${(e as Error).message}`);
      return null;
    })) as { data?: { items?: Array<{ id: number }> } } | null;
    const items = body?.data?.items ?? [];
    if (items.length === 0) {
      console.warn('[E2E] test.skip: 前置数据缺失/条件不满足');
      test.skip();
