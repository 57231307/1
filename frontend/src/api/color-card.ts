...
export function updateColorCard(
  id: number,
  dto: Partial<{
    card_name: string;
    card_type: string;
...
export function deleteColorItem(cardId: number, itemId: number) {
  return request.delete<{ data: null }>(`/color-cards/${cardId}/items/${itemId}`);
}
...
export function batchImportItems(cardId: number, items: Partial<ColorItemInfo>[]) {
  ...
  }>(`/color-cards/${cardId}/items/batch`, { items });
}
...
export function returnIssue(
  recordId: number,
  dto: { actual_return_date?: string; remark?: string }
) {
  return request.post<{ data: IssueRecordInfo }>(`/color-cards/issues/${recordId}/return`, dto);
}

// 登记遗失（与后端 MarkLostDto 对齐）
export function markIssueLost(
  recordId: number,
  dto: { compensation_amount: number; remark?: string }
) {
  return request.post<{ data: IssueRecordInfo }>(`/color-cards/issues/${recordId}/lost`, dto);
}

// 标记损坏（与后端 MarkDamagedDto 对齐）
export function markIssueDamaged(
  recordId: number,
  dto: { compensation_amount?: number; remark?: string }
) {
  return request.post<{ data: IssueRecordInfo }>(`/color-cards/issues/${recordId}/damaged`, dto);
}

// 取消发放（与后端 CancelIssueDto 对齐）
export function cancelIssue(recordId: number, dto: { remark?: string }) {
  return request.post<{ data: IssueRecordInfo }>(`/color-cards/issues/${recordId}/cancel`, dto);
}
...
export function getIssueList(params: {
  ...
  return request.get<{ data: PagedResponse<IssueRecordInfo> }>('/color-cards/issues', { params });
}
