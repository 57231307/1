...
<template>
  <div class="process-flow">
    <el-empty
      ...
      :description="t('common.processFlow.emptyText')"
    />
    ...
    <div v-if="nodes && nodes.length > 0" class="node-actions">
      <el-card v-for="node in nodes" :key="node.id" class="node-card" shadow="hover">
        ...
        <div class="node-info">
          <div>
            {{ t('common.processFlow.plannedStart')
            }}{{ node.planned_start_date || t('common.processFlow.notSet') }}
          </div>
          <div>
            {{ t('common.processFlow.actualStart')
            }}{{ node.actual_start_date || t('common.processFlow.notStarted') }}
          </div>
          <div>
            {{ t('common.processFlow.actualEnd')
            }}{{ node.actual_end_date || t('common.processFlow.inProgress') }}
          </div>
          <div>
            {{ t('common.processFlow.operator')
            }}{{ node.operator_id || t('common.processFlow.unassigned') }}
          </div>
        </div>
...
function getDescription(node: ProcessNode) {
  if (node.actual_end_date)
    return t('common.processFlow.descriptionCompleted', {
      date: new Date(node.actual_end_date).toLocaleDateString(),
    });
  if (node.actual_start_date)
    return t('common.processFlow.descriptionStarted', {
      date: new Date(node.actual_start_date).toLocaleDateString(),
    });
  if (node.planned_start_date)
    return t('common.processFlow.descriptionPlanned', {
      date: new Date(node.planned_start_date).toLocaleDateString(),
    });
  ...
  try {
    if (action === 'block') {
      const { value: reason } = await ElMessageBox.prompt(
        t('common.processFlow.blockReasonPrompt'),
        t('common.processFlow.blockNodeTitle'),
        {
          inputPattern: /\S+/,
