...
<template>
  <div class="quality-check">
    ...
    <el-table :data="issues" border stripe :empty-text="t('common.qualityCheck.emptyText')" :aria-label="t('common.qualityCheck.tableAriaLabel')">
      <el-table-column prop="issue_type" :label="t('common.qualityCheck.colIssueType')" width="140" />
      ...
      <el-table-column prop="description" :label="t('common.qualityCheck.colDescription')" min-width="200" show-overflow-tooltip />
      ...
      <el-table-column prop="resolution" :label="t('common.qualityCheck.colResolution')" min-width="200" show-overflow-tooltip />
    ...
    <!-- 上报异常对话框 -->
    <el-dialog v-model="reportVisible" :title="t('common.qualityCheck.reportDialogTitle')" :aria-label="t('common.qualityCheck.reportDialogAriaLabel')" width="540px">
      <el-form :model="reportForm" :rules="reportRules" ref="reportFormRef" label-width="100px" :aria-label="t('common.qualityCheck.reportFormAriaLabel')">
        <el-form-item :label="t('common.qualityCheck.formIssueType')" prop="issue_type">
          <el-select v-model="reportForm.issue_type" :placeholder="t('common.qualityCheck.formIssueTypePlaceholder')">
            <el-option :label="t('common.qualityCheck.issueType.colorDiff')" value="color_diff" />
            <el-option :label="t('common.qualityCheck.issueType.colorFastness')" value="color_fastness" />
        ...
        <el-form-item :label="t('common.qualityCheck.formSeverity')" prop="severity">
          <el-radio-group v-model="reportForm.severity">
            <el-radio-button label="low">{{ t('common.qualityCheck.severity.low') }}</el-radio-button>
            <el-radio-button label="medium">{{ t('common.qualityCheck.severity.medium') }}</el-radio-button>
            <el-radio-button label="high">{{ t('common.qualityCheck.severity.high') }}</el-radio-button>
            <el-radio-button label="critical">{{ t('common.qualityCheck.severity.critical') }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="reportForm.issue_type === 'color_diff'" :label="t('common.qualityCheck.colorDeltaE')">
        ...
        <el-form-item v-if="reportForm.issue_type === 'color_fastness'" :label="t('common.qualityCheck.colorFastnessGrade')">
          <el-select v-model="reportForm.color_fastness_grade" :placeholder="t('common.qualityCheck.colorFastnessGradePlaceholder')">
            <el-option :label="t('common.qualityCheck.grade', { n: i })" :value="i" v-for="i in [1, 2, 3, 4, 5]" :key="i" />
          </el-select>
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="submitting" @click="handleReportSubmit">{{ t('common.qualityCheck.submit') }}</el-button>
      </template>
    </el-dialog>

    <!-- 解决异常对话框 -->
    <el-dialog v-model="resolveVisible" :title="t('common.qualityCheck.resolveDialogTitle')" width="500px" :aria-label="t('common.qualityCheck.resolveDialogAriaLabel')">
      <el-form :model="resolveForm" label-width="80px" :aria-label="t('common.qualityCheck.resolveFormAriaLabel')">
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="submitting" @click="handleResolveSubmit">{{ t('common.qualityCheck.confirm') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const reportRules = computed(() => ({
  issue_type: [{ required: true, message: t('common.qualityCheck.rule.issueTypeRequired'), trigger: 'change' }],
  severity: [{ required: true, message: t('common.qualityCheck.rule.severityRequired'), trigger: 'change' }],
  description: [{ required: true, message: t('common.qualityCheck.rule.descriptionRequired'), trigger: 'blur' }],
}))
