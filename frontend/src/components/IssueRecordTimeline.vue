<template>
  <div class="issue-record-timeline">
    <el-empty
      ...
      :description="t('components.issueRecordTimeline.empty')"
    />
      ...
      >
        <el-card shadow="hover">
          <div class="record-header">
            <div>
              <strong>{{
                t('components.issueRecordTimeline.colorCard', { id: record.color_card_id })
              }}</strong>
            ...
            <div class="record-id">
              {{ t('components.issueRecordTimeline.recordId', { id: record.id }) }}
            </div>
          </div>
          <div class="record-body">
            <div class="row">
              ...
              <span class="label" style="margin-left: 24px">{{
                t('components.issueRecordTimeline.issueQty')
              }}</span>
              <span>{{ record.issue_qty }}</span>
              <span class="label" style="margin-left: 24px">{{
                t('components.issueRecordTimeline.operator')
              }}</span>
            ...
            <div v-if="record.compensation_amount" class="row">
              <span class="label">{{
                t('components.issueRecordTimeline.compensationAmount')
              }}</span>
              <span style="color: #f56c6c; font-weight: bold"
                >¥{{ record.compensation_amount }}</span
              >
            </div>
...
/** 发放状态对应的 el-tag 类型 */
const tagType = (status: string): TagType => (ISSUE_STATUS_COLORS[status] || '') as TagType;
