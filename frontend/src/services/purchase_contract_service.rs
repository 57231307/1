...
impl PurchaseContractService {
    ...
    pub async fn list_contracts(params: PurchaseContractQueryParams) -> Result<PurchaseContractListResponse, String> {
        ...
        let response: PurchaseContractListResponse = ApiService::get(&url).await?;
        Ok(response)
    }
    ...
    pub async fn get_contract(id: i32) -> Result<PurchaseContract, String> {
        let response: PurchaseContract = ApiService::get(&format!("/purchase-contracts/{}", id)).await?;
        Ok(response)
    }

    /// 创建采购合同
    pub async fn create_contract(req: CreatePurchaseContractRequest) -> Result<PurchaseContract, String> {
        let response: PurchaseContract = ApiService::post("/purchase-contracts", &req).await?;
        Ok(response)
    }

    /// 审核采购合同
    pub async fn approve_contract(id: i32) -> Result<PurchaseContract, String> {
        let response: PurchaseContract = ApiService::put(&format!("/purchase-contracts/{}/approve", id), &serde_json::json!({})).await?;
        Ok(response)
    }

    /// 执行采购合同
    pub async fn execute_contract(id: i32, req: ExecutePurchaseContractRequest) -> Result<PurchaseContract, String> {
        let response: PurchaseContract = ApiService::put(&format!("/purchase-contracts/{}/execute", id), &req).await?;
        Ok(response)
    }
    ...
    pub async fn cancel_contract(id: i32, reason: String) -> Result<PurchaseContract, String> {
        ...
        let response: PurchaseContract = ApiService::put(&format!("/purchase-contracts/{}/cancel", id), &req).await?;
        Ok(response)
    }
}
