...
impl PurchaseContractService {
    ...
    pub async fn get_contract(id: i32) -> Result<PurchaseContract, String> {
        let response: ApiResponse<PurchaseContract> = ApiService::get(&format!("/purchase-contracts/{}", id)).await?;
        response.into_result()
    }

    /// 创建采购合同
    pub async fn create_contract(req: CreatePurchaseContractRequest) -> Result<PurchaseContract, String> {
        let response: ApiResponse<PurchaseContract> = ApiService::post("/purchase-contracts", &req).await?;
        response.into_result()
    }

    /// 审核采购合同
    pub async fn approve_contract(id: i32) -> Result<PurchaseContract, String> {
        let response: ApiResponse<PurchaseContract> = ApiService::put(&format!("/purchase-contracts/{}/approve", id), &serde_json::json!({})).await?;
        response.into_result()
    }

    /// 执行采购合同
    pub async fn execute_contract(id: i32, req: ExecutePurchaseContractRequest) -> Result<PurchaseContract, String> {
        let response: ApiResponse<PurchaseContract> = ApiService::put(&format!("/purchase-contracts/{}/execute", id), &req).await?;
        response.into_result()
    }
    ...
    pub async fn cancel_contract(id: i32, reason: String) -> Result<PurchaseContract, String> {
        ...
        let response: ApiResponse<PurchaseContract> = ApiService::put(&format!("/purchase-contracts/{}/cancel", id), &req).await?;
        response.into_result()
    }
}
