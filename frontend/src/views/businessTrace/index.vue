...
const handleFiveDimensionTrace = async () => {
  ...
  try {
    ...
    const res = (await getTraceByFiveDimension(fiveDimensionId.value)) as {
      data?: FullTraceChainResponse;
    };
        ...
        <div v-if="traceResult" class="trace-result">
          <ElCard :title="$t('businessTrace.card.traceChainDetail')" class="detail-card">
            <ElDescriptions :column="3" border>
              ...
              <ElDescriptionsItem :label="$t('businessTrace.field.productId')">{{
                traceResult.product_id
              }}</ElDescriptionsItem>
              <ElDescriptionsItem :label="$t('businessTrace.field.batchNo')">{{
                traceResult.batch_no
              }}</ElDescriptionsItem>
              <ElDescriptionsItem :label="$t('businessTrace.field.colorNo')">{{
                traceResult.color_no
              }}</ElDescriptionsItem>
              <ElDescriptionsItem :label="$t('businessTrace.field.grade')">{{
                traceResult.grade
              }}</ElDescriptionsItem>
              <ElDescriptionsItem :label="$t('businessTrace.field.totalStages')">{{
                traceResult.total_stages
              }}</ElDescriptionsItem>
              <ElDescriptionsItem :label="$t('businessTrace.field.startTime')">{{
                traceResult.start_time
              }}</ElDescriptionsItem>
          ...
          <ElCard :title="$t('businessTrace.card.traceFlow')" class="flow-card">
            ...
            <ElTable
              :data="traceResult.stages"
              border
              style="width: 100%"
              :aria-label="$t('businessTrace.stageTableAriaLabel')"
            >
              <ElTableColumn
                prop="stage_id"
                :label="$t('businessTrace.table.stageId')"
                width="100"
              />
              <ElTableColumn
                prop="stage_name"
                :label="$t('businessTrace.table.stageName')"
                width="120"
              />
              <ElTableColumn
                prop="bill_type"
                :label="$t('businessTrace.table.billType')"
                width="120"
              />
              <ElTableColumn prop="bill_no" :label="$t('businessTrace.table.billNo')" width="150" />
              <ElTableColumn
                prop="warehouse_name"
                :label="$t('businessTrace.table.warehouse')"
                width="120"
              />
              <ElTableColumn
                prop="supplier_name"
                :label="$t('businessTrace.table.supplier')"
                width="120"
              />
              <ElTableColumn
                prop="customer_name"
                :label="$t('businessTrace.table.customer')"
                width="120"
              />
              <ElTableColumn
                prop="quantity_meters"
                :label="$t('businessTrace.table.quantityMeters')"
                width="100"
                align="right"
              />
              <ElTableColumn
                prop="quantity_kg"
                :label="$t('businessTrace.table.quantityKg')"
                width="100"
                align="right"
              />
              <ElTableColumn
                prop="created_at"
                :label="$t('businessTrace.table.time')"
                width="150"
              />
            </ElTable>
          </ElCard>
      ...
      <ElTabPane :label="$t('businessTrace.tab.forward')" name="forward">
        <ElCard :title="$t('businessTrace.card.supplierToCustomer')" class="trace-card">
          <ElForm
            :model="forwardForm"
            label-width="100px"
            :aria-label="$t('businessTrace.form.forwardAriaLabel')"
          >
            <ElRow :gutter="20">
              <ElCol :span="10">
                <ElFormItem :label="$t('businessTrace.field.supplierId')">
                  <ElInputNumber
                    v-model="forwardForm.supplier_id"
                    :placeholder="$t('businessTrace.placeholder.supplierId')"
                  />
                </ElFormItem>
              </ElCol>
              <ElCol :span="10">
                <ElFormItem :label="$t('businessTrace.field.batchNo')">
                  <ElInput
                    v-model="forwardForm.batch_no"
                    :placeholder="$t('businessTrace.placeholder.batchNo')"
                  />
                </ElFormItem>
              </ElCol>
              <ElCol :span="4">
        ...
        >
          ...
          <ElTableColumn
            prop="trace_chain_id"
            :label="$t('businessTrace.table.traceChainId')"
            width="150"
          />
          <ElTableColumn
            prop="five_dimension_id"
            :label="$t('businessTrace.field.fiveDimensionId')"
          />
          ...
          <ElTableColumn
            prop="current_stage"
            :label="$t('businessTrace.table.currentStage')"
            width="120"
          />
          <ElTableColumn
            prop="current_bill_no"
            :label="$t('businessTrace.table.currentBillNo')"
            width="150"
          />
          <ElTableColumn
            prop="created_at"
            :label="$t('businessTrace.table.createdAt')"
            width="150"
          />
        </ElTable>
      ...
      <ElTabPane :label="$t('businessTrace.tab.backward')" name="backward">
        <ElCard :title="$t('businessTrace.card.customerToSupplier')" class="trace-card">
          <ElForm
            :model="backwardForm"
            label-width="100px"
            :aria-label="$t('businessTrace.form.backwardAriaLabel')"
          >
            <ElRow :gutter="20">
              <ElCol :span="10">
                <ElFormItem :label="$t('businessTrace.field.customerId')">
                  <ElInputNumber
                    v-model="backwardForm.customer_id"
                    :placeholder="$t('businessTrace.placeholder.customerId')"
                  />
                </ElFormItem>
              </ElCol>
              <ElCol :span="10">
