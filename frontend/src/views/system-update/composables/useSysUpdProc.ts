...
export function useSysUpdProc(refresh: RefreshCallbacks) {
  ...
  const handleInstall = async (row: SystemVersion) => {
    try {
      ...
      // 批次 98 P2-D 修复（v5 复审）：原 catch (error: any) 改为 unknown + 类型守卫
      if (error !== 'cancel')
        ElMessage.error((error instanceof Error ? error.message : String(error)) || '安装失败');
    }
  };
  ...
  const handleCancelTask = async (row: UpdateTask) => {
    try {
      ...
      // 批次 98 P2-D 修复（v5 复审）：原 catch (error: any) 改为 unknown + 类型守卫
      if (error !== 'cancel')
        ElMessage.error((error instanceof Error ? error.message : String(error)) || '取消失败');
    }
  };
  ...
  const handleRollback = async (row: UpdateTask) => {
    try {
      ...
      // 批次 98 P2-D 修复（v5 复审）：原 catch (error: any) 改为 unknown + 类型守卫
      if (error !== 'cancel')
        ElMessage.error((error instanceof Error ? error.message : String(error)) || '回滚失败');
    }
  };
  ...
  const handleRestore = async (row: SystemBackup) => {
    try {
      ...
      // 批次 98 P2-D 修复（v5 复审）：原 catch (error: any) 改为 unknown + 类型守卫
      if (error !== 'cancel')
        ElMessage.error((error instanceof Error ? error.message : String(error)) || '恢复失败');
    }
  };
