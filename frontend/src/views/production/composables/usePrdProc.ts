...
/**
 * 流程回调（接收 usePrd 返回的列表 + 刷新）
 */
interface PrdCallbacks {
  // 列表数据
  data: Ref<ProductionOrder[]>
  // 刷新列表
...
export function usePrdProc(cb: PrdCallbacks) {
  ...
  const handleExport = () => {
    const csvContent = [
      ...
      ...cb.data.value.map((item: ProductionOrder) => [
        item.order_no,
        item.product_name,
  ...
  `
    ...
    <h1>生产订单列表</h1><div class="meta">打印日期: ${new Date().toISOString().split('T')[0]} | 共 ${cb.data.value.length} 条</div>
