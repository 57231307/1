        ...
        <div class="card-header">
          ...
          <div>
            ...
            <el-button
              type="success"
              :icon="Edit"
              @click="$router.push('/color-prices/batch-adjust')"
            >
...
const {
  ...
  onError: (err: unknown) => {
    ...
    ElMessage.error(
      t('colorPrices.message.loadFailed', { msg: errMsg || t('colorPrices.message.unknownError') })
    );
  },
});
...
const handleDelete = async (row: ColorPriceListItem) => {
  try {
    await ElMessageBox.confirm(
      t('colorPrices.message.deleteConfirm', { id: row.id }),
    ...
    ElMessage.error(
      t('colorPrices.message.deleteFailed', {
        msg: errMsg || t('colorPrices.message.unknownError'),
      })
    );
  }
};
