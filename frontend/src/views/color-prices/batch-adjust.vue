...
<template>
  <div class="batch-adjust">
    <el-page-header :content="$t('colorPrices.batchAdjust.back')" @back="$router.back()" />
      ...
      <el-col :span="14">
        <el-card :header="$t('colorPrices.batchAdjust.selectColor')">
          <el-form
            ...
            :model="filterForm"
            class="filter-form"
            :aria-label="$t('colorPrices.batchAdjust.filter.ariaLabel')"
          >
            <el-form-item :label="$t('colorPrices.batchAdjust.filter.productId')">
              <el-input
                v-model.number="filterForm.product_id"
                :placeholder="$t('colorPrices.batchAdjust.filter.productId')"
                clearable
                style="width: 140px"
              />
            </el-form-item>
            ...
            <el-form-item>
              <el-button type="primary" @click="loadPrices">{{
          ...
          >
            ...
            <el-table-column
              prop="product_id"
              :label="$t('colorPrices.batchAdjust.table.product')"
              width="100"
            />
            <el-table-column
              prop="color_id"
              :label="$t('colorPrices.batchAdjust.table.color')"
              width="100"
            />
            ...
            <el-table-column
              :label="$t('colorPrices.batchAdjust.table.currency')"
              width="80"
              prop="currency"
            />
          </el-table>
      ...
      <el-col :span="10">
        <el-card :header="$t('colorPrices.batchAdjust.adjustSetting')">
          <el-form
            :model="form"
            label-width="120px"
            :aria-label="$t('colorPrices.batchAdjust.form.ariaLabel')"
          >
            <el-form-item :label="$t('colorPrices.batchAdjust.form.selectedCount')">
              <el-tag type="info">{{
                $t('colorPrices.batchAdjust.form.countItems', { count: selectedRows.length })
              }}</el-tag>
            </el-form-item>
            <el-form-item :label="$t('colorPrices.batchAdjust.form.mode')">
              <el-radio-group v-model="form.mode">
                <el-radio-button value="percentage">{{
                  $t('colorPrices.batchAdjust.form.modePercentage')
                }}</el-radio-button>
                <el-radio-button value="fixed">{{
                  $t('colorPrices.batchAdjust.form.modeFixed')
                }}</el-radio-button>
                <el-radio-button value="tier">{{
                  $t('colorPrices.batchAdjust.form.modeTier')
                }}</el-radio-button>
              </el-radio-group>
            </el-form-item>
            <el-form-item
              v-if="form.mode === 'percentage'"
              :label="$t('colorPrices.batchAdjust.form.adjustPercentage')"
            >
              <el-input-number
            ...
            <el-form-item
              ...
              :label="$t('colorPrices.batchAdjust.form.adjustAmount')"
            >
              <el-input-number v-model="form.fixedAmount" :precision="2" style="width: 200px" />
              <span style="margin-left: 8px">{{
                $t('colorPrices.batchAdjust.form.unitYuanPerMeter')
              }}</span>
            </el-form-item>
            ...
            <el-form-item>
              ...
              <el-button @click="handleCalculate">{{
                $t('colorPrices.batchAdjust.form.calculateDemo')
              }}</el-button>
            </el-form-item>
          </el-form>
          ...
          <el-alert
            v-if="result"
            :title="
              $t('colorPrices.batchAdjust.result', {
                auto: result.auto_approved.length,
                pending: result.pending_approval.length,
              })
            "
            type="success"
            :closable="false"
...
const result = ref<{ auto_approved: number[]; pending_approval: number[]; total: number } | null>(
  null
);
...
const loadPrices = async () => {
  ...
  try {
    ...
    ElMessage.error(
      t('colorPrices.message.loadFailed', {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    result.value = r;
    ElMessage.success(
      t('colorPrices.message.batchAdjustSuccess', {
        auto: r.auto_approved.length,
        pending: r.pending_approval.length,
      })
    );
  } catch (e: unknown) {
    ElMessage.error(
      t('colorPrices.message.batchAdjustFailed', {
        msg: e instanceof Error ? e.message : t('colorPrices.message.unknownError'),
      })
    );
  } finally {
...
const handleCalculate = async () => {
  ...
  try {
    ...
    ElMessage.error(
      t('colorPrices.message.calcFailed', {
        msg: e instanceof Error ? e.message : t('colorPrices.message.unknownError'),
      })
    );
  }
