...
const addItem = () => {
  ...
  form.value.items.push({ product_id: 0, quantity: 0, price: 0, amount: 0 })
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (form.value.id) {
      await updatePurchaseReceipt(form.value.id, data)
...
<template>
  <div class="app-container">
    ...
    >
      ...
      <ElTableColumn prop="purchase_order_no" label="采购订单号" width="150" />
      <ElTableColumn prop="supplier_name" label="供应商" width="150" />
      <ElTableColumn prop="warehouse_name" label="仓库" width="120" />
      <ElTableColumn prop="total_amount" label="入库金额" width="120" align="right">
        ...
        <ElRow :gutter="20">
          <ElCol :span="12">
            <ElFormItem label="入库单号" prop="receipt_no">
              <ElInput v-model="form.receipt_no" readonly />
            </ElFormItem>
          </ElCol>
          <ElCol :span="12">
            <ElFormItem label="入库日期" prop="receipt_date">
              <ElDatePicker v-model="form.receipt_date" type="date" />
            </ElFormItem>
          </ElCol>
        </ElRow>
        <ElRow :gutter="20">
