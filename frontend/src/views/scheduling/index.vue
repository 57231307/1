...
const getMockTasks = (): ScheduleTask[] => {
  const workCenters = [
    { id: 1, name: '裁剪中心' },
    { id: 2, name: '缝纫中心' },
    { id: 3, name: '印染中心' },
  ...
  return workCenters.flatMap((wc, i) => {
    return Array.from({ length: 3 }, (_, j) => {
      const start = new Date(today)
      start.setDate(start.getDate() + i * 3 + j * 2)
      const end = new Date(start)
      end.setHours(end.getHours() + 8 + Math.floor(Math.random() * 16))
      const status = j === 0 && i === 1 ? 'conflict' : statuses[Math.floor(Math.random() * 4)]

      return {
        id: i * 10 + j + 1,
        order_no: `WO${String(2026001 + i * 10 + j).slice(-5)}`,
        product_name: products[j % products.length],
...
const getMockConflicts = (): ConflictItem[] => {
  return [
    {
      id: 1,
      task_id_1: 12,
      task_id_2: 15,
      work_center_id: 2,
      work_center_name: '缝纫中心',
      overlap_start: new Date().toISOString(),
      overlap_end: new Date(Date.now() + 2 * 3600000).toISOString(),
      order_no_1: 'WO00212',
      order_no_2: 'WO00215',
      severity: 'error',
    ...
    },
    {
      ...
      task_id_2: 23,
      work_center_id: 3,
      work_center_name: '印染中心',
      overlap_start: new Date(Date.now() + 24 * 3600000).toISOString(),
      overlap_end: new Date(Date.now() + 28 * 3600000).toISOString(),
      order_no_1: 'WO00321',
      order_no_2: 'WO00323',
      severity: 'warning',
