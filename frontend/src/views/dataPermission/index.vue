<template>
  <div class="data-permission">
          ...
          <el-table :data="permissionList" border stripe>
            ...
            <el-table-column prop="scopeType" label="数据范围">
              <template #default="{ row }">
                ...
                <el-tag v-else-if="row.scopeType === 'DEPT_AND_BELOW'" type="warning">本部门及以下</el-tag>
    ...
    <!-- 权限设置对话框 -->
    <el-dialog v-model="permissionDialogVisible" :title="isEdit ? '编辑数据权限' : '添加数据权限'" width="600px">
      <el-form :model="permissionForm" :rules="permissionRules" ref="permissionFormRef" label-width="120px">
        ...
        <el-form-item label="数据范围" prop="scopeType">
          <el-select v-model="permissionForm.scopeType" placeholder="请选择" style="width: 100%">
            <el-option v-for="scope in scopeTypeList" :key="scope.value" :label="scope.label" :value="scope.value">
              <span>{{ scope.label }}</span>
              <span style="color: #909399; font-size: 12px; margin-left: 8px;">{{ scope.description }}</span>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="自定义条件" prop="customCondition" v-if="permissionForm.scopeType === 'CUSTOM'">
          <el-input v-model="permissionForm.customCondition" type="textarea" :rows="4" placeholder="输入自定义SQL条件或JSON配置" />
        </el-form-item>
        <el-form-item label="允许字段" prop="allowedFields">
          <el-input v-model="permissionForm.allowedFields" type="textarea" :rows="2" placeholder="输入允许的字段，多个字段用逗号分隔" />
        </el-form-item>
        <el-form-item label="隐藏字段" prop="hiddenFields">
          <el-input v-model="permissionForm.hiddenFields" type="textarea" :rows="2" placeholder="输入隐藏的字段，多个字段用逗号分隔" />
        </el-form-item>
      </el-form>
      
      <template #footer>
        <el-button @click="permissionDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handleSavePermission" :loading="submitLoading">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const fetchScopeTypes = async () => {
  try {
    ...
    scopeTypeList.value = [
      ...
      { value: 'DEPT_AND_BELOW', label: '本部门及以下', description: '可以查看本部门及下级部门的数据' },
...
const handleDeletePermission = async (row: DataPermission) => {
  ...
  try {
    ...
    await deleteDataPermission(row.roleId, row.resourceType)
