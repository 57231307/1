  ...
  <div class="crm-leads-page">
    ...
    <el-card shadow="hover" class="filter-card">
      <el-form
        ...
        :model="queryParams"
        class="filter-form"
        :aria-label="$t('crmLeads.filter.ariaLabel')"
      >
    ...
    <el-card shadow="hover" class="table-card">
      ...
      >
        ...
        <el-table-column
          type="index"
          :label="$t('crmLeads.table.index')"
          width="60"
          align="center"
        />
        <el-table-column
          prop="lead_no"
          :label="$t('crmLeads.table.leadNo')"
          width="120"
        ...
        <el-table-column
          prop="contact_name"
          :label="$t('crmLeads.table.contactName')"
          width="100"
          show-overflow-tooltip
        />
        <el-table-column
          prop="mobile_phone"
          :label="$t('crmLeads.table.mobilePhone')"
          width="120"
          show-overflow-tooltip
        />
        <el-table-column
          prop="lead_source"
          :label="$t('crmLeads.table.leadSource')"
          width="100"
          align="center"
        >
        ...
        <el-table-column
          prop="lead_status"
          :label="$t('crmLeads.table.leadStatus')"
          width="100"
          align="center"
        >
          <template #default="{ row }">
        ...
        <el-table-column
          prop="priority"
          :label="$t('crmLeads.table.priority')"
          width="80"
          align="center"
        >
          <template #default="{ row }">
        ...
        <el-table-column
          prop="owner_name"
          :label="$t('crmLeads.table.owner')"
          width="100"
          show-overflow-tooltip
        />
        <el-table-column
          prop="last_follow_up_date"
          :label="$t('crmLeads.table.lastFollowUp')"
          width="120"
          align="center"
        />
        <el-table-column
          prop="next_follow_up_date"
          :label="$t('crmLeads.table.nextFollowUp')"
          width="120"
          align="center"
        />
        <el-table-column
          :label="$t('crmLeads.table.operation')"
          width="250"
          align="center"
          fixed="right"
        >
...
const handleView = async (row: LeadRow) => {
  try {
    ...
    await ElMessageBox.alert(lines.join('\n'), t('crmLeads.detail.title'), {
      confirmButtonText: t('crmLeads.detail.close'),
    });
  } catch (error) {
...
const handleContact = async (row: LeadRow) => {
  try {
    await ElMessageBox.confirm(
      t('crmLeads.message.contactConfirm', { name: row.contact_name }),
      t('crmLeads.message.tip'),
      {
        type: 'warning',
      }
    );
...
const handleConvert = async (row: LeadRow) => {
  try {
    await ElMessageBox.confirm(
      t('crmLeads.message.convertConfirm', { name: row.contact_name }),
      t('crmLeads.message.tip'),
      {
        type: 'warning',
      }
...
const handleLost = async (row: LeadRow) => {
  try {
    await ElMessageBox.confirm(
      t('crmLeads.message.lostConfirm', { name: row.contact_name }),
      t('crmLeads.message.tip'),
      {
        type: 'warning',
      }
    ...
    try {
      ...
      if (result.failed_count > 0) {
        ...
        await ElMessageBox.alert(
          t('crmLeads.message.importPartialResult', {
            total: result.total,
...
const handleExport = async () => {
  try {
    ...
    link.setAttribute(
      'download',
      `${t('crmLeads.exportFile.filename')}_${new Date().toISOString().split('T')[0]}.xlsx`
    );
