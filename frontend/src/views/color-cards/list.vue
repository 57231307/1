  ...
  <div class="color-card-list">
    <el-card>
      ...
      <el-form
        ...
        :model="filterForm"
        class="filter-form"
        :aria-label="$t('colorCards.filter.ariaLabel')"
      >
        <el-form-item :label="$t('colorCards.filter.cardType')">
          <el-select
            v-model="filterForm.card_type"
            :placeholder="$t('colorCards.filter.all')"
            clearable
            style="width: 140px"
          >
            <el-option
              v-for="value in cardTypeKeys"
              :key="value"
        ...
        <el-form-item :label="$t('colorCards.filter.keyword')">
          <el-input
            ...
            :placeholder="$t('colorCards.filter.keywordPlaceholder')"
            clearable
            style="width: 180px"
          />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :icon="Search" @click="handleSearch">{{
            $t('colorCards.filter.query')
          }}</el-button>
          <el-button :icon="Refresh" @click="handleReset">{{
            $t('colorCards.filter.reset')
          }}</el-button>
        </el-form-item>
      </el-form>
      ...
      <el-table
        ...
        border
        stripe
        :aria-label="$t('colorCards.table.ariaLabel')"
      >
        <el-table-column prop="card_no" :label="$t('colorCards.table.cardNo')" width="180" />
        <el-table-column
          prop="card_name"
          :label="$t('colorCards.table.cardName')"
          min-width="200"
        />
        ...
        <el-table-column
          prop="total_colors"
          :label="$t('colorCards.table.totalColors')"
          width="80"
          align="center"
        />
        ...
        <el-table-column :label="$t('colorCards.table.operation')" width="200" fixed="right">
          <template #default="{ row }">
            ...
            <el-button
              ...
              >{{ $t('colorCards.table.archive') }}</el-button
            >
          </template>
        </el-table-column>
      </el-table>
...
const handleArchive = async (row: ColorCardListItem) => {
  try {
    await ElMessageBox.confirm(
      t('colorCards.message.archiveConfirm', { name: row.card_name }),
      t('colorCards.message.archiveConfirmTitle'),
      {
        type: 'warning',
      }
    );
    ...
    if (e !== 'cancel')
      ElMessage.error(
        t('colorCards.message.archiveFailed') + ': ' + (e instanceof Error ? e.message : String(e))
      );
  }
};
...
const tagType = (status: string): TagType => (COLOR_CARD_STATUS_COLORS[status] || '') as TagType;
