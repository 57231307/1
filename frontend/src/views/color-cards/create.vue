  ...
  <div class="color-card-create">
    <el-card>
      ...
      <el-form
        ...
        :aria-label="$t('colorCards.create.formAriaLabel')"
      >
        <el-form-item :label="$t('colorCards.create.cardNo')" prop="card_no">
          <el-input
            v-model="form.card_no"
            :placeholder="$t('colorCards.create.cardNoPlaceholder')"
          />
        </el-form-item>
        <el-form-item :label="$t('colorCards.create.cardName')" prop="card_name">
          <el-input
            v-model="form.card_name"
            :placeholder="$t('colorCards.create.cardNamePlaceholder')"
          />
        </el-form-item>
        <el-form-item :label="$t('colorCards.create.cardType')" prop="card_type">
          <el-select
            v-model="form.card_type"
            :placeholder="$t('colorCards.create.pleaseSelect')"
            style="width: 100%"
          >
            <el-option
              v-for="(label, value) in COLOR_CARD_TYPE_LABELS"
              :key="value"
              :label="label"
              :value="value"
            />
        ...
        <el-form-item :label="$t('colorCards.filter.season')">
          <el-select
            v-model="form.season"
            :placeholder="$t('colorCards.create.optional')"
            clearable
            style="width: 100%"
          >
            <el-option
              v-for="(label, value) in SEASON_LABELS"
              :key="value"
              :label="label"
              :value="value"
            />
        ...
        <el-form-item :label="$t('colorCards.create.coverImageUrl')">
          <el-input
            ...
            :placeholder="$t('colorCards.create.coverImageUrlPlaceholder')"
          />
        </el-form-item>
        <el-form-item :label="$t('colorCards.create.description')">
          <el-input
            ...
            type="textarea"
            :rows="3"
            :placeholder="$t('colorCards.create.descriptionPlaceholder')"
          />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="submitting" @click="handleSubmit">{{
            $t('colorCards.create.submit')
          }}</el-button>
          <el-button @click="$router.back()">{{ $t('colorCards.create.cancel') }}</el-button>
        </el-form-item>
      </el-form>

      <el-alert v-if="createdCardId" type="success" :closable="false" style="margin-top: 24px">
        <template #title>
          {{ $t('colorCards.create.successAlert', { id: createdCardId }) }}
        </template>
        <div style="margin-top: 8px">
          <el-button type="primary" size="small" @click="goAddItems">{{
            $t('colorCards.create.addItems')
          }}</el-button>
          <el-button size="small" @click="resetForm">{{
            $t('colorCards.create.continueCreate')
          }}</el-button>
        </div>
      </el-alert>
    </el-card>
...
const rules = computed<FormRules>(() => ({
  card_no: [
    { required: true, message: t('colorCards.validation.cardNoRequired'), trigger: 'blur' },
  ],
  card_name: [
    { required: true, message: t('colorCards.validation.cardNameRequired'), trigger: 'blur' },
  ],
  card_type: [
    { required: true, message: t('colorCards.validation.cardTypeRequired'), trigger: 'change' },
  ],
}));
