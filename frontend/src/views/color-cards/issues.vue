<template>
  <div class="color-card-issue">
    <el-page-header
      :icon="ArrowLeft"
      :content="$t('colorCards.issue.back')"
      ...
      <el-tabs v-model="activeTab" :aria-label="$t('colorCards.issue.tabAriaLabel')">
        ...
        <el-tab-pane :label="$t('colorCards.issue.tab.issue')" name="issue">
          <el-form
            ...
            :aria-label="$t('colorCards.issue.form.ariaLabel')"
          >
            <el-form-item :label="$t('colorCards.issue.form.selectCard')" prop="color_card_id">
              <el-select
                v-model="issueForm.color_card_id"
                filterable
                :placeholder="$t('colorCards.issue.form.selectCardPlaceholder')"
                style="width: 100%"
              >
                <el-option
            ...
            <el-form-item :label="$t('colorCards.issue.form.customerId')" prop="customer_id">
              <el-input-number
                v-model="issueForm.customer_id"
                :min="1"
                style="width: 100%"
                :placeholder="$t('colorCards.issue.form.customerIdPlaceholder')"
              />
            </el-form-item>
            ...
            <el-form-item :label="$t('colorCards.issue.form.dyeLotNo')">
              <el-input
                v-model="issueForm.dye_lot_no"
                :placeholder="$t('colorCards.issue.form.dyeLotNoPlaceholder')"
              />
            </el-form-item>
            ...
            <el-form-item :label="$t('colorCards.issue.form.purpose')">
              <el-input
                ...
                :placeholder="$t('colorCards.issue.form.purposePlaceholder')"
              />
            </el-form-item>
            ...
            <el-form-item>
              ...
              <el-button
                v-permission="'color_card_issue:create'"
                type="primary"
                :loading="issuing"
                @click="handleIssue"
                >{{ $t('colorCards.issue.form.confirmIssue') }}</el-button
              >
            </el-form-item>
          </el-form>
        </el-tab-pane>
        ...
        <!-- 发放中列表 -->
        <el-tab-pane
          :label="$t('colorCards.issue.tab.active', { count: activeIssues.length })"
          name="active"
        >
          <el-table
            :data="activeIssues"
            border
            :aria-label="$t('colorCards.issue.activeTable.ariaLabel')"
          >
            <el-table-column
              prop="color_card_id"
              :label="$t('colorCards.issue.activeTable.cardId')"
              width="100"
            />
            <el-table-column
              :label="$t('colorCards.issue.activeTable.customerId')"
              prop="customer_id"
              width="100"
            />
            <el-table-column
              :label="$t('colorCards.issue.activeTable.issueQty')"
              prop="issue_qty"
              width="100"
            />
            <el-table-column
              :label="$t('colorCards.issue.activeTable.issuedBy')"
              prop="issued_by"
              width="100"
            />
            ...
            <el-table-column
              :label="$t('colorCards.issue.activeTable.dyeLotNo')"
              prop="dye_lot_no"
              width="140"
            />
            <el-table-column :label="$t('colorCards.issue.activeTable.purpose')" prop="purpose" />
            <el-table-column
              :label="$t('colorCards.issue.activeTable.operation')"
              width="360"
              fixed="right"
            >
              <template #default="{ row }">
                ...
                <el-button
                  v-permission="'color_card_issue:return'"
                  link
                  type="primary"
                  :aria-label="$t('colorCards.issue.activeTable.returnAriaLabel')"
                  @click="handleReturn(row)"
                  >{{ $t('colorCards.issue.activeTable.return') }}</el-button
                >
                <el-button
                  v-permission="'color_card_issue:damaged'"
                  link
                  type="warning"
                  :aria-label="$t('colorCards.issue.activeTable.damagedAriaLabel')"
                  @click="handleMarkDamaged(row)"
                  >{{ $t('colorCards.issue.activeTable.damaged') }}</el-button
                >
                <el-button
                  v-permission="'color_card_issue:lost'"
                  link
                  type="danger"
                  :aria-label="$t('colorCards.issue.activeTable.lostAriaLabel')"
                  @click="handleMarkLost(row)"
                  >{{ $t('colorCards.issue.activeTable.lost') }}</el-button
                >
                <el-button
                  v-permission="'color_card_issue:cancel'"
                  link
                  type="info"
                  :aria-label="$t('colorCards.issue.activeTable.cancelAriaLabel')"
                  @click="handleCancel(row)"
                  >{{ $t('colorCards.issue.activeTable.cancel') }}</el-button
                >
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-tab-pane :label="$t('colorCards.issue.tab.history')" name="history">
          <el-table
            :data="historyRecords"
            border
            :aria-label="$t('colorCards.issue.historyTable.ariaLabel')"
          >
            <el-table-column
              prop="id"
              :label="$t('colorCards.issue.historyTable.recordId')"
              width="100"
            />
            <el-table-column
              prop="color_card_id"
              :label="$t('colorCards.issue.historyTable.cardId')"
              width="100"
            />
            <el-table-column
              prop="customer_id"
              :label="$t('colorCards.issue.historyTable.customerId')"
              width="100"
            />
            <el-table-column
              prop="issue_qty"
              :label="$t('colorCards.issue.historyTable.issueQty')"
              width="100"
            />
            <el-table-column :label="$t('colorCards.issue.historyTable.status')" width="100">
              <template #default="{ row }">
                <el-tag
                  :type="
                    ISSUE_STATUS_COLORS[row.status] as 'success' | 'warning' | 'danger' | 'info'
                  "
                >
            ...
            <el-table-column
              :label="$t('colorCards.issue.historyTable.compensationAmount')"
              width="120"
            >
              <template #default="{ row }">{{ row.compensation_amount || '-' }}</template>
            </el-table-column>
            <el-table-column
              :label="$t('colorCards.issue.historyTable.dyeLotNo')"
              prop="dye_lot_no"
              width="140"
            />
    ...
    <el-dialog
      ...
      :title="$t('colorCards.issue.returnDialog.title')"
      :aria-label="$t('colorCards.issue.returnDialog.ariaLabel')"
      width="480px"
    >
      ...
      <template #footer>
        <el-button @click="showReturnDialog = false">{{
          $t('colorCards.issue.returnDialog.cancel')
        }}</el-button>
        <el-button type="primary" :loading="actionLoading" @click="confirmReturn">{{
          $t('colorCards.issue.returnDialog.confirm')
        }}</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="$t('colorCards.issue.lostDialog.title')"
      :aria-label="$t('colorCards.issue.lostDialog.ariaLabel')"
      width="480px"
    >
      ...
      <template #footer>
        <el-button @click="showLostDialog = false">{{
          $t('colorCards.issue.lostDialog.cancel')
        }}</el-button>
        <el-button type="danger" :loading="actionLoading" @click="confirmLost">{{
          $t('colorCards.issue.lostDialog.confirm')
        }}</el-button>
      </template>
    </el-dialog>
    ...
    <!-- 损坏对话框 -->
    <el-dialog
      v-model="showDamagedDialog"
      :title="$t('colorCards.issue.damagedDialog.title')"
      :aria-label="$t('colorCards.issue.damagedDialog.ariaLabel')"
      width="480px"
    >
      ...
      <template #footer>
        <el-button @click="showDamagedDialog = false">{{
          $t('colorCards.issue.damagedDialog.cancel')
        }}</el-button>
        <el-button type="warning" :loading="actionLoading" @click="confirmDamaged">{{
          $t('colorCards.issue.damagedDialog.confirm')
        }}</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="$t('colorCards.issue.cancelDialog.title')"
      :aria-label="$t('colorCards.issue.cancelDialog.ariaLabel')"
      width="480px"
    >
      ...
      <template #footer>
        <el-button @click="showCancelDialog = false">{{
          $t('colorCards.issue.cancelDialog.close')
        }}</el-button>
        <el-button type="info" :loading="actionLoading" @click="confirmCancel">{{
          $t('colorCards.issue.cancelDialog.confirm')
        }}</el-button>
      </template>
    </el-dialog>
  </div>
...
const issueRules = computed(() => ({
  color_card_id: [
    {
      required: true,
      message: t('colorCards.issue.message.selectCardRequired'),
      trigger: 'change',
    },
  ],
  customer_id: [
    { required: true, message: t('colorCards.issue.message.customerIdRequired'), trigger: 'blur' },
  ],
}));
