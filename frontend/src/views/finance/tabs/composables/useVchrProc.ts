 ...
 */
export function useVchrProc(vouchers: { value: Voucher[] }, fetchVouchers: () => Promise<void>) {
  /** 提交凭证 */
  ...
  const handlePrintVouchers = () => {
    ...
    const rows = vouchers.value
      ...
      <tr>
        ...
        <td>${escapeHtml(getVchrStatusLabel(item.status))}</td><td>${escapeHtml(item.created_by_name
          || '-')}</td>
      </tr>
    `
      )
