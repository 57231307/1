<template>
  <div class="quality-page">
        ...
        <el-card shadow="hover">
          <el-table :data="standards" v-loading="standardLoading" stripe>
            ...
            <el-table-column prop="status" label="状态" width="100" align="center">
              <template #default="{ row }">
                <el-tag :type="row.status === 'published' ? 'success' : row.status === 'approved' ? 'warning' : 'info'" size="small">
    ...
    <el-dialog v-model="standardDialogVisible" :title="standardForm.id ? '编辑标准' : '新建标准'" width="700px">
      <el-form ref="standardFormRef" :model="standardForm" label-width="100px">
        ...
        <el-form-item label="类型" prop="type">
          <el-select v-model="standardForm.type" style="width: 100%">
        ...
        <el-form-item label="附件" prop="attachments">
          <el-input v-model="attachmentsText" type="textarea" placeholder="JSON格式数组" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { listQualityStandards, getQualityStandard, createQualityStandard, updateQualityStandard, approveQualityStandard, publishQualityStandard, listQualityRecords, createQualityRecord, listDefects, processDefect as processDefectApi, type QualityStandard, type QualityRecord, type Defect } from '@/api/quality'
...
const getStandardStatusLabel = (status: string) => {
  const map: Record<string, string> = { draft: '草稿', approved: '已审批', published: '已发布' }
...
const standardForm = reactive({ id: 0, standard_code: '', standard_name: '', version: '1.0', type: 'product', status: 'draft', content: '', attachments: [] as string[] })
...
const recordForm = reactive({ id: 0, record_no: '', inspection_type: '', product_id: undefined as number | undefined, product_name: '', batch_no: '', inspection_date: '', inspector: '', result: 'pending', defects: [] as Defect[], remark: '' })
