...
-->
<template>
  <el-dialog v-model="visible" :title="t('customerCredit.adjust.title')" width="500px" :aria-label="t('customerCredit.adjust.ariaLabel')">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="120px" :aria-label="t('customerCredit.adjust.formAriaLabel')">
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="submitLoading" @click="handleSubmit">{{ t('customerCredit.adjust.button.confirm') }}</el-button>
    </template>
  </el-dialog>
</template>
...
const rules: FormRules = {
  adjustmentType: [{ required: true, message: t('customerCredit.adjust.validation.adjustmentTypeRequired'), trigger: 'change' }],
  amount: [{ required: true, message: t('customerCredit.adjust.validation.amountRequired'), trigger: 'blur' }],
  reason: [{ required: true, message: t('customerCredit.adjust.validation.reasonRequired'), trigger: 'blur' }],
}
