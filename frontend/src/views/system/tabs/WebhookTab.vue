...
<template>
  <div class="webhook-tab">
    ...
    <el-card shadow="hover">
      <el-table v-loading="webhookLoading" :data="webhookList" stripe :aria-label="t('system.webhook.aria.list')">
        ...
        <el-table-column prop="is_active" :label="t('system.webhook.column.status')" width="80" align="center">
        ...
        <el-table-column :label="t('system.webhook.column.action')" width="200" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-permission="'webhook:update'" size="small" link @click="openWebhookDialog(row as unknown as WebhookRow)">{{ t('system.webhook.button.edit') }}</el-button>
            <el-button
              size="small"
              link
              type="warning"
    ...
    <el-dialog
      ...
      :title="webhookForm.id ? t('system.webhook.dialog.editTitle') : t('system.webhook.dialog.createTitle')"
      width="500px"
      :aria-label="t('system.webhook.dialog.aria')"
    >
      <el-form ref="webhookFormRef" :model="webhookForm" label-width="100px" :aria-label="t('system.webhook.form.aria')">
        ...
        <el-form-item :label="t('system.webhook.form.label.eventType')">
          <el-select v-model="webhookForm.event_type" style="width: 100%">
            ...
            <el-option :label="t('system.webhook.event.inventoryChanged')" value="inventory.changed" />
            <el-option :label="t('system.webhook.event.approvalCompleted')" value="approval.completed" />
            <el-option :label="t('system.webhook.event.all')" value="all" />
          </el-select>
        </el-form-item>
        <el-form-item :label="t('system.webhook.form.label.secret')">
          <el-input v-model="webhookForm.secret" :placeholder="t('system.webhook.form.placeholder.secret')" />
        </el-form-item>
      ...
      <template #footer>
        <el-button @click="webhookDialogVisible = false">{{ t('system.webhook.form.button.cancel') }}</el-button>
        <el-button type="primary" @click="saveWebhook">{{ t('system.webhook.form.button.confirm') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const deleteWebhook = async (row: WebhookRow) => {
  try {
    await ElMessageBox.confirm(t('system.webhook.message.deleteConfirm'), t('system.webhook.message.deleteTitle'), { type: 'warning' })
