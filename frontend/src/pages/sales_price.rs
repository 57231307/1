    ...
    let load_data = {
        ...
        Callback::from(move |_| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                match SalesPriceService::list(None, None, None, None, None, 1, 50).await {
                    Ok(data) => {
                        prices.set(data);
                        loading.set(false);
                    }
                ...
                <div>
                    ...
                    <button class="btn btn-success" onclick={Callback::from(|_| {
                        if let Some(window) = web_sys::window() {
                            let _ = window.alert_with_message("新增价格功能开发中");
                        }
                    })}>{"新增定价"}</button>
                </div>
            </div>
                ...
                <table class="table" style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background-color: #f5f5f5; text-align: left;">
                            ...
                            <th style="padding: 10px; border-bottom: 1px solid #ddd;">{"客户类型"}</th>
                            <th style="padding: 10px; border-bottom: 1px solid #ddd;">{"价格等级"}</th>
                        ...
                        { for prices.iter().map(|p| {
                            html! {
                                <tr style="border-bottom: 1px solid #eee;">
                                    ...
                                    <td style="padding: 10px;">{ p.customer_type.clone().unwrap_or_else(|| "全部".to_string()) }</td>
                                    <td style="padding: 10px;">{ p.price_level.clone().unwrap_or_default() }</td>
                                    <td style="padding: 10px; font-weight: bold; color: #e67e22;">{ &p.price }</td>
