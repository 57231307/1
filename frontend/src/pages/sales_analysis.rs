            ...
            <table class="min-w-full bg-white border border-gray-200">
                <thead>
                    <tr>
                        ...
                        <th class="py-2 px-4 border-b">{ "目标金额" }</th>
                        <th class="py-2 px-4 border-b">{ "实际金额" }</th>
                        <th class="py-2 px-4 border-b">{ "完成率" }</th>
                            ...
                            html! {
                                for targets.iter().map(|target| html! {
                                    <tr key={target.id}>
                                        ...
                                        <td class="py-2 px-4 border-b text-center">{ &target.target_amount }</td>
                                        <td class="py-2 px-4 border-b text-center">{ &target.actual_amount }</td>
                                        <td class="py-2 px-4 border-b text-center">{ &target.completion_rate }</td>
                                        <td class="py-2 px-4 border-b text-center">{ &target.status }</td>
                                    </tr>
                                })
                            }
                        }
