...
async function testPage(page, pageConfig) {
  ...
  try {
    ...
    const addBtn = await page.$(
      'button:contains("新增"), button:contains("添加"), .el-button--primary'
    )
    if (addBtn) {
      try {
        await addBtn.click()
        ...
        if (form) {
          ...
          const saveBtn = await page.$(
            'button:contains("保存"), button:contains("确定"), .el-button--primary'
          )
          if (saveBtn) {
            await saveBtn.click()
            await new Promise((r) => setTimeout(r, 3000))
...
async function main() {
  ...
  // 保存详细报告
  fs.writeFileSync('/workspace/frontend/test_report.json', JSON.stringify(errorReport, null, 2))
  ...
  if (errorReport.buttonNotWorking.length > 0) {
    console.log('\n4. 按钮功能修复:')
    errorReport.buttonNotWorking.forEach((e) =>
      console.log(`   - ${e.page}: ${e.button} - ${e.reason}`)
    )
  }
