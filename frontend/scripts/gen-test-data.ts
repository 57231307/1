 ...
 *
 * 用法:
 *   1. 在浏览器中打开 POC 页面 /inventory-poc
 *   2. 打开 DevTools Console,执行 fetch('/__poc__/gen-test-data.json?count=10000')
 *      (由 VirtualStockTabPOC.vue 中暴露的 window.__poc__ 工具触发)
 *   3. 或直接导入此文件生成数据后写入 localStorage:
 *        const data = generateStocks(10000)
 *        localStorage.setItem('__poc_stock_test_data', JSON.stringify(data))
 *
 * 脚本目标: 为 el-table-v2 POC 提供 1 万行与原表格列结构一致的测试数据。
 */
