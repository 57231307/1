...
- **P2-1 实际已完成 5/5 PR**（#108 / #109 / #110 / #111 / #112 + #181 重提交），非 v0.3 摘要中"PR-1 + PR-2 已完成"
  - 全部早已合并至 main
- 修正后 P12 批 1 进展：**10/10 PR 完成**
- **P0 port 销售报价单 4 PR 串行全部完成**（PR #183/#184/#185/#186）
- **P2-1 5 PR 全部完成**（PR #108/#109/#110/#111/#112 + #181 重提交）
- **B-type-check 已完成**（PR #188）
- **P2-2 性能优化已完成**（PR #182）

#### P12 批 1 收尾计划（1 子代理 / 已无 PR）
- 当前 P12 批 1 v0.3 全部目标已完成（10/10 PR）
- 后续 P12 批 2 建议：
  - 子代理 E：清理 16 个预存 vue-tsc 错误（auth.ts / V2Table / useTableApi / StockTab / production / OrderListView），移除 ci-cd.yml `|| true`，让 type-check 真正拦截
  - 子代理 F：P3-1 安全加固 / B-慢查询审计（v0.3 §3.2/3.3 候选）
