...
---
### P12 批 1 合并汇总（2026-06-17 ~ 2026-06-18，4/10 PR 完成）
...
- **目标**：从 test 分支（独有 1154 commit）port 销售报价单数据层到 main
- **数据层范围**：
  - 3 个迁移文件（m0020/0021/0022：sales_quotations / sales_quotation_items / sales_quotation_terms）
  - 3 对 SQL up/down 脚本
  - 3 个 SeaORM 模型（sales_quotation / sales_quotation_item / sales_quotation_term）
- **关键约束**：
  - test 与 main 无共同祖先，所有代码**重新实现**（不直接 copy）
  - stub pricing（不引入 product_color_price 依赖）
  - 保留 test 业务语义 + 适配 main 现有 `sales_order_handler` 命名
  - 主外键 ID 均为 i32 / SERIAL（与 main 已有 sales_order / customer / user / product 保持一致）
- **CI 验证**：4 job 全绿（squash merge）
...
- **Redis 缓存层**（`backend/src/cache/redis_client.rs`，480 行）
  - `CacheBackend` trait 抽象 + `RedisBackend` 真实实现 + `NullBackend` 优雅降级
  ...
  - `CacheStats` 命中/未命中/错误统计 + `snapshot` 快照
- **缓存模块入口**：`backend/src/cache/mod.rs`（14 行，re-export 全部 API）
- **依赖**：`backend/Cargo.toml` 新增 `redis = { version = "0.27", features = ["tokio-comp", "connection-manager"] }`
- **业务接入**：
  - `backend/src/services/user_service.rs`（+60 行）：缓存 `user::Model` 列表（5min TTL）
  - `backend/src/services/product_service.rs`（+50 行）：缓存 `product::Model` 列表（5min TTL）
- **DB N+1 审计**：`docs/audits/2026-06-18-db-n1-audit.md`（278 行）
  - 审计 `services/` 下所有 Service 列表查询
  - 验证 Eager Loading 配置（`select::Model` 完整字段）
- **CI 修复历程**（4 轮迭代）：
  1. `a6d65a4`：用 `use crate::cache` 修正 import 路径
  2. `068f314`：在 `main.rs` 加 `mod cache` 暴露
  3. `bfbf6a1`：cache 模块文件级 `#![allow(dead_code)]` + TODO（server bin 独立编译时辅助 API 无调用方）
