...
### P14 批 2 B3 拆分大 .vue - I-3 第 1 批（2026-06-19）

- **PR #195**（commit 2834f86d）squash merge 入 main
- **目标**：拆分 3 个大 .vue（VoucherListTab 870 + system-update 725 + sales-contract 717）
- **子代理**：I-3 第 1 批（独立派发，6 commit 含 2 CI 修复）
- **新增 13 个子组件 + 6 composable + 3 工具**：
  - voucher/tabs/VoucherListTab（4 子组件 + 2 composable + 1 工具）：VchrLstFilter / VchrLstTbl / VchrLstForm / VchrLstDetail / useVchrLst / useVchrLstProc / vchrLstFmts
  - system-update（6 子组件 + 2 composable + 1 工具）：SuInfoCards / SuVerTbl / SuTaskTbl / SuBkpTbl / SuVerDetail / SuBkpForm / useSysUpd / useSysUpdProc / sysUpdFmts
  - sales-contract（3 子组件 + 2 composable + 1 工具）：ScFilter / ScTbl / ScForm / useSc / useScProc / scFmts
- **父组件重写**：
  - voucher/tabs/VoucherListTab.vue：870 → 141 行（-84%）
  - system-update/index.vue：725 → 154 行（-79%）
  - sales-contract/index.vue：717 → 129 行（-82%）
  - 合计 2312 → 424 行（-82%）
- **命名规范**：VchrLst（Voucher List 前缀，避免与 I-2 Vchr 系列冲突）/ Su / Sc
- **CI 4 轮迭代修复**（I-3 子代理全程自主修复）：
  1. **composable ref 字段在父组件访问时未自动解包**（10 个 vue-tsc 错误）— Vue 3 仅在 setup 顶层 return 时自动解包 ref，const vchr = useXxx() 中 vchr 是普通对象，访问 vchr.xxx 仍是 ref。改用 `reactive({...})` 包装 return（UnwrapNestedRefs 类型推导）
  2. **sc.dateRange.value 错误**（sales-contract/index.vue:100）— reactive 已自动解包 ref，移除 .value
  3. **VoucherForm 类型不兼容 Partial<VoucherEntity>**（VchrLstForm.vue:40）— VoucherForm 改为所有字段可选
  4. **未使用的 VoucherEntity import**（VchrLstForm.vue:138）— 移除 import
- **经验沉淀**：
  - composable 设计原则：用 `reactive({...})` 包装 return，访问字段时 Vue 自动解包 ref（UnwrapNestedRefs 类型推导）
  - Vue 自动解包机制仅作用于 setup 顶层 return，const 中间变量不会触发
  - 子组件 prop 期望 plain value 时，composable 必须用 reactive 包装 return
- **行为完全一致**：纯结构重构，不改业务逻辑/UI/交互

### P14 批 2 B3 拆分大 .vue - I-3 第 2 批（2026-06-19）

- **PR #196**（commit 0622b82）squash merge 入 main
- **目标**：拆分剩余 3 个大 .vue（purchase-return 695 + scheduling/gantt 691 + scheduling/index 689）
- **子代理**：I-3 第 2 批（独立派发，5 commit 含 4 CI 修复）
- **新增 15 个子组件 + 6 composable + 3 工具**：
  - purchase-return（5 子组件 + 2 composable + 1 工具）：PrRtnFilter / PrRtnTbl / PrRtnForm / PrRtnDetail / PrRtnApr / usePrRtn / usePrRtnProc / prRtnFmts
  - scheduling/gantt（5 子组件 + 2 composable + 1 工具）：SchGTool / SchGChart / SchGAuto / SchGAdj / SchGConf / useSchG / useSchGProc / schGFmts
  - scheduling/index（5 子组件 + 2 composable + 1 工具）：SchMTool / SchMTbl / SchMConf / SchMParam / SchMAdj / useSchM / useSchMProc / schMFmts
- **父组件重写**：
  - purchase-return/index.vue：695 → 211 行（-70%）
  - scheduling/gantt.vue：691 → 93 行（-87%）
  - scheduling/index.vue：689 → 109 行（-84%）
  - 合计 2075 → 413 行（-80%）
- **命名规范**：PrRtn（Purchase Return）/ SchG（Scheduling Gantt）/ SchM（Scheduling Main），gantt + index 用 G/M 区分避免冲突
- **CI 5 轮迭代修复**（I-3 第 2 批子代理全程自主修复）：
  1. **14 个子组件 `const props = defineProps<...>()` 未使用**（TS6133）— defineProps 不再赋给变量，直接 `defineProps<{...}>()` 即可
  2. **useSchGProc.ts 未使用 SchedulingParams import**（TS6133）— 移除未用 import
  3. **SchMTbl.vue v-model 不能用于 prop**（构建错误 v-model on prop）— 改用 `:model-value` + `@update:model-value` + emit 模式
  4. **SchMTbl emit 父组件类型推断错误**（vue-tsc 报 `(v: any) => any` 不可赋给 `(...args: any[]) => any`）— emit 改用 object 形式 `{ name: [args] }`（Vue 3.3+），与 I-3 第 1 批 ScTbl 保持一致
  5. **SchMTbl 冗余 size-change/current-change emit**（TS2769 No overload matches）— 移除冗余 emit（已被 update:* 取代）
- **关键设计决策**：
  - **ECharts 渲染从父组件迁移到 SchGChart 子组件** — 子组件内部管理 ECharts 实例 + resize 监听 + watch 渲染，父组件不再管理 ganttChartRef
  - **冲突列表传入 SchGAdj.vue 作为 prop** — 调整对话框接收 work-centers 而非单独加载
- **经验沉淀**：
  - 子组件 prop 定义 `const props = defineProps<...>()` 会触发 TS6133（当 props 未在 setup 中使用时）— Vue 3.3+ 直接用 `defineProps<...>()`
  - v-model 不能用于 prop（不仅是 vue-tsc，Vite 构建也会报）— 必须用 :model-value + @update:model-value
  - emit object 形式 `{ name: [args] }` 比 tuple 形式 `(e: 'name', args): void` 类型推断更稳定，与 vue-tsc 兼容性更好
  - ECharts/图表类组件应在子组件内部管理实例，父组件只传 data + 触发事件
- **行为完全一致**：纯结构重构，不改业务逻辑/UI/交互

### P14 批 2 B3 拆分大 .vue - I-3 第 3 批（2026-06-19）

- **PR #197**（commit 9367dc7）squash merge 入 main
- **目标**：拆分剩余 4 个大 .vue（622-677 行：sales-price 677 + sales OrderListView 644 + purchase-contract 644 + purchase-price 622）
- **子代理**：I-3 第 3 批（独立派发，2 commit 含 1 CI 修复）
- **新增 17 个子组件 + 8 个 composable + 4 个工具**：
  - sales-price（5 子组件 + 2 composable + 1 工具）：SpFilter / SpTbl / SpForm / SpView / SpHistory + useSp / useSpProc + spFmts
  - sales OrderListView（3 子组件 + 2 composable + 1 工具）：OlvStat / OlvFilter / OlvTbl + useOlv / useOlvProc + olvFmts
  - purchase-contract（4 子组件 + 2 composable + 1 工具）：PcFilter / PcTbl / PcForm / PcDetail + usePc / usePcProc + pcFmts
  - purchase-price（5 子组件 + 2 composable + 1 工具）：PpFilter / PpTbl / PpForm / PpHistory / PpDetail + usePp / usePpProc + ppFmts
- **父组件重写**：
  - sales-price/index.vue：677 → 147 行（-78%）
  - sales/views/OrderListView.vue：644 → 125 行（-81%）
  - purchase-contract/index.vue：644 → 142 行（-78%）
  - purchase-price/index.vue：622 → 137 行（-78%）
  - 合计 2587 → 551 行（-79%）
- **命名规范**：
  - Sp（Sales Price）/ Olv（Order List View）/ Pc（Purchase Contract）/ Pp（Purchase Price）
  - 子组件后缀：Filter（过滤栏）/ Tbl（列表）/ Form（新建/编辑）/ Detail 或 View（详情）/ History（历史）/ Stat（统计）
- **CI 1 轮迭代修复**（I-3 第 3 批子代理全程自主修复）：
  1. **OrderForm / OrderItemForm 接口从 useOlv 函数内移至顶层**（TS2304 Cannot find name 'OrderForm'）— 文件底部的 `export type { ... }` 无法看到函数内部定义的接口
  2. **ElTag renderCell 函数 h() 调用类型错误**（TS2769 No overload matches）— 改用 `h(ElTag, props, () => text)` 函数形式 + `type: ... as any` 绕过 Element Plus 严格类型
  3. **viewData 可选字段类型不匹配**（TS2345 PcDetail / PpDetail / SpView 多处）— viewData 字段是 `number | undefined` / `string | undefined`，但 formatCurrency / getStatusType / getStatusLabel 期望非可选类型。加 `|| 0` / `|| ''` fallback
- **关键设计决策**：
  - **OlvTbl 把操作列从父组件迁移到子组件** — 子组件用 h() 渲染 4 个操作按钮（查看 / 审批 / 发货 / 取消），按状态条件渲染；父组件只传 columns + data
  - **Olv 分页 hook 到 useTableApi 自动 watch** — handlePageChange / handleSizeChange 内部直接修改 page.value / pageSize.value，触发 useTableApi 内部 watch [page, pageSize] 自动 reload
  - **sp/olv/pc/pp 各 4 个 composables 都用 reactive 包装 return** — 严格沿用 I-3 第 1/2 批模式
- **经验沉淀**：
  - 文件底部 `export type` 只能看到顶层接口；接口必须在文件顶层定义才能被 export
  - Element Plus 组件 h() 渲染时 `type` 字段是严格枚举，string 字面量需用 `as any` 或在 fn signature 处用 `keyof typeof map`
  - 子组件 prop 类型如果定义为可选字段，必须在 viewData 使用时加 fallback（`field || defaultValue`），否则 TS2345 不可赋
  - V2Table 包装子组件接收 columns prop 时，把操作列作为额外列加在末尾（fullColumns = [...props.columns, actionsColumn]）— 父组件保持简洁
- **行为完全一致**：纯结构重构，不改业务逻辑/UI/交互

### P14 批 2 B3 拆分大 .vue - I-3 第 4 批（2026-06-19）

- **PR #198**（commit 0bc1f5e）squash merge 入 main
- **目标**：拆分剩余 4 个 598-618 行大 .vue（bpm/approval 618 + production 611 + logistics 605 + purchaseReceipt 598）
- **子代理**：I-3 第 4 批（独立派发，6 commit 含 2 CI 修复）
- **新增 20 个子组件 + 8 个 composable + 4 个工具**：
  - bpm/approval（6 子组件 + 2 composable + 1 工具）：BpmApStat / BpmApPendingTbl / BpmApCompletedTbl / BpmApAprDlg / BpmApTranDlg / BpmApChainDlg + useBpmAp / useBpmApProc + bpmApFmts
  - production（4 子组件 + 2 composable + 1 工具）：PrdFilter / PrdTbl / PrdForm / PrdDetail + usePrd / usePrdProc + prdFmts
  - logistics（6 子组件 + 2 composable + 1 工具）：LgsStat / LgsFilter / LgsTbl / LgsForm / LgsDetail / LgsStatDlg + useLgs / useLgsProc + lgsFmts
  - purchaseReceipt（4 子组件 + 2 composable + 1 工具）：PrcFilter / PrcTbl / PrcForm / PrcDetail + usePrc / usePrcProc + prcFmts
- **父组件重写**：
  - bpm/approval.vue → bpm/approval/index.vue：618 → 123 行（-80%）
  - production/index.vue：611 → 172 行（-72%）
  - logistics/index.vue：605 → 117 行（-81%）
  - purchaseReceipt/index.vue：598 → 97 行（-84%）
  - 合计 2432 → 509 行（-79%）
- **命名规范**：
  - BpmAp（BPM Approval）/ Prd（Production）/ Lgs（Logistics）/ Prc（Purchase Receipt）
  - 子组件后缀：Filter（过滤栏）/ Tbl（列表）/ Form（新建/编辑）/ Detail（详情）/ Stat（统计）/ Dlg（对话框）
- **CI 2 轮迭代修复**（I-3 第 4 批子代理全程自主修复）：
  1. **useXxx 返回值需用 `reactive({...})` 包装**（TS2322 'Ref<X>' is not assignable to X）— 父组件模板访问 `lgs.dateRange` 等字段时不会自动解包 ref（因为 lgs 是 useXxx 返回的对象，不是顶层 ref），必须用 reactive 包装才能让 `lgs.X` 在模板中自动解包；useXxxProc 的 callbacks 用 plain 类型（reactive 已在 useXxx 内层解包）
  2. **LgsFilter dateRange 是 readonly prop**（TS2540 Cannot assign to 'dateRange'）— 改用 emit 事件模式 `@update:model-value="emit('date-change', $event)"`，父组件监听后更新 `lgs.dateRange`
  3. **`const props = defineProps<...>()` 未使用**（TS6133 'props' is declared but its value is never read）— 子组件若不需访问 props 内部，移除 `const props =` 直接 `defineProps<{...}>()`；或加 `void props` 显式标记
  4. **PrdForm watch 未使用**（TS6133 'watch' is declared but its value is never read）— 移除 watch import
  5. **PrcForm `const props` + Reactive 未用**（TS6133）— 移除
  6. **usePrc 移除 getPurchaseReceipt / getReceiptItems / reactive / formRef**（只保留 loadData 用到的 listPurchaseReceipts）
  7. **usePrdProc `data: ProductionOrder[]` 而不是 Ref**（cb 类型用 plain）— 与 LgsProc / PrcProc 保持一致
  8. **usePrcProc API 返回值类型从 ApiResponse 改 .data?.items**（TS2352 Conversion of type 'ApiResponse<ReceiptItemsResponse>' to type '{ data: ReceiptItem[]; }' may be a mistake）— `res.data?.items` 实际类型是 ReceiptItem[]
  9. **PrdPartial<ProductionOrder> cast**（TS2345 OrderForm 字段不全）— `createProductionOrder(prd.orderForm as Partial<ProductionOrder>)`
- **关键设计决策**：
  - **useXxx 必须用 reactive 包装 return** — 沿用 I-3 第 1/2/3 批模式；父组件模板能自动解包 ref；父组件 script 中 `lgs.X = newVal` 会修改 reactive 包装层（reactive 会更新底层 ref）
  - **useXxxProc callbacks 用 plain 类型** — 因 reactive 已解包，cb.isEdit 是 boolean（不是 Ref），cb.formData 是 plain object（不是 reactive）
  - **LgsFilter dateRange 用 emit 事件模式** — readonly prop 不可直接赋值，emit 后父组件更新自身 state
  - **BpmApTranDlg / LgsForm / PrdForm / PrcForm 表单 ref 内部管理** — form 组件内部 `const formRef = ref<FormInstance>()` + `defineProps<{...}>()`，submit 前 `formRef.value.validate()` 通过后才 emit 'submit'；父组件不需管 form ref
- **经验沉淀**：
  - 父组件用 `const prd = usePrd()` 后模板 `prd.X` 自动解包，**前提是 useXxx 用 reactive({...}) 包装 return**
  - vue/no-mutating-props：每个子组件 `<template>` 顶部加 `<!-- eslint-disable vue/no-mutating-props -->` 是稳妥的批量解法
  - readonly prop 不可写：要么 emit 事件让父组件更新，要么用 :model-value 双向但不能直接 v-model
  - v-model on prop 仍然报错：必须 `:model-value` + `@update:model-value` + emit 分离
  - composable 中类型断言 `as Partial<T>` 用于简化 PrdOrderForm vs Partial<ProductionOrder> 字段差异
- **行为完全一致**：纯结构重构，不改业务逻辑/UI/交互

### P14 批 2 B3 拆分大 .vue - I-3 第 5 批（2026-06-19）

- **PR #199**（commit 6894923）squash merge 入 main
- **目标**：拆分剩余 4 个 579-596 行大 .vue（data-import 596 + purchase-inspection 594 + material-shortage 590 + bpm/definitions 579）
- **子代理**：I-3 第 5 批（独立派发，6 commit 含 2 CI 修复）
- **新增 17 个子组件 + 8 个 composable + 4 个工具**：
  - data-import（4 子组件 + 2 composable + 1 工具）：DiTplTbl / DiTaskTbl / DiTplForm / DiTplUpload + useDi / useDiProc + diFmts
  - purchase-inspection（5 子组件 + 2 composable + 1 工具）：PiStat / PiFilter / PiTbl / PiForm / PiDetail + usePi / usePiProc + piFmts
  - material-shortage（3 子组件 + 2 composable + 1 工具）：MsStat / MsSevCard / MsTbl + useMs / useMsProc + msFmts
  - bpm/definitions（5 子组件 + 2 composable + 1 工具）：BpmDfFilter / BpmDfTbl / BpmDfForm / BpmDfVerDlg / BpmDfTplDlg + useBpmDf / useBpmDfProc + bpmDfFmts
- **父组件重写**：
  - data-import/index.vue：596 → 127 行（-79%）
  - purchase-inspection/index.vue：594 → 113 行（-81%）
  - material-shortage/index.vue：590 → 85 行（-86%）
  - bpm/definitions.vue：579 → 150 行（-74%）
  - 合计 2359 → 475 行（-80%）
- **命名规范**：
  - Di（Data Import，注意与 Dashboard `Db` 区分）/ Pi（Purchase Inspection）/ Ms（Material Shortage）/ BpmDf（BPM Definitions，注意与 I-3 第 4 批的 BpmAp Approval 区分）
  - 子组件后缀：Tbl（列表）/ Form（新建/编辑）/ Detail（详情）/ Stat（统计）/ Filter（过滤栏）/ Dlg（对话框）/ TplTbl（模板表）/ TaskTbl（任务表）/ SevCard（严重程度卡片）/ VerDlg（版本对话框）/ TplDlg（模板对话框）/ Upload（上传对话框）
- **CI 3 轮迭代修复**（I-3 第 5 批子代理全程自主修复）：
  1. **bpm/definitions.vue import 路径错误**（rollup `Could not resolve "./composables/useBpmDfProc"`）— definitions.vue 在 bpm/ 目录（路由直接引用），需要在 import 路径加 `./definitions/` 前缀：`./definitions/composables/useBpmDf`
  2. **ref 不能从 element-plus 导入**（rollup `"ref" is not exported by "element-plus/es/index.mjs"`）— BpmDfForm / BpmDfTplDlg / PiForm 都误把 `ref` 写在 `import { ref, type FormInstance, type FormRules } from 'element-plus'` 中，应分开为 `import { ref } from 'vue'` + `import { type FormInstance, type FormRules } from 'element-plus'`
  3. **bpm/definitions.vue `type FormRules` 应从 element-plus 导入**（TS2305 Module '"vue"' has no exported member 'FormRules'）— `import { onMounted, reactive } from 'vue'` + `import { type FormRules } from 'element-plus'`
  4. **MsTbl string 类型 prop readonly**（TS2540 Cannot assign to 'filterSeverity' / 'filterStatus'）— `filterSeverity: string` 等基本类型 prop 不可写，改用 emit 模式 `@update:model-value="(v: string) => emit('update:filter-severity', v)"`，父组件监听 `@update:filter-severity="(v: string) => (ms.filterSeverity = v)"`
  5. **usePiProc formData.items 类型不匹配**（TS2322 items: never[] vs Partial<PurchaseInspectionItem>[]）— usePiProc 的 `formData.items: never[]` 应改为 `Partial<PurchaseInspectionItem>[]`，并 import `type PurchaseInspectionItem`
  6. **MsTbl el-table v-loading 引用旧 prop 名字**（TS2339 Property 'tableLoading' does not exist）— prop 改名为 `loading` 后模板内 `v-loading="tableLoading"` 也要改 `v-loading="loading"`
- **关键设计决策**：
  - **bpm/definitions.vue 路由直接引用 `bpm/definitions.vue`**（而非 `bpm/definitions/index.vue`）— 与 I-3 第 4 批的 `bpm/approval/index.vue` 不同；保持路由配置不变，composables/components 放在 `bpm/definitions/composables/...` 和 `bpm/definitions/components/...` 子目录
  - **BpmDf 前缀** — 与 I-3 第 4 批的 BpmAp（Approval）区分（同 BPM 域不同子业务）；保持命名规范
  - **string prop readonly 处理** — 同 LgsFilter dateRange readonly 模式（I-3 第 4 批经验）：emit 事件让父组件更新 state
  - **ref 必须从 vue 导入** — type { FormInstance, FormRules } 可以从 element-plus 导入（运行时不会触发 rollup tree-shake 警告）
- **经验沉淀**：
  - 路由直接引用的 .vue（如 bpm/definitions.vue）应在原目录，composables/components 放子目录
  - 子组件 prop 类型为 string/number/boolean 时是 readonly，必须用 emit 模式（不能直接赋值）
  - 子组件 prop 类型为 object/interface 时内部字段可变，可直接赋值（vue/no-mutating-props 用 eslint-disable 注释屏蔽）
  - ref 必须从 vue 导入，element-plus 只导出组件 + 类型
  - 拆分前先确认路由配置（是引用 `bpm/approval/index.vue` 还是 `bpm/approval.vue`）决定子目录结构
- **行为完全一致**：纯结构重构，不改业务逻辑/UI/交互
- **累计进展**：P14 批 2 I-3 全部 5/5 批完成（PR #195/#196/#197/#198/#199），18/24 大 .vue 已拆分
