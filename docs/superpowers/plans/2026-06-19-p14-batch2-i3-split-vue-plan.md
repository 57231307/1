...
### 8.7 进度
- ✅ I-3 第 5 批（PR #199）squash merge 入 main（6894923）

---

## 九、I-3 第 6 批（最终批）：拆分剩余 5 个大 .vue（535-562 行，2026-06-19）

### 9.1 背景
- I-3 第 1/2/3/4/5 批已合并（PR #195/196/197/198/199），累计拆分 18/24 大 .vue
- 本批为 I-3 最终批，拆分剩余 5 个 535-562 行大 .vue：capacity 562 + Dashboard 549 + security 547 + TwoFactorSetup 540 + sales-analysis 535
- 沿用 I-3 第 1/2/3/4/5 批全部经验
- 拆分后剩余约 5-6 个 >500 行的 .vue 单独启动 P14 批 3 处理

### 9.2 拆分目标

| 文件 | 当前行数 | 路径 | 业务领域 | 命名前缀 |
|------|---------|------|---------|---------|
| `frontend/src/views/capacity/index.vue` | 562 | 产能管理 | 产能（capacity） | `Cp`（Capacity） |
| `frontend/src/views/Dashboard.vue` | 549 | 仪表板 | 仪表板（Dashboard） | `Db`（Dashboard） |
| `frontend/src/views/security/index.vue` | 547 | 安全中心 | 安全（security） | `Sec`（Security） |
| `frontend/src/views/security/TwoFactorSetup.vue` | 540 | 双因素认证 | 安全（security 2FA） | `Tfa`（Two Factor Auth） |
| `frontend/src/views/sales-analysis/index.vue` | 535 | 销售分析 | 销售（sales-analysis） | `Sa`（Sales Analysis） |

### 9.3 拆分模式（沿用 I-3 第 1-5 批）

按 I-3 第 5 批 `bpm/definitions.vue` 模式：
- 父文件（路由直接引用）保留在原路径
- 新建子目录 `<basename>/` 存放 components + composables
- 父文件用相对路径 `./<basename>/components/...` 和 `./<basename>/composables/...`
- 子组件/工具使用 `../../api/...` 回到 `views/`

```
父文件 (535-562 → ~150 行)
├── <basename>/                    # 子目录（路由直接引用的父文件用此模式）
│   ├── composables/
│   │   ├── useXxxXxx.ts          # 主业务 composable（reactive 包装 return）
│   │   ├── useXxxProc.ts         # 业务流程 composable
│   │   └── xxxFmts.ts            # 格式化工具
...
### 9.4 命名规划
- **capacity 域**（`Cp` 前缀，注意区分 Pcr 采购收货）：
  - 子组件：`CpStat`（4 统计卡片：工作中心总数/正常/繁忙/超负荷） / `CpTrend`（产能负荷趋势 ECharts 图） / `CpTbl`（工作中心列表 + 负荷率/状态/瓶颈） / `CpBottleneck`（瓶颈识别侧栏）
  - composables：`useCp`（主业务：summary/trend/workCenters/bottlenecks + ECharts 实例） / `cpFmts`（getStatusType / getStatusLabel / getLoadRateType）
- **Dashboard 域**（`Db` 前缀，注意区分 Di 数据导入）：
  - 子组件：`DbStat`（8 统计卡片，分 2 行：面料/库存/订单/客户 + 今日/待处理/预警/销售） / `DbTrend`（销售趋势 ECharts 折线柱状图） / `DbPie`（库存分布 ECharts 饼图） / `DbActTbl`（最新活动表格）
  - composables：`useDb`（主业务：overview + 2 图表数据 + ECharts 实例） / `dbFmts`（formatNumber / formatCurrency / getActivityTypeColor）
- **security index 域**（`Sec` 前缀）：
  - 子组件：`SecStat`（4 统计卡片：今日登录/失败/锁定/告警） / `SecLogTbl`（登录日志表 + 过滤栏 + 分页） / `SecLockTbl`（锁定账户表 + 解锁操作） / `SecAlertTbl`（安全告警表 + 类型/状态标签）
  - composables：`useSec`（主业务：stats/loginLogs/lockedAccounts/alerts + 过滤/分页） / `useSecProc`（解锁/导出流程） / `secFmts`（getTypeLabel / getStatusType / getStatusLabel / getAlertType / getAlertLabel / getAlertStatusType / getAlertStatusLabel）
- **security TwoFactorSetup 域**（`Tfa` 前缀）：
  - 子组件：`TfaStepBar`（步骤条） / `TfaStep1`（启动设置面板：状态/账户名/启用按钮） / `TfaStep2`（扫描二维码面板：QR 码/账户/发行方/手动密钥/复制） / `TfaStep3`（验证并启用面板：6 位令牌输入/表单验证/错误提示） / `TfaStep4`（完成面板：成功结果/10 个恢复码/复制全部）
  - composables：`useTfa`（主业务：currentStep/qrCodeDataUrl/secretText/verifyForm/recoveryCodes + computed isEnabled/username） / `useTfaProc`（handleStartSetup/handlePrevStep/handleNextStep/handleCopySecret/handleVerifyAndEnable/generateRecoveryCodes/handleCopyRecovery/handleFinish） / `tfaFmts`（validateToken/verifyRules）
- **sales-analysis 域**（`Sa` 前缀）：
  - 子组件：`SaStat`（4 统计卡片：本月订单/销售/毛利率/客户 + 趋势百分比） / `SaTrend`（销售趋势 + 周期切换 + 销售构成饼图占位） / `SaProdRank`（产品销售排名表 + 金额/数量切换） / `SaCustRank`（客户销售排名表 + 金额/订单数切换） / `SaTarget`（销售目标表 + 完成率/差异/状态标签）
  - composables：`useSa`（主业务：stats/trendPeriod/productRankType/customerRankType + 4 个列表） / `useSaProc`（getStats/getProductRanking/getCustomerRanking/getSalesTargets/handleEditTarget/handleExport） / `saFmts`（formatCurrency / getProgressColor / getTargetStatusType / getTargetStatusLabel）

### 9.5 关键 CI 经验（沿用 I-3 第 1-5 批）
1. **JSDoc 中文注释在 TS 泛型内解析失败**：不要在 `defineProps<{...}>` 上方写 `/** 中文 JSDoc */`，改用 `// 中文行注释`
2. **TS2540 readonly 错误**：改 v-model + emit 模式（prop 类型从 `{ value: string }` 改为 `string`），父组件 `v-model:foo="ref.value"`
3. **vue/no-mutating-props ESLint**：每个子组件 `<template>` 顶部加 `<!-- eslint-disable vue/no-mutating-props -->`
4. **v-model 不能用于 prop**：用 `:model-value` + `@update:model-value` + emit
...
9. **TS2769 ElTag renderCell No overload matches**：改 `h(ElTag, props, () => text)` + `type: ... as any`
10. **TS2345 viewData optional 字段**：加 `|| 0` / `|| ''` fallback
11. **import 路径与实际不符**：路由直接引用的 .vue（`bpm/definitions.vue`）其子目录需加前缀
12. **ref 不能从 element-plus 导入**：必须从 'vue' 导入
13. **type FormRules 应从 element-plus 导入**，不要写在 'vue' 后面
14. **路由直接引用的 .vue 模式**：I-3 第 5 批 `bpm/definitions.vue` 方案：父文件保留原路径 + 同名子目录 `bpm/definitions/` 放 components + composables
15. **echarts.dispose 必须在 onBeforeUnmount 中调用**（避免内存泄漏）：capacity/Dashboard 都需要
16. **echarts resize 监听器**：`window.addEventListener('resize', handleResize)` + `onBeforeUnmount` 移除
...
- [ ] composable 用 reactive 包装
- [ ] 无未使用的 props / import
- [ ] 路由引用文件（Dashboard.vue / TwoFactorSetup.vue）的子目录 import 路径正确

### 9.7 I-3 全部 6/6 批完成总览

| 批次 | PR | 拆分文件 | 行数 before → after | 累计 |
|------|-----|---------|---------------------|------|
| I-3 第 1 批 | #195 | VoucherListTab / system-update / sales-contract | 870→141 + 725→154 + 717→129 = 2312→424 | 3 文件 |
| I-3 第 2 批 | #196 | purchase-return / scheduling/gantt / scheduling/index | 695→211 + 691→93 + 689→109 = 2075→413 | 6 文件 |
| I-3 第 3 批 | #197 | sales-price / OrderListView / purchase-contract / purchase-price | 677→147 + 644→125 + 644→142 + 622→137 = 2587→551 | 10 文件 |
| I-3 第 4 批 | #198 | bpm/approval / production / logistics / purchaseReceipt | 618→123 + 611→172 + 605→117 + 598→97 = 2432→509 | 14 文件 |
| I-3 第 5 批 | #199 | data-import / purchase-inspection / material-shortage / bpm/definitions | 596→127 + 594→113 + 590→85 + 579→150 = 2359→475 | 18 文件 |
| I-3 第 6 批（本批） | TBD | capacity / Dashboard / security / TwoFactorSetup / sales-analysis | 562+549+547+540+535 = 2733→~750 | 23 文件 |

**累计 I-3 6/6 批完成**：23 个大 .vue 拆分，**总行数从 ~14498 → ~3122（-78%）**，新增 ~75 子组件 + ~30 composable + ~6 工具文件

**剩余大 .vue 候选**（>500 行）将单独启动 P14 批 3 处理

### 9.8 进度
- ⏳ 进行中：子代理 I-3 第 6 批执行中（2026-06-19）
