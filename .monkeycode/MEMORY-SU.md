...
> 与规则 4（`///` 注释精简为 1-2 行）+ 规则 20（注释与功能一致性强制）联动。完整强制要求见对应规则。
> **通用原则**：注释解释"为什么"而非"做什么"；公共 API 必须有文档注释；保持文档与代码同步更新；所有 API 接口必须有文档说明（路径/参数/响应/示例）。
...
> 与规则 11（法律合规标准）+ 规则 12（法律安全标准）联动。完整强制要求见对应规则。
...
> 规则来源分类表（22 条规则的 PR/IR 分类 + 来源上下文 + 迭代历史）已合并到 [MEMORY.md §一规则索引表](file:///workspace/.monkeycode/MEMORY.md#一关键项目规则必读按优先级排序) 的"分类"列 + "详细说明"链接。
> 2026-07-22 合并整理：避免与 MEMORY.md 索引表重复维护，统一以 MEMORY.md 索引表为权威。
