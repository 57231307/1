...
- 每批次修复 65-99 个文件（2026-07-31 五次迭代更新，原"10-15 个文件"已升级为 65-99 个，用户明确要求大幅提升单批吞吐量；详见 6.1 批次大小偏好）
- 批次编号连续递增（不跳号）
...
### 规则 21：PR 描述强制依据模板填写（PR，2026-07-29 追加）

> **触发条件**：用户 2026-07-29 明确指令"PR 更新时需要依据 `/.github/PULL_REQUEST_TEMPLATE.md` 文件里面的格式进行 PR 填写"。
> **优先级**：🔴 高（PR 合并前置条件，与规则 13/14/20 同级）。
> **关联规则**：规则 13（修复流程自动化）/ 规则 14（警告零容忍）/ 规则 20（注释一致性）。

#### 1. 核心原则

**所有 PR 创建/更新时，PR 描述必须严格依据 `/.github/PULL_REQUEST_TEMPLATE.md` 模板填写**，禁止以下行为：

- ❌ 跳过模板章节，仅提供一句话描述
- ❌ 删除模板章节标题，自创章节结构
- ❌ 留空检查清单（Checklist）字段（必须逐项勾选 `[x]` 或说明"不适用"）
- ❌ 描述内容与实际改动不符（违反规则 20 注释一致性）
- ❌ 仅复制 commit message，不按模板填写

#### 2. 模板章节清单

依据 [PULL_REQUEST_TEMPLATE.md](file:///workspace/.github/PULL_REQUEST_TEMPLATE.md)，所有 PR 描述必须包含以下章节（即使标注"不适用"也必须保留）：

| 章节 | 必填 | 说明 |
|------|------|------|
| **## 描述** | ✅ | PR 的更改内容和目的 |
| **## 相关 Issue** | ✅ | `Closes #xxx` 或 `不适用` |
| **## 更改类型** | ✅ | 8 类勾选至少 1 项（feat/fix/refactor/docs/style/test/chore/perf） |
| **## 检查清单** | ✅ | 前端/后端/文档三组按实际情况勾选 |
| **## 测试说明** | ✅ | 测试步骤（至少 1 步）+ 测试结果（或截图） |
| **## 截图** | ⚪ | UI 更改必填，非 UI 可标"不适用" |
| **## 额外说明** | ⚪ | 可选，审查者需知信息 |
...
> **执行细则**：
> - 若 PR 仅修改文档（无代码逻辑），后端/前端检查清单可整组标注"不适用（仅文档变更）"
> - 若 PR 仅修改后端，前端检查清单可整组标注"不适用（仅后端变更）"
> - 但**禁止删除清单章节**，必须保留章节并明确标注

#### 4. 执行机制

- **AI 创建 PR 时强制执行**：所有由 AI 助手创建的 PR 必须按模板填写
- **模板自动加载**：GitHub 已配置 `PULL_REQUEST_TEMPLATE.md`，新建 PR 时自动加载模板内容
- **违规处理**：发现 PR 描述未按模板填写时，必须立即更新 PR 描述以符合模板要求
- **合并前置**：PR 描述符合模板是合并的必要条件（与 CI 检查同级）

#### 5. 适用范围

- 所有目标分支为 `main` 的 PR
- 所有目标分支为 `release/*` 的 PR
- 所有由 AI 助手创建的 PR（包括文档类 PR）
- 不适用：draft PR（但标记 Ready for Review 前必须补齐模板）

---

### 规则 22：每次 CI 推送必须完成至少 8 个任务（PR，2026-08-08 追加）

> **触发条件**：用户 2026-08-08 明确指令"每个ci必须修复完成8个任务才能进行ci推送，除非用户特批"。
> **优先级**：🔴 高（CI 推送前置条件）。
> **关联规则**：规则 13（修复流程自动化）。

#### 1. 核心原则

**每次推送到 CI 之前，必须完成至少 8 个任务**，除非用户明确特批。

#### 2. 强制执行要求

- **任务计数**：每个 PR 必包含至少 8 个已完成的任务（修复/功能/重构）
- **任务来源**：任务来自 doto.md 中的 P2/P3 任务清单
- **用户特批**：如需在不足 8 个任务时推送，必须获得用户明确批准
- **CI 失败处理**：CI 失败后修复不计入新任务，需补充新任务才能再次推送

#### 3. 例外情况

- **紧急修复**：生产环境紧急问题可单独推送（需用户特批）
- **规则变更**：规则本身的修改可单独推送
- **文档更新**：纯文档更新可单独推送

#### 4. 违规处理

- 未完成 8 个任务即推送 CI，视为违反规则
- 需在下次推送时补充不足的任务数
...
> 原 [project_rules.md](file:///workspace/.trae/rules/project_rules.md) 已删除（2026-07-17），内容已合并到本节。
> 2026-07-22 优化：22 个零散小节合并为 12 个功能聚类，消除空泛通用原则，保留项目特色内容。

### 沟通与协作

- **语言**：中文回复，简洁高效，直接给方案和结果
- **进度可见**：使用 TodoWrite 跟踪，每完成一项立即标记
- **错误透明**：遇到失败立即报告，不掩盖
- **任务管理**：使用中文建立待办任务（doto.md），每完成一项立即标记"已完成"
- **代码审查维度**：代码质量、安全性、性能、测试覆盖（与规则 00/14/20 联动）

### 编码规范

- **命名原则**：使用有意义的、描述性的名称，遵循项目或语言的命名规范；避免缩写和单字母变量（除循环计数器等约定俗成外）
- **代码组织**：相关代码放在一起，保持适当的抽象层次；函数只做一件事，保持单一职责原则
- **硬编码禁止**：所有文本需使用中文，禁止硬编码
- **注释语言**：代码注释必须使用中文

### 注释与文档

> 与规则 4（`///` 注释精简为 1-2 行）+ 规则 20（注释与功能一致性强制）联动。
- **通用原则**：注释解释"为什么"而非"做什么"；公共 API 必须有文档注释；所有 API 接口必须有文档说明（路径/参数/响应/示例）

### 死代码处理

> 与规则 14 联动：禁止警告抑制。完整强制要求 + 处理流程 + TODO 注释模板 + CI 强制见 [规则 14](#规则-14移除所有警告抑制所有警告视为错误pr2026-07-12-追加)。
- **例外**：[backend/src/models/](file:///workspace/backend/src/models/) 下的 SeaORM 自动生成模型可保留文件级 `#![allow(dead_code)]`（模型字段由派生宏使用）
- **通用原则**：不使用的文件/代码/文件夹必须删除（删除前评估影响范围），修改文件后保存前必须交叉自审

### 安全规范

> 与规则 11（法律合规标准）+ 规则 12（法律安全标准）联动。
- **通用原则**：敏感信息禁止硬编码（使用环境变量/配置管理），所有用户输入必须验证清理（防 SQL 注入/XSS/CSRF）

### 工程规范

> 合并自原 project_rules.md（版本控制/性能/错误处理/测试/持续改进），与规则 6 联动。

- **版本控制**：提交信息中文描述"做了什么"和"为什么"；功能开发用功能分支，修复 bug 用修复分支；所有代码变更需经审查
- **性能**：避免 N+1 查询，使用适当索引，大数据量分页处理；合理使用缓存，明确失效策略；及时释放资源，避免内存泄漏
- **错误处理**：所有可能失败的操作都需错误处理；业务错误返回友好提示，系统错误记录详细日志返回通用信息，验证错误明确指出失败原因；实现优雅降级和重试机制
- **测试**：新增功能必须编写单元测试，修改功能需更新测试；单元测试测函数功能，集成测试测模块交互，E2E 测完整用户流程；测试函数名清晰描述场景，使用中文描述测试目的
- **持续改进**：定期审查代码质量，及时重构低质量代码；记录技术债务并制定偿还计划

### 面料行业业务术语（用户 2026-07-17 明确澄清）

- **缸号（batch_no）= 染色批次号**：同一概念，指一次染色生产的一个缸的批次号
- **染色批号（dye_lot_no）**：面料行业 lot 概念，指同产品同颜色不同时间/染缸的批次标识，用于库存/发货/追溯防色差混批
- **四维标识**：product_id + color_no + dye_lot_no + batch_no（面料追溯核心）
- **术语使用规范**：禁止将 dye_lot_no 译为"染色批次号"（与 batch_no 混淆），统一用"染色批号"；"缸号"和"染色批次号"可互换指代 batch_no
...
- **项目名称**：统一以 main 仓库 README 为准，所有文档/界面/输出信息一致
- **数据库**：PostgreSQL，远程数据库连接模式

### 记忆管理

- 实时查看和更新 `MEMORY.md` 规则记忆文档
- **文件职责分工**：详见 [规则 10](#规则-10记忆文件定期整理归档--实时归档pr2026-07-08-追加2026-07-10-修正2026-07-14-二次修正) 的文件分工表（MEMORY.md 索引 / MEMORY-SU.md 详细 / doto.md 未完成任务 / doto-su.md 已完成归档 / CHANGELOG.md 一句话总结 / audit_assignment.md 审计任务分配）
- **路径策略（2026-06-19 确认）**：test 分支合并入 main 时 `-X theirs` 会覆盖 `.monkeycode/`，必须以 main 版本为准

### 打包与发布

打包时必须进行全面的项目测试（功能测试 + 兼容性测试 + 稳定性测试），确保打包后的程序能够在目标环境中正常启动并完整运行所有功能模块，无运行时错误或功能缺失。
...
### Rust 集成测试模块名称遮蔽（D10-3b 教训，批次 490）
- **现象**：`tests/xxx_test.rs` 中 `mod common;` 声明本地模块后，`mod tests` 内 `use crate::models::status::{common, ...}` 导入的 `common` 会**遮蔽**本地 `mod common`
- **后果**：`use common::setup_test_db;` 解析到 `status::common`（不含 `setup_test_db`）→ E0432 unresolved import
- **修复**：改用 `use super::common::setup_test_db;` 通过 `super::` 显式访问外层本地模块
- **适用场景**：集成测试文件中本地 `mod common;` 与 `use ...::status::common` 共存时
- **影响文件**：tests/ap_payment_workflow_test.rs + tests/production_order_workflow_test.rs（purchase_receipt 无此问题，未导入 status::common）
...
| 批次大小偏好 | 3+ 次对话中反复要求"每次最少修改 6-8 文件，2-3 个文件太少了"（V15 修复阶段 Batch 455-457 合并批次、Batch 458/459 7-9 文件批次均源于此偏好）；2026-07-17 三次迭代升级为"每批 9-12 文件"，批次总数从 27 压缩为 22；2026-07-23 四次迭代升级为"每批 10-15 文件"；2026-07-31 五次迭代大幅升级为"每批 65-99 文件"（用户明确要求） | 用户偏好单批次覆盖 65-99 个文件，避免过小批次导致上下文切换开销过大；超大批次可拆分微批次（参考 Batch 475c/475d/475e） | 每批规划 65-99 文件，跨域合并可接受；批次表见 doto.md §3 |
...
> 每次规则迭代的摘要索引，按时间倒序排列。
> 2026-07-22 整理：5 条详细迭代记录归档到 [archives/2026-07-22/memory-su-iteration-history.md](file:///workspace/.monkeycode/docs/archives/2026-07-22/memory-su-iteration-history.md)。
> 更旧记录（规则 16/17/18/19 新增、规则 15 精简、规则 8 合并、第六节精简、规则 13 批次文件数修正等 8 条）已归档到 [doto-su.md](file:///workspace/.monkeycode/doto-su.md)。

| 迭代次数 | 日期 | 规则 | 触发条件 | 摘要 |
|----------|------|------|----------|------|
| 第 12 次 | 2026-07-31 | 规则 13 | 用户明确指令"修改规则13里面的文件数量从13个改为65~99" | 批次文件数 10-15 → 65-99（大幅提升单批吞吐量）；同步更新 §6.1 批次大小偏好 + audit_assignment.md §2.2 流程引用 |
| 第 11 次 | 2026-07-29 | 规则 21 | 用户明确指令"PR 更新时需要依据 `/.github/PULL_REQUEST_TEMPLATE.md` 文件里面的格式进行 PR 填写" | 新增规则 21：PR 描述强制依据模板填写（5 章节必填 + 3 组检查清单 + 执行机制 + 合并前置） |
...
| 第 6 次 | 2026-07-17 | 规则 20 | 用户明确指令 | 新增规则 20：注释与功能一致性强制（6 类禁止行为 + CI 强制检查） |
| 第 5 次 | 2026-07-17 | 术语约定 | 用户明确指令 | 新增面料行业业务术语：缸号（batch_no）= 染色批次号，dye_lot_no = 染色批号 |
...
规则迭代历史归档：
- 2026-07-22 整理：5 条详细迭代记录（规则 13 步骤 0/步骤 4/规则 20/术语约定/MEMORY-SU 梳理）归档到 [archives/2026-07-22/memory-su-iteration-history.md](file:///workspace/.monkeycode/docs/archives/2026-07-22/memory-su-iteration-history.md)
- MEMORY-SU.md 6.4 更旧迭代记录（规则 16/17/18/19 新增、规则 15 精简、规则 8 合并、第六节精简、规则 13 批次文件数修正等 8 条）：已归档到 [doto-su.md](file:///workspace/.monkeycode/doto-su.md) 的"规则迭代历史归档"节
