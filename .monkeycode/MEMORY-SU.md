...
### 规则 2：代码质量强制（PR，2026-07-12 追加，2026-07-17 合并规则 14/20）

> 所有警告视为错误必须修复，禁止 `#[allow(...)]` 警告抑制。注释必须与功能实现一致。
...
### 规则 18：功能变更必须同步测试代码（PR，2026-08-14 追加）

> 修改功能或功能新增/删除时，需要同步进行测试代码的变更。
...
### 规则 19：编码行为规范（PR，2026-08-14 追加）

> 编码前先思考，保持简单，精准修改，不过度推断，正确处理错误。
...
#### 19.3 精准修改
- 只改你要的，不顺手重构、不乱碰无关代码
- diff 干干净净，每行修改都有明确目的
- 禁止"顺便"修改、"顺手"优化

#### 19.4 不过度推断需求
- 用户说什么做什么，不自行脑补额外需求
- 不确定的功能点先确认，不假设用户想要什么
- 避免"用户可能还需要..."的过度设计

#### 19.5 错误处理原则
- 不堆无意义的 try-catch，只处理真会发生的异常
- 修改后错误应减少而非增多，不引入新错误
- 优先修复根本问题，而非掩盖症状

**违规判定**：
- 代码量明显超出需求复杂度 → 过度设计，要求精简
- 修改涉及无关文件/功能 → 违反精准修改原则
- 新增错误多于修复错误 → 违反错误处理原则
...
| 历史经验 | [docs/archives/](file:///workspace/.monkeycode/docs/archives/) |
| 规划文档 | [docs/](file:///workspace/.monkeycode/docs/) |
