...
- 每批次修复 10-15 个文件（2026-07-23 四次迭代更新，原"9-12 个文件"已升级为 10-15 个，用户明确要求提升单批吞吐量；详见 11.2 批次大小偏好）
- 批次编号连续递增（不跳号）
...
### 规则 21：PR 格式强制（PR，2026-07-29 追加）

> PR 创建/更新时必须依据 `/.github/PULL_REQUEST_TEMPLATE.md` 模板格式填写。

**强制执行要求**：
- 所有 PR 的描述/正文必须包含 PULL_REQUEST_TEMPLATE.md 中定义的结构化章节：
  - **描述**：PR 的更改内容和目的
  - **相关 Issue**：关联的 Issue 编号（如适用）
  - **更改类型**：勾选 feat/fix/refactor/docs/style/test/chore/perf
  - **检查清单**：前端/后端/文档检查项（如适用，按实际情况勾选）
  - **测试说明**：测试步骤和测试结果（如适用）
  - **截图**：UI 更改前后对比（如适用）
  - **额外说明**：审查者需知晓的信息（如适用）
- 禁止仅写一句话或无结构化描述的 PR
- 禁止跳过检查清单中与本次更改相关的检查项
- 创建 PR 时使用 `gh pr create --body` 传入符合模板格式的正文

**违规示例**：
- ❌ `gh pr create --title "fix: 修复bug" --body "修复了bug"` — 无结构化章节
- ❌ PR 正文仅一句话，无检查清单

**合规示例**：
- ✅ PR 正文包含描述/更改类型/检查清单/测试说明等模板章节
...
> **优先级**：🔴 高（与规则 0/13/20 同级，所有 PR 强制执行）。
> **关联规则**：规则 13（修复流程自动化）/ 规则 0（真实实现强制）/ 规则 20（注释与功能一致性）。
> **依据文件**：[/.github/PULL_REQUEST_TEMPLATE.md](file:///workspace/.github/PULL_REQUEST_TEMPLATE.md)

#### 1. 核心原则

**所有 PR 创建/更新时必须依据 `/.github/PULL_REQUEST_TEMPLATE.md` 模板格式填写描述/正文**，禁止以下行为：
- ❌ 仅写一句话描述（如"修复 bug"）即提交 PR
- ❌ 无结构化描述，只粘贴 commit message
- ❌ 跳过检查清单中与本次更改相关的检查项
- ❌ 使用非模板格式的自定义 PR 描述

#### 2. 模板必填章节

PR 描述必须包含 `PULL_REQUEST_TEMPLATE.md` 中定义的结构化章节：

| 章节 | 必填 | 说明 |
|------|------|------|
| **描述** | ✅ 是 | PR 的更改内容和目的，清晰说明做了什么、为什么做 |
| **相关 Issue** | 如适用 | 关联的 Issue 编号（`Closes #xxx`） |
| **更改类型** | ✅ 是 | 勾选 feat/fix/refactor/docs/style/test/chore/perf 之一 |
| **检查清单** | ✅ 是 | 前端/后端/文档检查项，按本次更改实际情况勾选 |
| **测试说明** | 如适用 | 测试步骤和测试结果 |
| **截图** | 如适用 | UI 更改前后对比（仅 UI 改动时） |
| **额外说明** | 如适用 | 审查者需知晓的信息（如已知限制、迁移步骤等） |
...
修复 `list_products` 和 `execute_custom_report` 中的分页占位符（规则 0 违规），
移除 7 个文件中的 `#[allow(unused_imports)]` 警告抑制（规则 14 违规）。

## 相关 Issue

Closes #legacy-p1-batch-1
...
1. `cd backend && cargo clippy -- -D warnings`
2. `cd backend && cargo test`
...
# ❌ 违规示例 2：仅 commit message
fix: pagination placeholder + unused_imports cleanup
...
- **创建 PR**：使用 `gh pr create --body` 传入符合模板格式的正文，或创建分支后让 GitHub 自动加载模板并填写
- **更新 PR**：修改 PR 描述时必须保持模板格式，不得删除结构化章节
- **违规处理**：发现 PR 不符合模板格式，必须补充完善后再请求审核
- **自动化提示**：GitHub 仓库已配置 `PULL_REQUEST_TEMPLATE.md`，创建 PR 时会自动加载模板

#### 6. 适用范围

- 所有目标分支为 `main`/`master` 的 PR
- 所有类型的 PR（feat/fix/refactor/docs/chore 等）
- 包含文档更新、代码修复、功能新增等所有场景
...
| 迭代次数 | 日期 | 规则 | 触发条件 | 摘要 |
|----------|------|------|----------|------|
| 第 11 次 | 2026-07-29 | 规则 21 | 用户明确指令"PR 更新时需要依据 `/.github/PULL_REQUEST_TEMPLATE.md` 文件里面的格式进行 PR 填写" | 新增规则 21：PR 格式强制，所有 PR 创建/更新必须依据模板格式填写（描述/相关 Issue/更改类型/检查清单/测试说明/截图/额外说明 7 章节） |
