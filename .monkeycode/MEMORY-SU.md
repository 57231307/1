...
> 所有预留 API/功能/占位符/路由必须真实实现，禁止 stub/placeholder。
...
- 所有"功能扩展空间"、"预留扩展点"、"未来支持"等标注的内容视为未实现功能，必须在本批次真实实现
...
### 规则 1：修改前关联影响评估强制（PR，2026-07-11 追加）

> 每次修改后、推送 CI/CD 前，必须评估关联影响。所有修改均为代码级修改。
...
### 规则 2：代码质量与编码规范（PR，2026-07-12 追加，2026-07-17 合并规则 14/20，2026-08-14 合并规则 19）

> 所有警告视为错误必须修复，禁止 `#[allow(...)]` 警告抑制。注释必须与功能实现一致。编码前先思考，保持简单，精准修改，不过度推断，正确处理错误。
...
- **注释与功能一致性**：代码注释必须与功能实现完全一致，禁止以下行为：
...
- **注释精简**：所有 `///` 文档注释必须精简为 1 行（首选），最多 2 行（复杂场景），禁止 3 行及以上的 `///` 注释块
- **CI 强制**：
...
| 原则 | 要求 |
|------|------|
| **编码前先思考** | 需求不确定时先问清，不瞎猜、不瞎改；动手前先理解代码结构和上下文；明确要改什么、改在哪里、怎么改 |
| **优先保持简单** | 能 50 行解决，绝不堆出 200 行；专治过度设计，避免不必要的抽象和复杂度；简单方案优先，复杂方案需充分论证 |
| **精准修改** | 只改你要的，不顺手重构、不乱碰无关代码；diff 干干净净，每行修改都有明确目的；禁止"顺便"修改、"顺手"优化 |
| **不过度推断需求** | 用户说什么做什么，不自行脑补额外需求；不确定的功能点先确认，不假设用户想要什么；避免"用户可能还需要..."的过度设计 |
| **错误处理原则** | 不堆无意义的 try-catch，只处理真会发生的异常；修改后错误应减少而非增多，不引入新错误；优先修复根本问题，而非掩盖症状 |
...
### 规则 3：修复流程自动化与连续执行（PR，2026-07-11 追加）

> 修复任务按批次连续执行，CI 全绿后自动开始下一批，直到所有任务完成。用户其他指令优先处理，处理完后继续修复流程。

**修复流程（每个批次必须严格按此执行）**：
0. **确定审计结果内容是否存在**（修复前置门）：开始修复前，必须先核实审计报告中描述的问题在当前代码库中是否仍然存在，避免基于过时审计结果做无效修复
   - **核实维度**：
...
4. **修复后推送前自审**（PR 前自审门）：修复完成、提交 commit 前，必须进行自审，确认以下 3 项均通过才能进入步骤 5 提交：
   ...
   - **4.3 注释一致性自审**（规则 2 落地）：复审注释是否正确表达代码含义
     - 注释描述功能 A，代码必须实现功能 A
...
- 每批次修复 65-99 个文件（用户明确要求大幅提升单批吞吐量）
- 批次编号连续递增（不跳号）
...
### 规则 4：复审严格规范（PR，2026-07-13 追加）

> 所有复审严格按照规矩进行，baseline 警告视为错误需全部修复。复审完自动开始修复，无需用户确认。
...
2. **复审完成后自动开始修复**，无需用户确认（规则 3 连续执行）；修复按优先级分批：P0 → P1 → P2 → P3
...
- 报告中发现的 E2E 失败必须结合规则 0 进行修复和评估：占位符/未接入功能导致的失败必须真实接入
...
### 规则 6：测试规范（PR，2026-07-08 追加，2026-08-14 合并规则 18）

> 测试 mock 数据禁止硬编码；功能变更必须同步测试代码。
...
- 修复现有测试时同步迁移硬编码 mock 数据到 fixtures（违反规则 0 的占位符式写法）

#### 6.2 功能变更同步测试
- **功能修改时**：修改现有功能的实现代码时，必须同步更新对应的测试用例，确保测试与功能实现保持一致
- **功能新增时**：新增功能时，必须同步编写对应的测试用例（单元测试 + 集成测试），测试覆盖率不低于现有水平
- **功能删除时**：删除功能时，必须同步删除或标记废弃对应的测试用例，避免测试引用不存在的功能
- **接口变更时**：修改函数签名、返回值类型、错误类型时，必须同步更新所有调用该函数的测试代码
- **数据模型变更时**：修改 Model 字段（新增/删除/重命名/类型变更）时，必须同步更新所有构造该 Model 的测试代码
- **状态常量变更时**：修改状态机常量（新增/删除/值变更）时，必须同步更新所有引用该常量的测试断言

**验证方式**：
- 每次功能变更后，运行 `cargo test` 确保所有测试通过
- CI 流水线中集成测试失败视为功能未完成，不允许合并

**违规判定**：
- 功能代码已修改但测试未更新导致测试失败 → 视为未完成，禁止合并
- 功能代码已删除但测试仍引用已删除功能 → 视为技术债务，必须在下一批次清理
- 测试 mock 数据硬编码 → 视为技术债务，必须在下一批次清理

### 规则 7：法律合规与安全标准（PR，2026-07-08 追加）
...
### 规则 8：记忆文件管理（PR，2026-07-08 追加，2026-07-10 修正，2026-07-14 二次修正）
...
### 规则 9：.monkeycode 全目录强制追踪（PR，2026-07-17 追加）

> `.monkeycode/` 目录下所有文件必须强制追踪变更，不允许忽略任何文件。

**强制执行要求**：
- `.gitignore` 已移除 `.monkeycode/` 忽略规则及白名单例外
...
### 规则 10：审计文档同步规则（PR，2026-07-17 追加）

> 审计计划/复审规则变更时，所有关联文档必须同步更新，不允许遗漏。

**强制执行要求**：
- **主文档变更时必须同步的关联文档**：
  1. `audit_assignment.md`（审计任务分配）— 维度全景表/触发条件/执行流程/修复队列
  2. `CHANGELOG.md`（变更记录）— 每轮升级一行总结
  3. `doto.md`（未完成任务）— 审计计划专项提醒
  4. `MEMORY.md`（规则记忆）— 规则 4 下复审完成状态表
- **同步时机**：每次审计计划变更（新增维度/调整类别/修改流程）后立即同步所有关联文档
- **同步验证**：同步完成后用 `grep` 验证各文档中的维度数、类别数、执行批次数一致
- **禁止遗漏**：任何一次主文档变更都必须同步全部关联文档，不允许只更新主文档

### 规则 11：规则自我迭代机制（PR，2026-07-17 追加）
...
| 分类 | 定义 | 优先级 | 强制性 | 示例 |
|------|------|--------|--------|------|
| **项目规则（PR）** | 技术架构/CI/安全/业务流程强制要求 | 最高 | 强制执行 | 规则 0 真实实现强制、规则 3 修复流程、规则 2 代码质量 |
| **项目习惯（PH）** | 非强制但约定俗成的项目工作方式 | 中 | 遵循但不强制 | 提交信息中文描述、批次编号连续递增、子代理仅编辑不推 PR |
| **个人规则（IR）** | 用户明确要求的偏好（优先级高于项目规则） | 高于 PR/PH | 强制执行 | 规则 12 个人规则高于项目规则、规则 13 成品文档 .xlsx/.docx |
...
| **合并** | 两规则内容高度重复 | 保留主规则，被合并规则标记"已合并到规则 X" |
...
- **定期整理**：每 15 个批次（与规则 8 配合）整理一次自我迭代日志，归档旧记录到 doto-su.md

### 规则 12：工具连接异常重试策略（PR，2026-07-17 追加）

> 遇到工具调用持续返回连接异常时，必须按分级响应策略处理，持续异常时每隔 30 分钟再次重试，直到连接恢复。
...
| **L3 持续连接异常** | 连续 3+ 次失败或返回 `connection refused` | 进入 30 分钟周期重试循环 | 1800 秒 |
...
- **禁止行为**：
  - 禁止在 L3 重试间隔内频繁短重试（如 sleep 3/5/10/30 等）
  - 禁止在未恢复前向用户报告"无法继续"并等待人工介入
  - 禁止跳过或缩短等待时间
  - 禁止在等待期间完全无输出

**恢复后动作**：
1. 立即继续被中断的原任务（不需重新询问用户）
2. 把等待期间的推理结论立即应用到具体任务中
...
### 规则 13：PR 描述强制依据模板填写（PR，2026-07-29 追加）

> 所有 PR 创建/更新时，PR 描述必须严格依据 `/.github/PULL_REQUEST_TEMPLATE.md` 模板填写。
...
### 规则 14：产品图谱驱动工作流（PR，2026-08-11 追加）

> 适用于所有用户请求的强制精益、产品图谱驱动工作流。

**执行顺序要求**（严格按照以下顺序执行）：
1. **阅读完整的常驻技能指令**
2. **加载最新的产品图谱**，且仅加载与请求相关的节点
3. **锁定核心业务目标**和最小验收范围
4. **检查并复用**成熟的项目解决方案
5. **实现最小可行**的完整解决方案
6. **立即写入产品图谱**（新需求、变更模块、接口、变量、页面、业务目标、约束和经验教训）
7. **验证结果**，清理无关内容和冗余代码，更新图谱，返回简洁可用的交付结果

**产品图谱规范**：
- 通过定向文件搜索定位项目本地的规则文件、或已确立的等效文件
- 优先使用最近/当前项目的图谱
- 仅阅读其目标、约束、复用规则、历史坑点、输出规则、Token 节约规则以及与任务相关的功能/业务节点
- 不要全局遍历无关文件或历史记录

**图谱标题**（当图谱已存在或初始化新图谱时，保留以下标题）：
- 核心目标
- 业务清单
- 功能节点
- 约束黑名单
...
**精益执行规则**：
- 保持一个明确的核心目标和最小必要范围
- 仅阅读与任务相关的上下文；优先使用定向搜索和小范围文件切片
- 复用现有的命名规范、CSS、JS、API、组件、模板和项目约定
- 移除重复逻辑、未使用变量、过时注释、死代码、冗余空格和不必要的依赖
- 避免推测性扩展、大范围重构、冗长解释、重复定义和非请求的示例
- 不要重复询问可从工作区中获取的事实
- 在不同任务间保持提示词、结构、命名和输出格式的标准化
- 将产品图谱黑名单和历史坑点视为硬性约束

**交付约定**：
- 一次性交付完整、可用的结果
- 在响应前，确认相关产品图谱节点已同步、未添加无关内容、冗余代码已移除
- 仅报告结果、关键文件或产物、验证状态以及任何实际阻碍；保持回复简洁
...
### 规则 15：个人规则高于项目规则（IR 个人规则，2026-07-08 追加）
...
- 个人规则包括本文件中标注为 [个人规则] 的规则
- 项目规则指本文件第一章标注为 PR 的规则
- 个人习惯（IH）和项目习惯（PH）的识别、记录、维护见规则 11 自我迭代机制

### 规则 16：项目成品导入/导出文档格式（IR 个人规则，2026-07-06 追加）
...
### 规则 17：禁止简洁方案（IR 个人规则，2026-07-08 追加）
...
- 与规则 0 一致：所有方案必须服务于真实业务需求

---

## 三、基础规范

### 匹号/缸号领域术语（2026-09-05 用户确认，权威设计 .monkeycode/docs/piece-number-domain-design.md）
- **生产匹号**：生产报工时逐匹登记，附带机台号+开机人；仅在**生产环节**使用，胚布无缸号
- **外发染色**：有生产匹号的胚布外发染色；染色完成后**必须有缸号**并生成**染色匹号**
- **染色匹号**：在入库/外发/销售/出库/对账都必须体现；与生产匹号不通用
- **匹号编号语义（2026-09-05 用户二次确认）**：
  - **生产匹 = 生产单号下的产品生产出来的第 * 匹**：inventory_piece.batch_no 记生产单号（production_order.order_no），piece_no 由报工逐匹登记传入（胚布仓维度）
  - **染色匹 = 缸号/染色批次号染色后的第 * 匹**：piece_no = {缸号}-{seq:03}，batch_no = 缸号，piece_seq 同缸递增（与验布打卷 generate_next_piece_no 同语义）；净布外发的无缸号胚布匹例外用回仓单号编号
- **净布工艺例外**：净布外发不需要缸号；其他外发工艺必须在染色之后
- **仓库类型约束**：胚布仓（warehouse_type=greige）只存未染色/未做工艺的胚布；成品仓（finished）只存染色/工艺后成品；入错仓库必须拒绝
- **查布**：胚布逐匹全查（产品/匹号/疵点/重量/查布时间）；成品入库前抽查


> 原 project_rules.md 已删除（2026-07-17），内容已合并到本节。
...
- **代码审查维度**：代码质量、安全性、性能、测试覆盖（与规则 1/2 联动）
...
> 与规则 2 联动：禁止警告抑制。完整强制要求见规则 2。
- **例外**：backend/src/models/ 下的 SeaORM 自动生成模型可保留文件级 `#![allow(dead_code)]`（模型字段由派生宏使用）
...
- **文件职责分工**：详见规则 8 的文件分工表（MEMORY.md 索引 / MEMORY-SU.md 详细 / doto.md 未完成任务 / doto-su.md 已完成归档 / CHANGELOG.md 一句话总结 / audit_assignment.md 审计任务分配）
- **路径策略**：test 分支合并入 main 时 `-X theirs` 会覆盖 `.monkeycode/`，必须以 main 版本为准
...
- **多租户已删除**（2026-06-28）：所有 tenant_id 列/字段/过滤/索引/管理表均已移除，项目不再支持多租户
...
> 本节记录所有规则/习惯的自我迭代历史，按规则 11 自动维护。
> 每条记录包含：日期、操作类型、规则编号、分类、触发条件、来源上下文、迭代内容、验证结果。
> 旧记录每 15 个批次（与规则 8 配合）归档到 [doto-su.md](file:///workspace/.monkeycode/doto-su.md)。
...
| 重视规则同步性 | V15 第八轮/第九轮反馈 | 用户反复追问"为什么没有同步" | 触发规则 10 审计文档同步规则 |
| 重视完整性审计 | 第九轮反馈"所有维度都应该被严格审计" | 用户要求全面扫描不遗漏 | 触发 V15 从 15 类扩展到 25 类 |
| 重视推送后清理 | 多次对话中"删除推送分支" | 用户偏好推送后立即删除分支保持仓库整洁 | 每次推送合并后立即删除分支 |
| 批次大小偏好 | 3+ 次对话中反复要求"每次最少修改 6-8 文件" | 用户偏好单批次覆盖 65-99 个文件 | 每批规划 65-99 文件，跨域合并可接受 |
| 简洁高效专业速度 | 用户 2026-07-17 明确要求"MEMORY.md 文件需要简洁高效，专业，速度" | 用户偏好规则索引文件精简化，详细内容拆分到独立文件 | 触发 MEMORY.md/MEMORY-SU.md 拆分 |
| 梳理合并排序 | 用户 2026-07-17 明确指令"MEMORY-SU.md 梳理和合并排序" | 用户偏好规则文件结构清晰，章节合并去重 | 触发 MEMORY-SU.md 压缩 |
| 环境重置偏好 | 2026-07-28 用户明确要求"重置环境" | 用户偏好在开始新任务前将工作区重置到干净状态 | 每次大任务完成后或开始新阶段前，主动重置环境到 main 最新干净状态 |
...
| 习惯名 | 来源 | 表现 |
|--------|------|------|
| 批次编号连续递增 | 规则 3 确立 | 批次编号不跳号，397→398→399... |
| 提交信息中文描述 | 多次对话确认 | commit message 使用中文，描述"做了什么"和"为什么" |
| 子代理仅编辑不推 PR | 子代理协作模式确立 | 子代理只编辑文件，主代理汇总后开 1 个 PR |
| heredoc 格式 commit message | 规则 3 确立 | 多行 commit message 使用 heredoc 格式 |
| 修复分支命名 fix/batchN-描述 | 规则 3 确立 | 分支命名规范统一 |
| 规则文件拆分索引/详细 | 2026-07-17 三次迭代确立 | MEMORY.md 索引版 + MEMORY-SU.md 详细版 |
| 外部规则文件内联化 | 2026-07-17 八次迭代确立 | project_rules.md 删除后内容内联到 MEMORY-SU.md |

### 6.3 最近迭代记录

| 日期 | 操作 | 规则 | 分类 | 内容摘要 |
|------|------|------|------|----------|
| 2026-08-14 | 合并 | 规则 6+18 | PR | 测试规范合并（mock 数据 + 功能变更同步测试） |
| 2026-08-14 | 合并 | 规则 2+19 | PR | 代码质量与编码规范合并 |
| 2026-08-11 | 新增 | 规则 14 | PR | 产品图谱驱动工作流 |
| 2026-07-29 | 新增 | 规则 13 | PR | PR 描述强制依据模板填写 |
| 2026-07-17 | 新增 | 规则 11/12 | PR | 规则自我迭代机制 + 工具连接异常重试策略 |
| 2026-07-17 | 合并 | 规则 0+8 | PR | 规则 8 合并到规则 0 |
| 2026-07-17 | 精简 | 规则 4 | PR | 复审严格规范精简 |

---

## 七、归档索引

> 历史批次修复详情、审计报告、迭代历史已归档到以下位置：

| 归档内容 | 位置 |
|----------|------|
| 已完成任务详细记录 | [doto-su.md](file:///workspace/.monkeycode/doto-su.md) |
| 审计报告 | [docs/audits/](file:///workspace/.monkeycode/docs/audits/) |
