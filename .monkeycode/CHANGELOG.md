...
| PR #941 | Round 7-iter31 续五：把用户拍板的纺织四维口径全部落地并本地提交——白坯布改为「无色即白坯」的唯一共享判定（废除三处按色号名称含白猜白坯，带白字的色号其实是染色布）、出库预留分支与扣减同源强制四维、收货与订单不符从「货物照入+进度不累加+只记 error 日志」升级为整单拒绝（同事务 Drop 自动回滚，不留部分成功）、退货明细追溯三列按 出库行→订单行 优先级回写且三种缺源各自报错、业务错误新增显式可外显变体（首批 5 条，含数量/内部 ID/状态语义仍脱敏）并修掉 currency_handler 绕过脱敏直接回显内部文案的真实泄露、去掉 ai-models 与 contract-signatures 两处双重 nest 使整族端点可达、7 处方法不符按真实语义判定（5 处是后端未实现的功能缺口，未伪造端点）；新增 check-api-paths.mjs 前缀感知门禁（A 类 0，22 条例外逐条带原因）。权限资源注册表（1284 行）本轮**主动撤出**：403 策略虽未放宽，但「485 前缀是否覆盖全部真实路由」与「解析名与旧猜名不同会让已授权角色突然 403」两点在禁本地编译的约束下不可证明，静态文件×挂载模型既漏又假阳，前置条件是先做出权威路由清单 |
...
| PR #941 | Round 7-iter29 续十二：先按「每轮错误日志全读」复核上一轮 run，确认 4622 的唯一失败是 `📦 Rust 测试预编译`，其日志显示 `cargo nextest archive` 从 05:17 一直跑到 05:49 被我自己的推送取消（Terminate orphan process: cargo-nextest），E2E 那 28 个 FAIL 全是 cancelled 而非断言失败——台账里据此登记的「已修复」不受影响，但也说明这一轮的验证仍未真正落地。随后把状态词表核对方法用到库存台账，核出的不是「字面量没收敛」而是**常量选错了域**：`POST /inventory/stock` 建单写 `stock_status = master_data::ACTIVE`（值 active）与 `quality_status = "qualified"`，而这两列的取值域是 正常/报废/已删除 与 合格/待检/不合格，这批行对所有按「正常+合格」过滤的可用量、可出库、缺料预警查询永久不可见；同时 `dashboard_service.rs` 六处按 `StockStatus.eq(master_data::ACTIVE)` 过滤库存，仪表盘总量/低库存/零库存/仓库分布恒为 0，唯一能看见的反而是被写坏的那些行，两个错误互相抵消所以十几轮 CI 无人发现；`five_dimension_service.rs` 更写字面量 "ACTIVE"（大写，连 active 都不匹配），五维统计对任何数据都恒空。修法：三处一律改用本列常量，`inventory_stock_status` 与 `inventory_stock_quality_status` 各补 ALL 作取值域单一来源，建单初始值抽成 `initial_stock_statuses()` 以便测试钉住，新增 production 域迁移 `m0057_normalize_stock_status_domain` 归一存量（落在 m0056 同域同位置，执行顺序晚于建表已核对）；台账状态的越界筛选值由「200 + 零行」改为 400 并列出合法值，列表与导出同口径。顺带修掉同一条链路上四处：库存列表的 keyword 后端根本没这个入参（界面筛选框提交即被 axum 丢弃，属假控件），现按产品编码/名称取候选 ID 再下推、无命中直接空集；导出 xlsx 丢掉已算好的产品编码/名称/仓库名称与状态阈值只回显裸 ID，现与列表同口径并把全部筛选条件写进导出审计快照；打印用的 properties 里写 `quantity`（后端字段是 quantity_on_hand）致该列恒空白、表头回显英文字段名，现改为 print-js 真正认的 `properties`+`displayName` 并补本地化表头，状态取值→文案映射收进 `invFmts.ts` 供列表/详情/打印共用；接口模型与 DTO 两处文档仍写 `库存状态（正常/冻结/待检）`——正是当初诱导前端做出假筛选项的来源，改为真实取值域并标注「本列从未有冻结/待检写入方」。另发现并改掉上一轮我自己写错的一条断言：`public_message()` 对所有错误统一脱敏，E2E 里 `expect(body).toContain('合格')` 永远不会成立，改为断稳定错误码 VALIDATION_ERROR，「必须列出合法值」由后端单元测试钉住；登记待决两项：越界入参的回显合法值在对外响应里看不见（是否给参数校验开脱敏例外属安全决策，不擅改），库存页「采购」按钮跳转带的 product_name 采购页从未读取（要真生效需给采购列表补按产品筛选，跨端改造单独一轮）。补 `backend/tests/handlers_inventory_stock_status_test.rs` 五例与 01-p2p 1-6 的关键词/越界值断言；purchase-receipt 两处对话框标题内联中文改走文案键；README 后端测试统计按实测更新为 257 文件 / 2,095 函数、i18n 为 zh 10,196 / en 10,219（引用 9,679）；同批把这条核对方法用到质检结论回写路径，坐实 `sync_receipt_inspection_status` 把中文结论（待检/合格/不合格）原样复制进 `purchase_receipt.inspection_status`（该列取值域是大写码，唯一合法值 PENDING 还是个裸字面量），凡是回写过结论的入库单，该列都成了任何读取方都不认识的取值，按大写码判断的逻辑一律视为「从未检验」；现补 `PASSED/REJECTED/ALL` 与 `from_inspection_result` 显式映射，映射不到即报错而不是默认成某个值，建单裸字面量改引常量，并把该路径上「关联 ID 缺失即静默跳过、入库单查不到也不出声」两处静默分支改为显式报错（外键破坏必须让调用方知道），m0057 追加第三条归一（`purchase_receipt` 在 business 域建表，production 域执行顺序晚于它，已核对）。核对过：全仓含 e2e 与后端测试与该列无任何读取或断言，也没有后端路径自动写 related_type=PURCHASE_RECEIPT，故不存在旧行为被测试背书的问题；映射回归用例见 `handlers_quality_inspection_result_test.rs::test_receipt_inspection_status_maps_within_its_own_domain` |
| PR #941 | Round 7-iter29 续十三：run 4623（head bb028f30）跑完，43 success / 9 failure，本轮把 9 个失败逐条读日志判责（job 日志 + artifact 内 playwright-output.txt 与 backend.log 全读，不再靠残缺信号下结论）。两个后端门禁红是我上一批 `customer_name` 下推留下的 `E0063 missing field`——给 handler 的 Query 结构体加字段时漏改 `tests/handlers_sales_order_handler_test.rs` 的两处字面量初始化器，这是「本地无编译权」同类问题第 5 次复发，自审清单据此新增一条：加字段必须同步 grep tests/ 下该结构体的全部初始化点（本轮把 `StockListFilter`/`RecordQuery`/`ListStockParams` 一并核过，只有 SalesOrderQuery 中招）。五个 E2E 分片共 8 条失败中，6 条判为**测试判错**且各有不同根因：① 缺料 `level=critical` 期望被拒却 200——后端 `validate_enum_param` 用 `eq_ignore_ascii_case` 归一是仓库既有约定，真正越界的是换词根的 high/medium/low，现改为断那些值 4xx，并新增"归一后必须与规范码同结果集、行内 level 为 Critical"，防止接受小写反而造出第二套口径；② 缺料状态下拉读出级别文案——核对页面两个 select 的选项源证明页面正确，是测试 Escape 后未等 teleport 浮层收起就点下一个 select，点击落在浮层上，现加收起确认与"第二个面板 5 项"前置断言；③ `5-6 BPM 任务` 断 `data.items` 失败——`query_user_tasks` 返回 `PageResponse{...,data}` 且前端 `api/bpm.ts` 一直按此消费，是测试假设了不存在的字段名，现按真实契约断言并把状态域收窄到 `bpm_task` 四值（原用例还放行库里不存在的 processing）；同因发现 10d 的 A1-3/A1-5 是 `expect(expr)` 无匹配器的真空断言所以长期"假绿"，一并补成真实断言；④ `M1-6` 的 `GET /business-modes/rules` 是 405——该路径只注册了 POST，规则根本没有全局列表端点（只有 by-mode），改用真实入口并断 mode_id 归属与 rule_type 取值域，同时登记"是否需要全局规则列表"待产品决定（该域前端零调用方，不擅自建端点）；⑤ `1-8 付款` 的 400「未付金额为 0，申请金额 56500 超过未付金额」（对外文案脱敏，真因只在 backend.log detail）——核对 `ap_invoice_ops/receipt.rs` 证明后端两种生成逻辑都正确（红字退货单 unpaid=0 是有意设计），根因是 1-7 查应付单不带 supplier_id 而取了库里任意一张再写死 56500，现按本供应商查询、优先挑未付金额大于 0 的、断 supplier_id 归属，申请金额改取该单真实未付金额，两处 `test.skip` 前置兜底改成显式断言。另把库存/质检/入库三处状态词表跨域误用与库存出口口径一并修完（见续十二）。仍待判责：31d~38 分片 1 条、39b~54 分片 3 条（48-3 AR 幂等、53-2 跨用户审批、54 新域流转链多条）、smoke 4/5 分片 1 条，判责证据在各自 artifact，按"日志全读"原则逐条处理，不允许靠猜。 |
| PR #941 | Round 7-iter29 续十四：趁 run 4624 在跑（52 job 已绿、E2E 分片进行中），把续十三登记为待判责的三条失败读完并修完，同时读 artifact 时挖出一个更严重的页面级契约缺陷。`54-new-domains` 验布用例报「等不到成功提示」，看 error-context 快照才发现页面已经进了 ErrorBoundary、顶栏显示未登录；再翻 diag 才看到真实拒因：`views/fabric-inspections/index.vue` 的建单与编辑表单提交的是 `fabric_batch_no` 与 `total_length_m`，这两列在后端 `CreateInspectionRequest` 与 `fabric_inspection_record` 里都不存在，而该 DTO 唯一的必填项 `inspection_date`（NaiveDate）表单压根没提供，于是每次新建验布单都被 422 missing field 拒掉（首轮还叠了一次 CSRF 403，重试后才暴露真因）。修法是把表单按真实契约重建：验布日期必填并按当天预填、缸号/色号/验布员/机台号/评分制式/门幅英寸/备注可选；编辑态把 `UpdateInspectionRequest` 不接受的日期/缸号/色号三项置灰——不能留一个改了也不会生效的输入框；详情面板撤掉两个死字段，改列出色号/验布日期/机台/评分制式/门幅/总扣分/每百平方码分数；api 层的 `Record<string, unknown>` 换成与后端字段一一对应的 `CreateFabricInspectionPayload`/`UpdateFabricInspectionPayload`，字段写错从此由类型检查拦截；评分制式收进 `constants/fabric-scoring.ts`（four_point/ten_point，与后端 `fabric_scoring` 同源），提交码值不提交文案。另两条判责结论：44f-7 的「业务处理失败」detail 是「审核前处方明细不能为空」，后端约束正确、是用例建了张空处方去审核，现补一条染料明细；53-2 断「无敏感角色」实际是 `GET /roles` 出参为 `RoleListResponse{roles,total}`（全量无分页）而用例按 items 取值恒 undefined，现按真实契约断言并去掉误导性的分页参数；48-3 断「AR 列表应可达」拿到 403 的根因是路径写错（真实路由 `/ar/invoices`），不存在的端点被权限层先拒成 403，同文件 48-2 早已为 AP 踩过并留下注释、AR 这条却仍用 `/ar-invoices`。README 的前端 TS 文件数按 `find src -name '*.ts' ! -name '*.d.ts'` 实测更正为 267（原记 261 已滞后 6 个文件）。 |
| PR #941 | Round 7-iter29 续十五：修完库存预警整页不可用的契约缺陷。`GET /inventory/stock/alerts` 原先返回裸 `json!({list,total})`，行内只有 ID 与数量，而前端 `api/inventory.ts` 声明的是 `StockAlert[]`（product_name/product_code/warehouse_name/current_quantity/min_quantity/unit/alert_level 七项后端都不存在），`views/inventory/index.vue` 与 `store/inventory.ts` 两处都是把 res.data 直接（带空数组兜底）赋给数组——把分页对象直接赋给数组，预警表格因此恒空且没有任何提示，级别列还按自造的 warning 与 danger 两档渲染。后端改为 `StockAlertQuery` + `StockAlertRow` + `PaginatedResponse`：分页真实生效（原来 page/page_size 被完全忽略）、产品编码/名称/单位/仓库名称按 ID 批量带出、主数据缺失记 error 而不是用 ID 拼假名称、错误原样透传不再一律压成 internal。前端 `StockAlert` 与后端逐字段对齐（数量为 Decimal 字符串序列化），告警类型词表收进 `constants/stock-alert-type.ts`（normal/low_stock/out_of_stock/over_stock/slow_moving/expiring/discrepancy，含 el-tag 颜色映射与越界值告警），两处取数改为按 `items` 并在形状不符时直接抛错、取满一屏仍有剩余时 warn 说明截断。同时把 `StockAlert↔StockAlertRow` 纳入 `check-contract.mjs`（11 组映射 / 146 字段 / 0 挂账），`10a` 的 L1-10 从「真空断言 + 失败改查 /material-shortage 顶包」改成真实契约断言（分页回显、告警类型在取值域内、必须带出产品名、在库量可解析）。撤掉因改走 alert_type 词表而不再被引用的两个旧文案键；README 的 i18n 与前端 TS 文件数按实测更正为 zh 10,201 / en 10,224（引用 9,677）与 268 个。 |
