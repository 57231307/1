...
| D10-6a | #698 main 9d26d7d | D10 第 6 批 1/4：event_bus.rs (1243行) 拆为 facade (338行，含 ShippedItem/BusinessEvent/EventBusState/EventBus 类型 + EVENT_BUS/EVENT_BUS_STATE/MAIN_LISTENER_HANDLE 静态 + new/publish/subscribe/Default 公共 API + pub use 3 自由函数) + event_bus_ops/{mod 20, kafka 110, listener 847}，EventBusState 改 pub(crate) struct + 4 字段 pub(crate)，lock_event_bus_state/MAIN_LISTENER_HANDLE 改 pub(crate)；po/order.rs (1234行) 拆为 facade (561行含测试，PurchaseOrderDto/PurchaseOrderItemDto/PurchaseOrderService + new) + po/order_ops/{mod 19, crud 536, lifecycle 66, query 137}，db 字段 pub(crate)，generate_order_no_with_txn 宏在 crud.rs 被 price.rs 调用；auth_service.rs (1201行) 拆为 facade (621行含测试，AppClaims/AuthService/AuthError/2 From impl + pub use 8 JTI 函数) + auth_service_ops/{mod 13, auth 256, jti 374}，db/encoding_key 字段 pub(crate)，REVOKED_USERS/REVOKED_USER_TTL_SECS 改 pub(crate) 供测试访问；inventory_finance_bridge_service.rs (1192行) 拆为 facade (108行，3 args struct + new) + inventory_finance_bridge_ops/{mod 14, listener 202, voucher 925}，BRIDGE_LISTENER_HANDLE 迁入 listener.rs，3 私有上下文 struct 迁入 voucher.rs，7 create_*_voucher 方法改 pub(crate) 供 listener.rs 调用；CI 一次通过（Clippy+后端构建+单元测试全绿） |
