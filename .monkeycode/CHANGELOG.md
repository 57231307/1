...
### 修复内容（P1-19/21/22 前端路由与菜单治理）

**router/index.ts**：给 17 条详情页/子页/设置页路由添加 `meta.hidden: true`（不在侧边栏显示但可通过路由跳转访问）
- 详情页：客户360视图、报价单详情、定制订单详情、色卡详情、色号价格详情、工艺优化详情
- 子页/新建：新建报价单、编辑报价单、报价单审批、新建定制订单、工艺跟踪、新建色卡、新建色号价格
- 设置页：双因素认证、修改密码、个人信息（头部下拉已有入口）
- 演示页：组件示例

**MainLayout.vue**：补齐 32 条主入口孤儿路由的侧边栏菜单项 + 新建 AI 智能菜单
- 面料管理：色卡列表、色卡借出、色号价格、批量调价
- 库存管理：物流管理
- 销售管理：销售合同、销售价格、销售分析、报价单管理
- 采购管理：采购合同、采购价格、采购检验、采购退货
- 客户关系：公海客户池、客户分配、线索管理、商机管理
- 生产管理：定制订单、染色配方（从工作流移入）、染色批次
- 财务管理：增强版应收对账、BI 销售分析
- 工作流：移除染色配方，新增 BPM 流程定义、流程模板、审批中心
- 系统管理：安全管理、邮件管理、租户计费、主备监控
- 新建 AI 智能菜单：AI 分析深化、AI 工艺优化、AI 质量预测（MagicStick 图标）

**影响**：解决 48 条孤儿路由无入口、菜单分类混乱（染色配方归错工作流）、CRM 子页全孤儿等问题。
