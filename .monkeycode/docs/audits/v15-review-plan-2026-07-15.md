# V15 全项目综合审计计划（2026-07-15，第十轮升级版）
...
### 类二十五：部署流程与升级管理审计专项（4 维度）⭐ V15 新增（用户 2026-07-16 第十轮反馈"添加部署流程/内置 CLI 工具/内置升级流程/平滑升级")

> **目的**：当前部署/升级相关代码分散在 5 个 deploy/*.sh 脚本 + backend/src/cli/ 子模块（admin/migrate/util/{service,backup,upgrade,misc}），需要独立专项审计，覆盖部署流程闭环、CLI 工具完整性、升级流程健壮性、平滑升级能力四大维度。
>
> **现状调研**（基于代码扫描）：
> - 部署脚本：`deploy/deploy.sh` / `deploy-frontend.sh` / `deploy-backend.sh` / `deploy-prepare.sh` / `deploy-latest.sh`（5 个脚本）
> - CLI 入口：`backend/src/bin/cli.rs` + `backend/src/cli/mod.rs`（顶层 `Command` 枚举：Admin/Migrate/Util）
> - CLI 子模块：`admin.rs`（密码哈希）+ `migrate.rs`（**占位未实现**）+ `util/{service,backup,upgrade,misc}.rs`（14 个子命令：Status/Start/Stop/Restart/Logs/Backup/Restore/Health/Upgrade/Deploy/Clean/Config/Info/Rollback）
> - 升级实现：`backend/src/cli/util/upgrade.rs`（cmd_upgrade/cmd_deploy/cmd_rollback 三个核心函数）
> - 备份实现：`backend/src/cli/util/backup.rs`
> - 安装目录：`/opt/bingxi-erp`，服务名：`bingxi`（systemd），后端端口：8082
> - GitHub 仓库：`57231307/1`，6 个国内加速镜像源
>
> **关键发现**（V15 扫描初步）：
> 1. **数据库迁移 CLI 是占位实现**（`migrate.rs` 标注"占位"），未真实接入
> 2. **部署过程有服务停止时间**（cmd_deploy 第 184 行 `systemctl stop`），非零停机
> 3. **无灰度升级/蓝绿部署机制**，无金丝雀发布
> 4. **回滚仅恢复二进制文件**，不回滚数据库 schema 变更
> 5. **无版本兼容性校验**（升级前后端 API 版本/DB schema 版本无校验）
> 6. **无健康检查自动回滚**（部署后仅检查服务 active，未检查业务健康）
> 7. **CLI 错误处理不统一**（部分 println! + return，部分 exit(1)）
> 8. **无升级日志持久化**（仅 println! 输出，未写入升级日志文件）

#### 25.1 部署流程审计

| 检查项 | 期望状态 | 来源 |
|--------|----------|------|
| 部署脚本完整性 | 5 个脚本职责清晰：deploy-prepare（环境准备）/deploy-backend（后端部署）/deploy-frontend（前端部署）/deploy.sh（协调总入口）/deploy-latest.sh（一键最新版） | 类七 7.3 |
| 脚本幂等性 | 多次执行不产生副作用（mkdir -p/cp -r 幂等，rm -rf 需校验路径非根） | 通用运维 |
| 脚本错误处理 | set -euo pipefail 必须配置，关键命令失败立即退出 | 通用运维 |
| 脚本路径一致性 | `/etc/bingxi-erp/.env` 与 systemd `EnvironmentFile` 一致，`grep -r '/etc/bingxi' deploy/` 全部一致 | 类七 7.3 |
| 配置目录 | `/etc/bingxi-erp/.env` 权限 600，owner root | 类七 7.3 |
| 安装目录 | `/opt/bingxi-erp` 权限 755，子目录 backend/frontend/logs/backups 结构完整 | 类七 7.3 |
| systemd 服务文件 | `EnvironmentFile` 路径 + `ExecStart` 路径 + `User` + `Restart=on-failure` + `LimitNOFILE` | 类七 7.3 |
| 依赖检查 | 部署前检查 Nginx/curl 已安装，**不安装** PostgreSQL 客户端/Redis（用远程服务） | MEMORY.md 部署限制 |
| 端口冲突检查 | 8082 端口未占用检查，Nginx 80 端口未占用检查 | 通用运维 |
| 密钥自动生成 | 首次部署自动生成 base64 32 字节密钥（熵比 > 0.15） | 类七 7.3 |
| 备份前置 | 升级/部署前必须自动备份（database + files），失败则中止 | 类七 7.3 |
| 部署后健康检查 | 部署后 `/api/health` 返回 200，DB 连接正常，Redis 连接正常 | 类二十 20.4 |
| 部署日志 | 部署全过程写入 `/opt/bingxi-erp/logs/deploy-YYYYMMDD-HHMMSS.log` | 通用运维 |
| 回滚机制 | 部署失败自动回滚到上一版本（含二进制 + 前端 dist） | 25.4 |
| Nginx 配置 | 反向代理 8082 + WebSocket 升级头 + gzip + 静态资源缓存 | 通用运维 |
| 防火墙 | 仅开放 80/443 端口，8082 不对外暴露 | 类三 3.4 |
| 禁止 Docker | 不创建 Dockerfile/docker-compose.yml（MEMORY.md 限制） | 类七 7.3 |
| CI/CD 集成 | GitHub Actions 构建 → GitHub Release → 手动部署到生产服务器 | 类七 7.3 |

**合理性评估**：当前部署脚本可能缺少 `set -euo pipefail`、端口冲突检查、部署后业务健康检查、部署日志持久化，**部分不合理**。

#### 25.2 内置 CLI 工具审计

| 检查项 | 期望状态 | 来源 |
|--------|----------|------|
| CLI 入口完整性 | `bingxi` 命令顶层 3 个子命令：Admin/Migrate/Util | 代码现状 |
| Admin 子命令 | `bingxi admin hash-password` 密码哈希工具，禁止默认 "admin123" | 类三 3.5 |
| Migrate 子命令 | `bingxi migrate` 必须真实实现（当前是占位）：列出迁移/执行迁移/回滚迁移/状态查询 | **V15 新增** |
| Util 子命令完整性 | 14 个子命令：Status/Start/Stop/Restart/Logs/Backup/Restore/Health/Upgrade/Deploy/Clean/Config/Info/Rollback | 代码现状 |
| 服务管理 | Status/Start/Stop/Restart/Logs 5 个命令，支持 systemd 服务管理 | 代码现状 |
| 备份恢复 | Backup（database/files/all）+ Restore（从文件恢复），失败必须 exit(1) | 类七 7.3 |
| 健康检查 | `bingxi util health` 检查后端+DB+Redis+磁盘空间 | 类二十 20.4 |
| 升级部署 | Upgrade（从 GitHub Release 下载）+ Deploy（本地包）+ Rollback（回滚） | 25.3/25.4 |
| 清理工具 | `bingxi util clean` 清理 logs/backups/temp，保留最近 N 份 | 通用运维 |
| 配置查看 | `bingxi util config` 显示当前配置（脱敏密钥） | 通用运维 |
| 系统信息 | `bingxi util info` 显示版本/安装目录/服务状态/磁盘空间/内存 | 通用运维 |
| 命令帮助 | 每个子命令必须有 `--help` 文档，clap derive 自动生成 | 通用 CLI |
| 版本号 | `bingxi --version` 输出 CARGO_PKG_VERSION | 代码现状 |
| 错误处理统一 | 所有子命令失败必须返回非零退出码，禁止 println! + 静默 return | 类二 2.5 |
| 日志输出 | CLI 输出同时写入日志文件（`/opt/bingxi-erp/logs/cli-YYYYMMDD.log`） | 通用运维 |
| 镜像源配置 | GitHub 下载支持 6 个国内镜像源 + 直连，自动降级 | 代码现状 |
| 权限校验 | 升级/部署/备份/恢复命令必须 root 权限，普通用户运行报错退出 | 类十二 12.3 |
| 危险操作确认 | Upgrade/Deploy/Rollback/Restore 必须二次确认（`--yes` 跳过） | 通用 CLI |

**合理性评估**：当前 CLI 工具 migrate 子命令占位未实现、错误处理不统一、无日志持久化、无权限校验、无危险操作确认，**不合理**。

#### 25.3 内置升级流程审计

| 检查项 | 期望状态 | 来源 |
|--------|----------|------|
| 升级流程完整性 | 当前版本检查 → 目标版本获取 → 备份 → 下载 → 校验 → 部署 → 健康检查 → 清理 | 代码现状 |
| 版本获取 | `bingxi upgrade` 不指定版本时自动获取 GitHub Release 最新版本 | 代码现状 |
| 版本指定 | `bingxi upgrade --version v2026.x.x.xxxx` 支持指定版本 | 代码现状 |
| 版本格式校验 | 版本号必须匹配 `v\d+\.\d+\.\d+\.\d+` 格式，禁止任意字符串 | 通用运维 |
| 版本回退检查 | 禁止降级到比当前版本更低的版本（除非 `--force-downgrade`） | **V15 新增** |
| 备份前置 | 升级前必须自动备份（database + files），`--no-backup` 需二次确认 | 代码现状 + V15 |
| 下载校验 | 下载后 SHA256 校验，与 Release assets 中的 sha256sum 文件对比 | **V15 新增** |
| 镜像源降级 | 直连失败自动尝试 6 个国内镜像源 | 代码现状 |
| 下载中断恢复 | 支持断点续传（`curl -C -`），避免大文件下载失败重来 | **V15 新增** |
| 包完整性校验 | tar -tf 列出内容 + 路径穿越检查（无 `..` 和绝对路径）+ 解压后二次校验 | 代码现状 |
| schema 版本兼容 | 升级前检查 DB schema 版本与目标版本兼容性，不兼容则警告 | **V15 新增** |
| API 版本兼容 | 升级前检查前后端 API 版本兼容性（前端 dist 与后端 server 版本一致） | **V15 新增** |
| 配置文件迁移 | 升级时自动迁移配置文件格式（如 .env 字段变更），保留旧配置 | **V15 新增** |
| 数据库迁移自动 | 升级后自动执行 `bingxi migrate up`，迁移失败自动回滚 | **V15 新增** |
| 服务重启策略 | 升级后重启服务，失败立即回滚到上一版本 | 25.4 |
| 健康检查 | 升级后 `/api/health` 返回 200 + DB 连接 + Redis 连接 + 关键业务接口 | 类二十 20.4 |
| 升级日志 | 升级全过程写入 `/opt/bingxi-erp/logs/upgrade-YYYYMMDD-HHMMSS.log` | **V15 新增** |
| 升级通知 | 升级成功/失败通知（邮件/Webhook） | 类十九 19.5 |
| 回滚支持 | `bingxi rollback` 回滚到上一版本（二进制 + 前端 + 配置） | 代码现状 |
| 回滚 DB schema | 回滚时同步回滚 DB schema（`bingxi migrate down`） | **V15 新增** |
| 多版本保留 | 保留最近 3 个版本备份，超出自动清理 | **V15 新增** |

**合理性评估**：当前升级流程缺少 SHA256 校验、断点续传、schema 版本兼容、API 版本兼容、配置文件迁移、数据库迁移自动执行、回滚 DB schema、多版本保留，**不合理**。

#### 25.4 平滑升级（零停机/灰度/回滚）审计

| 检查项 | 期望状态 | 来源 |
|--------|----------|------|
| 零停机部署 | 当前 `systemctl stop` 导致服务中断（2-5 秒），必须改造为零停机 | **V15 新增** |
| 蓝绿部署 | 双实例部署（A/B），新版本启动健康后切换 Nginx upstream，旧实例优雅退出 | **V15 新增** |
| 滚动部署 | 多实例滚动升级，逐个实例升级，期间保持最小可用实例数 | **V15 新增** |
| 金丝雀发布 | 新版本先发布到 1 个实例，观察 5-10 分钟无异常后全量发布 | **V15 新增** |
| 灰度发布 | 按用户 ID/IP 百分比灰度（5% → 25% → 50% → 100%） | **V15 新增** |
| 健康检查门禁 | 新版本启动后健康检查通过才接入流量，失败则不切换 | 类二十 20.4 |
| 优雅退出 | 收到 SIGTERM 后停止接受新请求，等待已有请求完成（max 30s），再退出 | **V15 新增** |
| 连接 draining | Nginx upstream 切换前先 drain 旧实例连接（nginx drain mode） | **V15 新增** |
| WebSocket 连接保持 | 升级期间 WebSocket 连接保持，客户端自动重连到新实例 | 类二十 20.3 |
| 长任务处理 | 升级期间长任务（报表生成/数据导入）暂停或迁移到新实例 | **V15 新增** |
| 数据库迁移兼容 | DB schema 迁移必须向后兼容（新增字段 nullable/有默认值），支持新旧版本同时运行 | **V15 新增** |
| 配置热更新 | 非破坏性配置变更支持热更新（无需重启），通过 SIGHUP 或 admin API | **V15 新增** |
| 自动回滚触发 | 升级后健康检查连续失败 3 次（30 秒内）自动回滚 | **V15 新增** |
| 回滚时间目标 | 回滚 RTO ≤ 5 分钟，RPO ≤ 0（无数据丢失） | **V15 新增** |
| 回滚验证 | 回滚后健康检查 + 业务接口验证，失败通知运维 | **V15 新增** |
| 状态保持 | 升级期间用户会话保持（Redis session store），登录状态不丢失 | 类三 3.7 |
| 缓存预热 | 升级后预热关键缓存（用户/产品/客户/供货商），避免冷启动性能下降 | 类七 7.5 |
| 流量切换 | Nginx upstream 切换脚本（`nginx -s reload` + upstream 配置切换） | **V15 新增** |
| 监控告警 | 升级期间监控指标不中断，异常立即告警（错误率/响应时间/资源使用） | 类二十 20.2 |
| 升级演练 | 生产升级前必须在 staging 环境演练，记录演练报告 | **V15 新增** |
...
21. 部署流程与升级管理审计架构设计完成（类二十五 25.1 部署流程 + 25.2 CLI 工具 + 25.3 升级流程 + 25.4 平滑升级方案已设计）
...
V15 审计阶段（自动执行，20 批并行子代理）
  ...
  └── 第 21 批：类二十五部署流程与升级管理审计专项（4 维度）= 4 维度 × 1 子代理
...
1. **修复顺序**：P0 → P1 → P2 → P3 → 长期治理 → 色卡发放业务规则修正 → 打印导出审计补齐 → 权限维度修正 → 业务主体维度修正（含加工商功能补齐）→ AI 模块修正（含可解释性+权限+监控补齐）→ 财务深化修正（含会计期间结账+应收催收+预算闭环补齐）→ CRM 全链路修正（含线索评分+客户池回收补齐）→ 报表 BI 通知协同修正（含报表订阅+通知去重补齐）→ 可观测性运维修正（含 trace 链路+故障转移补齐）→ 胚布拆匹质量修正（含拆匹规则+8D 流程补齐）→ 库存排程物料修正（含调拨+排程算法补齐）→ 组织定制物流修正（含 incoterms 补齐）→ 前端架构体验修正（含响应式+WCAG+前端测试补齐）→ 部署流程与升级管理修正（含 migrate CLI 实现+零停机部署+灰度升级+自动回滚补齐）
...
- 部署流程与升级管理相关代码（V15 类二十五审计对象）：
  - 部署脚本：`deploy/deploy.sh` + `deploy/deploy-frontend.sh` + `deploy/deploy-backend.sh` + `deploy/deploy-prepare.sh` + `deploy/deploy-latest.sh`
  - CLI 入口：`backend/src/bin/cli.rs` + `backend/src/bin/hash_password.rs`
  - CLI 顶层：`backend/src/cli/mod.rs`（Command 枚举：Admin/Migrate/Util）
  - Admin 子命令：`backend/src/cli/admin.rs`（密码哈希）
  - Migrate 子命令：`backend/src/cli/migrate.rs`（**占位未实现**，V15 需补齐）
  - Util 子命令：`backend/src/cli/util/mod.rs`（14 个子命令枚举 + 共享辅助函数 run_cmd/is_service_active/build_release_url/download_with_mirrors/fetch_with_mirrors/parse_json_field）+ `backend/src/cli/util/service.rs`（Status/Start/Stop/Restart/Logs/Health）+ `backend/src/cli/util/backup.rs`（Backup/Restore）+ `backend/src/cli/util/upgrade.rs`（Upgrade/Deploy/Rollback）+ `backend/src/cli/util/misc.rs`（Clean/Config/Info）
  - 路径校验工具：`backend/src/utils/path_validator.rs`（tar 解压路径穿越校验）
...
## 九、V15 未覆盖维度识别与建议（用户 2026-07-16 第十轮反馈"还有没有新的维度没有被审计到"）

> **本节目的一**：回答用户"还有没有新的维度没有被审计到"的问题，基于现有 25 大类 194 维度全量扫描，识别仍未被独立审计或仅部分覆盖的维度。
> **本节目的二**：为 V16 审计计划预留扩展方向（V15 完成后视项目状态决定是否触发）。

### 9.1 已部分覆盖但未独立成专项的维度（建议 V15 内合并加强）

| 维度 | 现状覆盖位置 | 建议补强方向 |
|------|------------|------------|
| 数据库迁移管理（schema migration） | 类七 7.3 第 5 项 + 类二十五 25.2 | 独立审计 046 个迁移文件命名规范/版本号/回滚脚本/数据迁移/索引迁移/外键约束/幂等性 |
| 备份恢复策略 | 类七 7.3 第 3 项 + 类二十五 25.1 | 独立审计备份频率/备份类型（全量/增量/差异）/备份验证/异地备份/加密备份/恢复演练/RPO/RTO |
| 灾难恢复（DR） | 类二十 20.4 故障转移 | 独立审计 RTO/RPO 目标/异地容灾/数据同步/故障切换/回切/演练周期/灾备演练报告 |
| 限流降级熔断 | 类二十 20.6 API 网关 | 独立审计限流策略（QPS/并发/用户级）/熔断阈值/降级方案/熔断后响应/恢复策略 |
| 配置管理（动态配置） | 类七 7.3 + 类二十五 25.4 | 独立审计配置中心/动态配置/配置版本/配置回滚/配置加密/配置审计/环境隔离 |
| 监控告警通知 | 类二十 20.2 metrics + 类十九 19.5 通知中心 | 独立审计告警规则/告警分级/告警去重/告警静默/告警通知渠道/告警值班/告警闭环 |

### 9.2 完全未覆盖的维度（建议 V16 新增专项）
...
| **数据归档与生命周期** | 中 | 审计历史数据归档策略（audit_logs/operation_logs/business_traces 等大表）/归档周期/归档存储/归档查询/数据销毁/数据保留合规 |
| **文档完整性** | 高 | 审计 API 文档（OpenAPI/Swagger）/用户文档/管理员文档/部署文档/开发文档/架构文档/变更日志的完整性、准确性、时效性 |
| **开发者体验（DX）** | 中 | 审计本地开发环境（docker-compose dev/seed 数据）/调试工具/热重载/代码生成器/MCP 工具/IDE 配置/贡献指南 |
| **国际化（i18n）后端** | 低 | 类七 7.2 + 类二十四 24.16 主要审计前端 i18n，后端 AppError 硬编码中文 163 处未独立审计（错误码国际化/邮件模板国际化/PDF 模板国际化） |
| **性能压测** | 高 | 类七 7.5 性能优化与缓存策略偏代码层，缺独立压测专项：压测场景/压测工具（k6/wrk）/压测指标（TPS/P99/错误率）/压测报告/性能基线/性能回归 |
| **安全渗透测试** | 高 | 类三安全性独立审计偏代码扫描，缺独立渗透专项：OWASP Top 10/SQL 注入/XSS/CSRF/SSRF/反序列化/越权/信息泄露/渗透报告 |
| **数据质量审计** | 中 | 审计数据完整性（外键孤儿记录/必填字段空值/枚举值合法/数据格式/数据范围）/数据一致性（跨表冗余字段一致/跨模块数据一致）/数据去重/数据清洗 |
| **第三方依赖治理** | 中 | 审计 npm/cargo 依赖许可证/依赖漏洞（npm audit/cargo audit）/依赖过期/依赖冲突/依赖替换/依赖镜像源 |
| **移动端适配** | 中 | 类二十四 24.1 响应式设计已覆盖部分，但缺独立移动端专项：移动端独立 App/PWA/移动端交互/移动端性能/移动端离线/移动端推送 |
| **无障碍（WCAG）后端** | 低 | 类二十四 24.13 WCAG 主要审计前端，后端无障碍包括：API 错误消息可读性/API 响应结构一致性/API 文档可访问性 |
| **绿色计算/能耗** | 低 | 审计代码能效（CPU/内存/IO 优化）/资源利用率/碳足迹/绿色部署/能耗监控（与项目 v14 批次 428 能耗管理业务模块区分，此处指代码自身的绿色计算） |

### 9.3 V15 现有维度补强建议

| 类别 | 补强建议 |
|------|----------|
| 类一回归验证 | 增加 v8-v14 修复项的**自动化回归测试**（非人工扫描），每个修复项关联单元测试/集成测试 |
| 类二通用代码质量 | 增加**代码复杂度定量指标**（圈复杂度/认知复杂度/嵌套深度/函数长度分布） |
| 类六测试体系 | 增加**测试有效性审计**（突变测试 mutation testing/测试覆盖率分支覆盖率/测试执行时间/测试稳定性 flaky test） |
| 类八法律合规 | 增加**数据出境合规**（GDPR/PIPL/数据本地化/跨境传输） |
| 类十六 AI 模块 | 增加**AI 模型漂移检测**（数据漂移/概念漂移/模型重训练触发机制） |
| 类十九报表 BI | 增加**报表数据准确性审计**（报表数据与源数据对账/报表计算逻辑验证/报表交叉验证） |
| 类二十可观测性 | 增加**混沌工程**（Chaos Engineering 故障注入/容错验证/弹性测试） |
| 类二十四前端 | 增加**前端性能预算**（Performance Budget 包体积/首屏时间/交互时间/资源数量上限） |

### 9.4 V16 触发建议

V15 全部修复完成后，建议 V16 重点覆盖：
1. **API 版本管理专项**（高优先级）
2. **性能压测专项**（高优先级）
3. **安全渗透测试专项**（高优先级）
4. **文档完整性专项**（高优先级）
5. **数据库迁移管理专项**（中优先级，V15 已部分覆盖）
6. **备份恢复与灾难恢复专项**（中优先级，V15 已部分覆盖）
7. **消息队列治理专项**（中优先级）
8. **任务调度治理专项**（中优先级）
9. **数据归档与生命周期专项**（中优先级）
10. **第三方依赖治理专项**（中优先级）

> **注**：以上未覆盖维度不强制在 V15 完成，可作为 V16 审计计划输入。V15 当前 25 大类 194 维度已覆盖项目核心审计需求。
