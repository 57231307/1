...
3. **RLS 激活的功能回归风险（已排查，2026-09-08）**（§3.5）：RLS 策略为 self 级（`owner_id/created_by = current_setting('app.user_id')`），应用层 DataScope 为三级（all/dept/self），粒度错位产生以下风险点：
   ...
   - 修复方向建议：①最小侵入方案——task-local 仅对 `DataScope::Self_` 绑定（dept 用户不设 GUC 走 fail-open，由应用层 dept 过滤兜底，与现状一致），同时为 crm_lead/crm_opportunity 策略补公海分支（`OR lead_status = 'pool'`）或回收时清 owner_id；②完整方案——迁移扩展策略支持 dept EXISTS 子查询（成本高，评估见下）。
   - **方案②（dept EXISTS）改动面评估（2026-09-08）**：
     - 前置事实：①应用层 5 张 RLS 表的 dept 分支全部把 `CreatedBy/OwnerId` 误传为 dept_column（如 order_query.rs:122），dept 用户实际执行 `created_by = department_id`——语义混淆的既有 bug，几乎空集；②5 张表均无 department_id 列；③AuthContext.department_id 来自 users.department_id（单值，另有 v15 的 user_departments 多部门表但 auth 未读取）；④策略现有第二分支 `app.role_code IN ('admin','gm','deputy_gm')` 无任何写入点（死分支）。
     - 改动文件与内容：A. 新增迁移（finance/mod.rs 追加幂等 SQL 或新域迁移）：5 张表各重写 CREATE POLICY，USING/WITH CHECK 各加 `OR EXISTS (SELECT 1 FROM users u WHERE u.id = current_setting('app.user_id', true)::int AND u.department_id = 该表创建人.department_id)` 形态——注意 5 张表无 department_id 列，EXISTS 必须反向关联（`u.id = GUC AND u.department_id = (SELECT department_id FROM users WHERE id = created_by)`），谓词成本翻倍且 suppliers/sales_orders 的 created_by 可空需 COALESCE/NULL 处理；需同步补 `idx_users_department_id`（已有，m0001:185）与 `idx_customers_owner`/`idx_suppliers_created_by`（已有），sales_orders.created_by 无索引需新增。B. `src/middleware/rls_context.rs`：task-local 从 `Option<i32>` 扩为携带 department_id 的结构体，钩子 set_config 增设 `app.department_id` GUC，策略改读双 GUC（或策略内 EXISTS 反查，无新 GUC）。C. 权限矩阵梳理：`init_service_ops/role.rs` 逐角色确认 dept 视角预期。D. 测试：finance 迁移的 PG 策略验证测试 + rls_context_test PG 锚点扩展 dept 断言。
     - 结论：改动集中在 2 个源码文件 + 1 个迁移 + 测试，但因①既有 bug（应用层 dept 分支本来就错）需先修复数据权限语义才谈得上 RLS 对齐——方案②实际牵出第三层工作（应用层 dept 过滤修复 + owner/dept 列设计决策：5 张表均无 department_id，EXISTS 反查 users 表的语义依赖「创建人的部门即数据部门」，转移 owner 后数据部门不变，与业务预期需产品确认）。净评估：方案②技术改动可控但业务语义决策成本高，维持建议先落地方案①。
