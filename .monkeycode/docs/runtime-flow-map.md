...
物流链路（`handlers/logistics_handler.rs`，不经事件总线、在请求内同步完成）：
建单（订单 → shipped，运单置 IN_TRANSIT）→ 登记送达（IN_TRANSIT → DELIVERED，写 actual_arrival）
→ 电子签收 `POST /logistics/{id}/sign`（DELIVERED → SIGNED，写 signed_by/signed_at，并在同一事务内
把该订单草稿状态的 ar_invoice 推进为 approved，即确认应收）。状态取值只认
`models/status::logistics_waybill`，`PUT /logistics/{id}` 无法把状态写成 SIGNED（会绕过应收确认），
已送达/已签收的运单也不可删除。前端 `views/logistics/` 的状态值以
`constants/waybill-status.ts` 为单一真相源。
