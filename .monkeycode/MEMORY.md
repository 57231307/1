...
## 二、当前任务状态（2026-07-05 批次 113 完成 - v7 复审 P1 持续修复中）
...
| 114 | - | - | P1-6 通知路径 warn 日志化（8 处）+ P1-5 启动期 expect 安全化（3 处中风险） | ⏳ |

### 剩余 v7 复审 P1 待修复项

| 类别 | 描述 | 处数 | 备注 |
|------|------|------|------|
| P1-3 | failover/database.rs dead_code | 4 | 需架构改动，备库 DB 连接 |
| P1-4 | cache/redis_client.rs 辅助 API 未接入 | 11 | 需 AppState 装配 + 监控端点 |
| P1-5 | 生产代码 .unwrap()/.expect() panic 点 | 11（3 中 + 8 低） | 中风险先修，低风险保留加注释 |
| P1-6 | let _ = 真实错误吞没 | 8 | 通知路径统一改 warn 日志 |
