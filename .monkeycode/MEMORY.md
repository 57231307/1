...
- Instructions:
  ...
  - **销售管理（47个子功能）**：
    ...
    - 销售价格：列表、创建、编辑、删除、详情、审批、历史、策略
    - 销售退货：列表、创建、编辑、删除、详情、提交审批、审批、拒绝
    - 销售分析：统计、趋势、排名、目标
  - **采购管理（58个子功能）**：
  ...
  - **CRM客户关系（53个子功能）**：
    ...
    - CRM客户：列表、创建、编辑、删除、详情、标签管理、联系人、360视图、标签CRUD
    - 公海客户池：列表、认领、回收
  ...
  - **生产管理（46个子功能）**：
    ...
    - MRP计算：执行计算、计算结果、需求查询、转单、历史记录
    - 生产排程：自动排程、甘特图、冲突检测、任务列表、更新排程、排程历史、排程结果、确认排程
    - 产能分析：产能概览、工作中心CRUD、可用产能预测、负荷分析、过载检查
  ...
  - **面料行业专用（41个子功能）**：
    ...
    - 染色批次：列表、创建、编辑、删除、详情、完成染色、按色号查询
    - 双计量单位：单位换算、双计量验证
    - 条码扫描：扫码查询、扫码发货、扫码历史
    - 物流管理：运单CRUD
  - **质量管理（15个子功能）**：
  ...
  - **其他功能模块（17个子功能）**：
    - 页面追踪：页面访问埋点
    - 指标监控：Prometheus指标
    - 系统初始化：状态检查、测试数据库、系统初始化、带数据库初始化、重置密码
