...
### 🔴 规则 19（最高优先级，2026-07-17 追加）：工具连接异常重试策略

> 遇到工具调用持续返回 `transport exception: connection refused (os error 111)` 或类似底层连接异常时，
> 必须每隔 30 分钟再次重试，直到连接恢复（OK）为止，禁止放弃。

**强制执行要求**：
- **触发条件**：RunCommand / Glob / LS / Read / Grep 等任意工具调用返回 `transport exception: connection refused` 或同类网络连接错误时
- **重试间隔**：每次重试之间等待 **30 分钟**（禁止使用更短间隔，避免加重服务负担；禁止使用更长间隔，避免拖延）
- **重试方式**：使用 `RunCommand` 执行 `sleep 1800 && echo retry-ok && date` 作为单次 30 分钟等待 + 探活
  - 若该命令成功执行（输出 `retry-ok` 和日期），视为连接恢复，立即继续原任务
  - 若该命令仍返回 `transport exception`，则继续下一个 30 分钟等待循环
- **终止条件**：仅当探活命令成功执行（连接恢复）才终止重试循环，进入正常任务流程
- **禁止行为**：
  - 禁止在重试间隔内频繁短重试（如 sleep 3/5/10/30 等），会加重工具服务负担
  - 禁止在未恢复前向用户报告"无法继续"并等待人工介入，应自主按 30 分钟周期持续重试
  - 禁止跳过或缩短等待时间
- **恢复后动作**：
  1. 立即继续被中断的原任务（不需重新询问用户）
  2. 在 doto.md / CHANGELOG.md 中记录此次工具连接异常事件及恢复时间
  3. 检查 main 分支状态、git 工作目录是否仍干净，如有偏差先恢复至异常前状态
