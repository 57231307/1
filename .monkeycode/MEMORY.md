...
> 最近整理：2026-07-17（自我迭代整理：移除重复的"规则 1-9"节 + 精简第六节批次修复细节 + 合并第七/八节重复内容 + 精简规则 15 下 v13/v14/V15 已完成冗长内容为完成状态表 + 新增规则 16 .monkeycode 全目录强制追踪 + 新增规则 17 审计文档同步规则 + 合并规则 0/8 真实实现强制 + **新增规则 18 规则自我迭代机制（四分类体系 PR/PH/IR/IH + 6 条触发条件 + 自动记录流程 + 迭代日志）** + 新增第十一节规则自我迭代日志（反向标注规则 0-17 来源分类 + 个人习惯观察 + 项目习惯观察 + 6 条迭代记录）+ 规则 3/4/7/9 标注 [个人规则] + 规则 9 与规则 18 对齐明确优先级顺序 + 新增规则 19 工具连接异常重试策略）。
> **二次迭代（2026-07-17）**：规则 19 增强为分级响应策略（瞬时抖动短重试 + 持续异常 30 分钟周期重试 + 等待期间非阻塞推理）+ 11.1 来源分类表补录规则 19 + 11.2 个人习惯观察新增"批次大小偏好"（6-8 文件）+ 规则 13 批次文件数从 1-3 修正为 6-8（消除规则内部矛盾）+ 11.4 迭代记录新增 4 条日志（规则 19 增强 / 批次大小偏好 / 规则 19 补录 / 规则 13 修正）。
...
### 🔴 规则 3（最高优先级，2026-07-06 追加）[个人规则]：项目成品导入/导出文档格式
...
### 🔴 规则 4（最高优先级，2026-07-06 追加）[个人规则]：项目文件 /// 注释精简规则
...
### 🔴 规则 7（最高优先级，2026-07-08 追加）[个人规则]：禁止简洁方案
...
### 🔴 规则 9（最高优先级，2026-07-08 追加，2026-07-17 与规则 18 对齐）[个人规则]：个人规则高于项目规则
...
### 🔴 规则 18（最高优先级，2026-07-17 追加）：规则自我迭代机制
...
### 🔴 规则 19（最高优先级，2026-07-17 追加，2026-07-17 二次迭代增强）：工具连接异常重试策略
...
> 2026-07-17 规则 18 新增后，反向扫描现有规则 0-17，标注来源分类。
...
| 规则 13 | 修复流程自动化与连续执行 | PR（项目规则） | 用户 2026-07-11 强调"CI 全绿后自动下一批，无需用户确认" | 2026-07-11 新增 |
