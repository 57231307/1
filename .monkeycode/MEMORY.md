...
**整理记录**：
- 2026-07-14（批次 398 后，轻量整理）：批次 398 完成配置合规性修复（PR #572 已合并 CI 全绿）；核心修复：①settings.rs 启动时同步 config.yaml env 字段到 APP_ENV（消除 is_production() 部署陷阱：config.yaml 设 env: "production" 但未设置 APP_ENV → is_production() 返回 false → 错误详情未脱敏、Cookie Secure 未启用）②.env.example 移除中文占位符密码（不进入 validate_secret 校验链路，易被原样复制）和 GRPC 残留变量③deploy-latest.sh 移除 grpc 死配置段④config.yaml.example 更新 env 字段注释；额外修复：clippy baseline 文件格式问题（274 行混合内容中仅 2 行是 warning: 摘要行，导致 116 条已存在 dead_code 警告被误判为新增，已重建为 118 条纯摘要行格式）；更新 doto.md 进度总览（97 完成 / 281 剩余，v14 低风险 7/74）+ CHANGELOG.md 追加批次 398 记录；下一批次 399：死代码补充清理 8 项；下一个整理点批次 405
