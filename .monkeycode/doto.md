...
| 🔄 待提交 PR | 1 批 | P1-batch04/05 续作（事件贯通+业财一致性，分支 feat/p1-batch04-05-fabric-logic，2 commit：① listener.rs 8 个缺失业务事件 match 分支消除 catch-all warn；② voucher.rs 移动加权平均成本 + compute_moving_average_cost 纯函数 + 8 单元测试） |
