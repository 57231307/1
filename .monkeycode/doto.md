...
| ⏳ 进行中 | 1 | D10（1000 行文件拆分，第 5 批 4/4 完成，待推进第 6 批） |
...
- **当前批次**：Batch 488 进行中（15/17 完成）—— D 系列已完成 15 项（详见 [doto-su.md §V15 Batch 488](file:///workspace/.monkeycode/doto-su.md)）+ D08 全部完成（第一/第二梯队 28 函数 + 第三梯队 53 函数 + 第四梯队 84 函数 + D09 收尾 2 个100+行函数 = 167 函数，PR #669-#682）+ D09 全部完成（D08 子集，精确扫描 100+行函数仅2个已拆分）+ D10 第 1 批完成（ar_service/production_order_service/so/delivery 3 个 >1800 行文件，PR #683/#684）+ D10 第 2 批完成（voucher_service/energy_service/outsourcing_service/business_mode_service 4 个 >1700 行文件，PR #685/#686 main f836552/882cecc）+ D10 第 3 批 2/4 代码完成待推送 CI（chemical_service.rs 1730→349 facade + chemical_ops 6 子模块 43 方法 + bi_analysis_service.rs 1711→317 facade + bi_analysis_ops 6 子模块 20+ 方法，分支 d10-batch3a-chemical-bi，遵循 D10 facade 模式，规则13禁止本地编译待 CI 验证）；剩余 2 项大型任务（D05/D13/D14）2026-07-19 精确审计完成（详见 §4 各项条目）
...
- **批次规划**：
  ...
  - 第 6 批：剩余 15 个 1000-1200 行文件（doto 原记录 11 个，核实实际 15 个，含 D10-1 副产物 ar_ops/verification.rs 1062 行）：✅ event_bus.rs (1243→240 facade + event_bus_ops/{mod 20, publish 298, subscribe 410, retry 234}，3 Service 23 方法) + ✅ po/order.rs (1234→310 facade + po/order_ops/{mod 17, crud 470, state 222, receive 188}，PurchaseOrderService 18 方法) + ✅ auth_service.rs (1201→459 facade含测试 + auth_service_ops/{mod 20, auth 487, jti 357}，AuthService 12 方法 + 7 JTI free functions) + ✅ inventory_finance_bridge_service.rs (1192→410 facade含测试 + inventory_finance_bridge_ops/{mod 17, sync 350, recon 230, voucher 178}，3 Service 15 方法，PR #698 main 9d26d7d，--admin 合并) / ✅ lab_dip_service.rs (1188→230 facade + lab_dip_ops/{mod 23, types 171, request 334, sample 305, resample 284}，LabDipService 16 方法) + ✅ production_recipe_service.rs (1181→631 facade含测试 + production_recipe_ops/{mod 24, recipe_crud 369, recipe_state 72, addition 185}，2 Service 14 方法) + ✅ product_service.rs (1075→170 facade + product_ops/{mod 17, sync 58, crud 338, color 143, import_export 451}，ProductService 18 方法) + ✅ system_update_service.rs (1074→459 facade含测试 + system_update_ops/{mod 20, status 179, apply 221, backup 117, github 229}，SystemUpdateService 14 方法，PR #700 main 325dfed，CI 修复 3 轮：2 unused sea_orm::prelude + 6 doc overindented + PaginatorTrait + 8 警告入 baseline，--admin 合并) / ⏳ 第 6b-2 批 4 文件待推进：purchase_receipt_service.rs (1074) / ar/recon.rs (1070) / ar_ops/verification.rs (1062 ⚠️ D10-1 副产物) / bpm_service.rs (1060) / ⏳ 第 6b-3 批 3 文件待推进：bom_service.rs (1046) / import_export_service.rs (1018) / main.rs (1005)
