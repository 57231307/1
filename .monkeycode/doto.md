...
> 含原 75 项 + 代码深挖审计新增 22 项（A.5/A.6/A.20 与 30.1/30.2/30.3 重叠不重复计）
...
| 30.1 | 状态机实现风格统一 | 缸号=DB规则表驱动、8D=枚举+payload匹配、定制订单=utils纯函数、BPM=JSON图遍历，四范式并存，新域接入需选型，应统一为可插拔状态机框架 | P2 | 架构技术债 |
| 30.2 | 调度器统一框架 | notification_scheduler/report_subscription_scheduler/stock_alert_notification_scheduler/color_card_issue_scheduler 各自为政，需统一任务调度框架 | P2 | 架构技术债 |
| 30.3 | 迁移目录按域重组 | 迁移按版本批次命名(v15_batch18…)持续膨胀，考虑按业务域重组，随时间推移目录会无限增长 | P2 | 架构技术债 |
