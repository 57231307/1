...
**子任务 2.2：view 表格逻辑接入 useTableApi（22/56 完成 🔄）**
...
- 批次 275：`notification/index.vue`（移除 listNotifications + pagination ref，onMounted 仅保留 fetchUnreadCount） / `warehouse/index.vue`（移除手写 warehouses/total/loading + 修复翻页 bug + Warehouse[] 类型断言） / `bom/index.vue`（移除手写 boms/total/loading）
- 批次 276：`customer/index.vue`（移除手写 customers/total/loading + 修复翻页 bug + 类型断言） / `system/tabs/UserTab.vue`（移除手写 users/userTotal/userLoading + 修复翻页 bug，refresh 别名 fetchUsers 保留 defineExpose） / `inventoryBatch/tabs/BatchListTab.vue`（移除手写 batchList/loading/pagination，refresh 别名 fetchBatches 保留 defineExpose）
...
**待修复文件清单**（剩余 34 个 ⏳，优先级排序，批次 277 候选：inventoryCount/tabs/CountListTab.vue + sales-contract + sales-price）：
