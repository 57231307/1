...
- **当前进度**：D10-1 ✅ 完成（ar_service.rs 2489→259 行 facade + 5 子模块 2256 行，PR #683 main 34b8cae）；D10-2 ✅ 完成（production_order_service.rs 2141→689 行 facade + production_order_ops/{mod,types,crud,completion,approval} 5 子模块 1628 行，41 方法按职责分散到多 impl 块，PR #684 main 0385401）；D10-3 ✅ 完成（so/delivery.rs 2095→822 行 facade + delivery_ops/{mod,types,ship,inventory,cancel,export} 6 子模块 1403 行，30 方法按职责分散到多 impl 块，PR #684 main 0385401）；D10-2a ✅ 完成（voucher_service.rs 2058→882 行 facade + voucher_ops/{mod,crud,workflow,balance,assist} 5 子模块，39 方法 5+12+11+11，PR #685 main f836552）；D10-2b ✅ 完成（outsourcing_service.rs 1879→436 行 facade + outsourcing_ops/{mod,types,order,order_item,receipt,voucher} 6 子模块 + business_mode_service.rs 1739→741 行 facade + business_mode_ops/{mod,types,config,flow_step,rule,order_link} 6 子模块，PR #686 main 882cecc）；D10-3a ⏳ 代码完成待推送 CI（chemical_service.rs 1730→349 行 facade + chemical_ops/{mod,types,master,category,lot,requisition} 6 子模块 43 方法 + bi_analysis_service.rs 1711→317 行 facade + bi_analysis_ops/{mod,types,sales,profit,drilldown,olap} 6 子模块 20+ 方法，分支 d10-batch3a-chemical-bi，规则13禁止本地编译待 CI 验证）；第 1 批 3 个 >1800 行文件全部完成，第 2 批 4/4 完成，第 3 批 2/4 代码完成
- **批次规划**：
