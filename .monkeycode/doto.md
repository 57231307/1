...
- **当前批次**：Batch 482 已合并（PR #667）—— P0-B05 大额调拨 + P0-B06 预算超支 + P0-B07 CRM 回收 + P0-B08 赢率 + P0-B09 输单原因 + P0-B14 Incoterms 完成（fund_management_service.rs large_transfer_threshold() 10万阈值 + fund_dto.rs confirm_large 字段 + budget_management_service.rs enforce_budget_available 阻塞式 + po/price.rs check_and_occupy_budget ? 传播 + po/order.rs 事务回滚 + crm/recycle_executor.rs RecycleExecutor 6h 定时扫描 + main.rs 后台任务 + opp.rs default_win_probability_by_stage 5阶段映射 + close_as_lost 输单原因 + crm_dto.rs CloseAsLostRequest + crm_handler.rs close_opportunity_as_lost + routes/crm.rs /close-lost + incoterms.rs 11种术语全量 = 13 文件；CI 2 轮：第 1 轮 E0382 borrow of moved value v 在 Set(Some(v)) 后 &v 借用，第 2 轮改为先算 default_prob 再 Set 后 14/14 全绿；教训：变量移动顺序需在 Set 前计算衍生值 + rust_decimal 1.42 Decimal::new 非 const fn 已确认（docs.rs 验证 pub fn new 非 pub const fn new））
- **下一批次**：Batch 483（P0-B10 BI 权限过滤 + P0-B11 定制订单打样报价 + P0-B12 售后质量集成 + P0-B13 物流电子签收，4 项打包 ~11 文件，B12 依赖 480 的 8D 流程）
