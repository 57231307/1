...
- [x] E2E 分片 8 failure 根因（认证 token 拉到完整日志+后端 PG 日志）：**`column outsourcing_order_item.piece_no does not exist`**——匹号二期（e0fc24e）给 model 加了 piece_no 字段但迁移遗漏 ALTER TABLE，E2E 发料链路（issue → list_by_order）SQL 全部报错。已补迁移（v15/mod.rs:2197，本地 PG 事务回滚验证语法通过）
- [x] Rust 测试 (2/30) failure 根因（认证 token 拉到 nextest 报告）：`middleware_public_routes_test.rs:40` 断言 `!is_public_path("/init/test-database")`——本批把 test-database 加入 PUBLIC_PATHS（Setup 向导匿名放行），安全契约测试需同步。已更新：test-database 公开 + 变体路径不放行 + handler 三层门禁契约注释
...
### 推送前自审强化（用户批评整改，2026-09-08）

- [x] 批评点：连续"修一个推一个"未做推送前关联性自审。整改：等待 CI 期间完成 15 项关联性自审（A-O），覆盖双模式形态/Cookie 轮换/进程重启/CSRF 契约/请求体字段/权限注入六类交互面
- [x] 自审结论：当前 HEAD（05a01a4）spec 与前端、脚本、CI 环境变量、后端中间件栈的契约全部对齐；十轮已实证 test-database 200 + 初始化真实落库，十一轮验证断言修正后的完整 9 测试链
