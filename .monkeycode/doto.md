...
| 475c | A | P0-S12 前端导出接入后端（B 类批次 1/3）| XL | 474 | inventory + warehouse + production 3 模块：后端新增 3 端点（export_stock/export_warehouses/export_production_orders）+ 前端切换 4 文件（inventory/index.vue + warehouse/index.vue + usePrdProc.ts + production/index.vue）+ 3 路由注册 + 自审门 = ~10 文件 |
