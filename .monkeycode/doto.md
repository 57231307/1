...
- `backend/src/services/purchase_order_service.rs` — 采购订单列表分页

**特殊处理**：
- `quotation_service.rs`：返回类型是 `ServiceError`，需添加 `From<AppError> for ServiceError` 转换或改用 `AppError`
- 需先扫描确认这些文件确实使用手写分页逻辑
