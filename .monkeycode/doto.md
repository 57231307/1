...
### 门禁覆盖真相（本轮最重要发现，影响此前所有"已收口"结论）
`check-api-envelope` 长期报 `344 可比对 / 0 失配`，被当作响应契约收口的依据。实测它漏了三类写法：
1. **`request.get<ApiResponse<T>>(url)`**：类型挂在调用表达式上、函数没有 `: Promise<...>` 签名，
   整类不进统计 —— 全仓 49 个 api 文件共 **500 处**（与本已统计的 505 处同量级）。
2. **带类型实参的具名类型**：只匹配 `^Foo$`，`PageResult<T>` 这类万能泛型整体落盲区；
   且它同时给 data/list/items 多个规范键，"怎么返回都对"，本就不该被判为一致。
3. **handler 定位**：不认目录型模块（`pub mod advanced;` → `handlers/advanced/*.rs`，
   12 条被误报成"编译期即失败"），也不认"router 注册在 handler 文件内部"（`sales_return_handler::router()`）
   的裸名 handler。

补齐 + 加"详情聚合体"判据（`{bom, items}` 不是列表信封，消掉 5 条假失配）后：
可比对 344 → **670**，handler 符号未定义 12 → **0**，新暴露真实信封漂移 **15** 条（本轮修）、
