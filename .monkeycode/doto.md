...
> 最近梳理：2026-07-17（规则 13 三次迭代新增步骤 4"修复后推送前自审"，与规则 20 联动；按业务模块重组未完成任务 + 新增依赖关系图 + 新增批次规划表 + 新增工作量预估；P0 进度 65/104；剩余 39 P0 + 257 P1 + 248 P2 + 123 P3 = 667 项；规划 25 个批次完成 P0；下一批次 473 起按业务模块顺序推进；暂停指令：等待新指令）。
...
- **当前批次**：Batch 472 已合并（PR #648，squash 99c2af1），复审 PR #649 已合并（squash 534f3c6），文档梳理 PR #650 已合并（squash 7fc03d1）
- **暂停指令**：用户 2026-07-17 明确"修复完 472 批次后暂停,等待新指令"
- **下一批次**：Batch 473（模块 A 安全权限：P0-S14 migration + P0-S19 审计字段补齐）
- **执行策略**：规则 13+14+15+20 联动；CI 全绿后自动进入下一批；所有警告视为错误必须真实修复；修复前必须调研现有实现禁止重复造轮子；注释必须与功能一致禁止随意编写（规则 20）
...
| 批次 | 模块 | 任务 | 工作量 | 依赖 | 说明 |
|------|------|------|--------|------|------|
| 473 | A | P0-S14 migration 047 + P0-S19 审计字段补齐 | S+M | 无 | 复审重新打开项优先；解锁后续安全链路 |
...
- **依赖**：P0-S14（migration 先行）/ P0-S15（水印先行）
- **工作量**：XL
...
- **复审状态**：⚠️ 复审发现 migration 047 完全缺失（详见 [doto-su.md §V15 复审核实发现的已完成项](file:///workspace/.monkeycode/doto-su.md)）
- **修复方案**：
...
| 规则 13 | 🔴 | 修复流程自动化：CI 全绿后自动开始下一批，无需用户确认；**步骤 4 修复后推送前自审**（内容正确性+注释规范性+注释一致性，与规则 20 联动） |
