...
> 最近梳理：2026-07-17（规则 13 四次迭代新增步骤 0"确定审计结果内容是否存在"修复前置门 + 步骤 4"修复后推送前自审"与规则 20 联动；按业务模块重组未完成任务 + 新增依赖关系图 + 新增批次规划表 + 新增工作量预估；Batch 473 已合并 PR #656（P0-S14 migration + P0-S19 审计字段补齐）；P0 进度 67/104；剩余 37 P0 + 257 P1 + 248 P2 + 123 P3 = 665 项；规划 25 个批次完成 P0；下一批次 474 起按业务模块顺序推进；规则 13 连续执行）。
...
| 优先级 | 总数 | 已完成 | 未完成 | 完成率 |
|--------|------|--------|--------|--------|
| **P0 阻塞级** | 104 | 67 | **37** | 64.4% |
| **P1 高优先级** | 257 | 0 | **257** | 0% |
| **P2 中优先级** | 248 | 0 | **248** | 0% |
| **P3 低优先级** | 123 | 0 | **123** | 0% |
| **合计** | **732** | **67** | **665** | **9.2%** |

> P0 已完成 67 项 = 原 62 项 + 复审发现已完成 4 项（P0-S08/S16/F14/T04）- 复审重新打开 1 项（P0-S14）+ Batch 473 修复 2 项（P0-S14 migration 补齐 + P0-S19 condition 字段补齐）。

### 1.2 状态：🔄 规则 13 连续执行中

- **当前批次**：Batch 473 已合并（PR #656，squash e19c1aa）—— P0-S14 migration 补齐 + P0-S19 审计字段补齐
- **下一批次**：Batch 474（模块 A 安全权限：P0-S15 导出水印 + P0-S12 前端导出接入后端核心页面）
- **执行策略**：规则 13+14+15+20 联动；CI 全绿后自动进入下一批；所有警告视为错误必须真实修复；修复前必须调研现有实现禁止重复造轮子；注释必须与功能一致禁止随意编写（规则 20）；规则 13 步骤 4 自审必须 grep 所有引用新字段/新结构体的调用点
...
| 474 | A | P0-S15 导出水印 + P0-S12 前端导出接入后端（核心页面） | M+XL | 473 | 前端 25+ 页面分批改造，本批先做核心 5 个 |
| 475 | A | P0-S12 前端导出接入后端（剩余页面）+ P0-S13 审计日志导出 | XL+M | 474 | 完成 25+ 页面改造 + 审计日志闭环 |
...
### 4.1 模块 A：安全与权限（5 项，原 7 项 - Batch 473 完成 P0-S14 + P0-S19）
...
- **依赖**：✅ P0-S14 已完成（Batch 473）/ P0-S15 水印先行
- **工作量**：XL
- **批次**：474-475（拆 2 批，核心页面 + 剩余页面）
