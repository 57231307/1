...
**业务场景覆盖**：原称 6/16 = 37.5%，但"16 个场景"计数不准（销售出库单=销售发货单，应合并），实际去重约 15 个初始场景；二次查漏又发现约 26 个业务实体需打印，合计约 60 场景分批推进。
...
| ❌ 待实现（A1-A4） | 销售发货单(=销售出库单)/采购合同/库存盘点单/工资单 | 原清单未实现项 |
| ❌ 待实现（A1 纺织专用） | 生产流转卡/验布打卷单/染色技术卡/色卡发放单/大货批色单/卷标签·条码标签/打样单 Lab Dip/生产任务单/质检记录 | 纺织核心+业务+生产 |
| ❌ 待实现（A2 P0） | 收款单/付款单/销项·进项发票/销售报价单/销售退货单/采购退货单/委外加工单/委外收货单/物流运单/产地证/出口报关单/危废五联单/不合格品单/染化料领用单 | 通用+法定 |
