...
> 最近整理：2026-07-27（P0 全部完成，P1 修复进行中：P1-A + P1-B1 + P1-B2 已推送 PR #758，待 CI 验证）
...
| 状态 | 数量 | 批次 |
|------|------|------|
| ✅ 代码完成待 CI | 3 批 | P1-A（安全加固6项）、P1-B1（法律合规5项）、P1-B2（用户协议后端+DbErr修复） |
| ⏳ 进行中 | 0 批 | — |
...
| 批次 | 内容 | 状态 |
|------|------|------|
| P1-A | batch-03 类三安全性 6 项 | 🔵 待 CI |
| P1-B1 | batch-08 类四法律合规 5 项（脱敏/HTTPS/前端协议） | 🔵 待 CI |
| P1-B2 | batch-08 + batch-02（用户协议后端 + DbErr→AppError） | 🔵 待 CI |
...
- **当前批次**：Batch 10 ✅ 已合并 main（PR #754 cf6aac4）—— D05 Batch 10: 60 个 .vue 文件 i18n 接入完善 + locales 双语同步修复（zh-only 28 键补全到 en-US.ts）+ V2Table/AdvancedFilter withDefaults hoist 修复 + tests/setup.ts 全局 i18n 注入 + 7 个 Batch 10 工具脚本
- **上一批次**：Batch 9 ✅ 已合并 main（PR #753 40dc7f6）—— D05 Batch 9 useI18n 接入收尾（18 文件，接入率 100%）
- **接入率验证（2026-07-26 合并后核实）**：375/375 .vue 文件 100%（useI18n 导入 0 缺失 + 0 模板硬编码中文 + 0 脚本字符串硬编码中文 + 8947 个唯一翻译键 zh-CN/en-US 双语 0 缺失）
- **CI 验证（PR #754）**：13 项全绿（前端格式/ESLint/类型检查/测试/构建 + Rust 格式/Clippy/单元测试/后端构建 + 依赖审计 + 环境信息 + 依赖图记录 + 构建通知），仅 Rust 覆盖率非阻塞失败（已知问题）
...
### 3.1 P0-D05 useI18n 接入（类七，XL，✅ 已完成）
...
- **依赖**：无前置依赖
- **工作量**：XL
- **批次**：493-494（D08 独立批次；2 子批次规划，实际仅执行 Batch 1，Batch 2 无需执行）
...
- **梯队规划**：
  ...
  - D08 Batch 1（services/ 目录 39 函数）：✅ 全部完成（PR #740 main 88af0f1，4 并行代理拆分 39 个 >80 行函数，主函数 ≤50 行 + helper ≤50 行 + 公共 API 签名不变 + 无 #[allow] + 注释精简 1 行）
