...
- **下一批次**：Batch 488（P0-D01~D17 全部 17 项一次性打包，用户指令合并为单批；预计 ~44 文件；CI 全绿后立马推进；当前 Batch 487 CI 因 criterion 在 [dev-dependencies] 不允许 optional 导致 Rust 后端构建 failure，已修复将 criterion 移到 [dependencies]，待 CI 重跑验证）
