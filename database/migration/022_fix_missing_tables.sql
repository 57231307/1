...
CREATE TABLE IF NOT EXISTS email_logs (
    ...
    sent_at TIMESTAMPTZ WITH TIME ZONE,
...
-- 3. 创建 report_subscriptions 表（如果不存在）
CREATE TABLE IF NOT EXISTS report_subscriptions (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'ACTIVE',
    next_run_at TIMESTAMPTZ WITH TIME ZONE,
    last_run_at TIMESTAMPTZ WITH TIME ZONE,
...
-- 7. 插入默认数据
INSERT INTO email_templates (name, code, subject_template, body_template, template_type, description, is_active, status, created_by)
VALUES
    ('系统通知', 'SYSTEM_NOTIFICATION', '系统通知: {{title}}', '<h1>{{title}}</h1><p>{{content}}</p>', 'NOTIFICATION', '系统默认通知模板', true, 'ACTIVE', 1),
    ('审批通知', 'APPROVAL_NOTIFICATION', '审批通知: {{title}}', '<h1>{{title}}</h1><p>您有一个新的审批任务</p>', 'WORKFLOW', '审批流程通知模板', true, 'ACTIVE', 1),
    ('库存预警', 'STOCK_ALERT', '库存预警: {{product_name}}', '<h1>库存预警</h1><p>产品 {{product_name}} 库存不足</p>', 'ALERT', '库存预警通知模板', true, 'ACTIVE', 1)
