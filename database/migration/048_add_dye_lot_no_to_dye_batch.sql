...
-- 2. 创建索引：按 dye_lot_no 查询（缸号追溯/四维关联查询高频使用）
CREATE INDEX IF NOT EXISTS "idx_dye_batch_dye_lot_no" ON "dye_batch" ("dye_lot_no");

-- 3. 字段注释
COMMENT ON COLUMN "dye_batch"."dye_lot_no" IS '染色批次号（染缸号），面料四维标识之一：product_id+color_no+dye_lot_no+batch_no。历史数据回填 DEFAULT，新数据由创建接口传入。';
