...
def update_page(file_path, entity_type):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # We need to find the specific `html! {` inside `fn view`.
    # Let's locate `fn view`
    view_start = content.find('fn view(')
    html_start = content.find('html! {', view_start)
    
    # We need to find the matching closing brace for `html! {`
    brace_count = 0
    html_end = -1
    for i in range(html_start + 6, len(content)):
        if content[i] == '{':
            brace_count += 1
        elif content[i] == '}':
            brace_count -= 1
    ...
    if html_start == -1 or html_end == -1:
        print(f"Could not parse HTML block in {file_path}")
        return

    # Let's replace the whole HTML structure for the list view to match the new spec
                ...
                <div class="space-y-4">
                    ...
                    <div class="card p-4 flex flex-wrap gap-3 items-center">
                        <div class="w-full md:w-[200px]">
                            <input type="text" placeholder="客户名称/编号" value={self.keyword.clone()} oninput={on_keyword_change} />
                        </div>
        ...
        new_html = """html! {
            <MainLayout current_page={"供应商管理"}>
                <div class="space-y-4">
                    ...
                    <div class="card p-4 flex flex-wrap gap-3 items-center">
                        <div class="w-full md:w-[200px]">
                            <input type="text" placeholder="供应商名称/编号" value={self.keyword.clone()} oninput={on_keyword_change} />
                        </div>
                        <div class="w-full md:w-[150px]">
    ...
    # Replace from `html! {` to the end of the `view` function
    new_content = content[:html_start] + new_html + content[html_end+1:]
    
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_content)

update_page('frontend/src/pages/customer.rs', 'customer')
update_page('frontend/src/pages/supplier.rs', 'supplier')
print("Updated customer and supplier pages")
